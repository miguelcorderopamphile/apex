...
Esta tasa afectará todos los nuevos cobros y tickets a partir de este momento.`)){const p=await F("fijar_tasa_manual",{tasa:d.toFixed(2)});p&&this.pintar(p),n()}})}async actualizar(){const e=await this.obtenerTasa("obtener_tasa_bcv");e&&this.pintar(e)}async forzar(){const e=document.getElementById("bcv-refresh");e&&e.classList.add("animate-spin");const o=await this.obtenerTasa("forzar_actualizacion_tasa");e&&e.classList.remove("animate-spin"),o&&this.pintar(o)}async obtenerTasa(e){try{return await F(e)}catch{return null}}pintar(e){const o=document.getElementById("bcv-valor"),a=document.getElementById("bcv-fluctuacion");if(!o||!a)return;const n=Number(e.valor);o.textContent=Number.isFinite(n)&&n>0?n.toLocaleString("es-VE",{minimumFractionDigits:2,maximumFractionDigits:2}):"--",Number.isFinite(n)&&n>0&&window.dispatchEvent(new CustomEvent("tasa_actualizada",{detail:n}));const c=Number(e.fluctuacionPct);if(e.direccion&&Number.isFinite(c)){const i=e.direccion==="subio"?Ve:e.direccion==="bajo"?Ge:He,b=c>0?"+":"";a.textContent=`${b}${c.toFixed(2)}%`,a.style.color=i}else a.textContent=""}}class Ke{constructor(){A(this,"config",null);A(this,"rol","caja");A(this,"tasa",0);A(this,"oyentes",[])}suscribir(e){this.oyentes.push(e)}notificar(){this.oyentes.forEach(e=>e())}async cargarConfig(){return this.config=await D.config(),this.config}getConfig(){return this.config}async inicializar(e,o,a,n,c){await D.inicializar(e,o,a,n,c),await this.cargarConfig(),this.notificar()}get rubroAbasto(){var e,o;return(((e=this.config)==null?void 0:e.rubros)??0&ye)!==0||(((o=this.config)==null?void 0:o.rubros)??0)===ye}tieneRubro(e){var o;return((((o=this.config)==null?void 0:o.rubros)??0)&e)!==0}tieneCapacidad(e){var o;return((((o=this.config)==null?void 0:o.capacidades)??0)&e)!==0}setRol(e){this.rol=e,this.notificar()}get rolActual(){return this.rol}setTasa(e){this.tasa=e,this.notificar()}get tasaActual(){return this.tasa}bs(e){return(Number(e)*this.tasa).toLocaleString("es-VE",{minimumFractionDigits:2,maximumFractionDigits:2})}}function We(u,e){const o=u.replace(/[^a-zA-Z0-9]/g,"");if(o.length!==16)return!1;const a=o.split("").map(b=>parseInt(b,10)).filter(b=>!isNaN(b)&&b<10);if(a.length!==16)return!1;let n=0;for(let b=0;b<12;b++)n+=a[b]*(b+1);const c=n%1e4,i=a[12]*1e3+a[13]*100+a[14]*10+a[15];return c===i}class Ze{constructor(e,o,a){A(this,"contenedor");A(this,"modelo");A(this,"alTerminar");A(this,"modo","nuevo");A(this,"bloquearPanel",!0);this.contenedor=e,this.modelo=o,this.alTerminar=a}render(){this.contenedor.innerHTML=`
                ...
                <div id="wz-sec-nuevo" class="${this.modo==="nuevo"?"block":"hidden"} space-y-5">
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                            ...
                            ${this.tarjetaRubro("wz-panaderia","Panadería","Horneados FEFO",Ae,"bg-brand-yellow")}
        ...
        <div class="fixed inset-0 bg-black/40 flex items-center justify-center z-[100] p-4">
            ...
            `,"max-w-md"),(n=this.modal.querySelector("[data-cerrar]"))==null||n.addEventListener("click",()=>this.cerrarModal()),(c=this.modal.querySelector("#btn-cancelar-inicio-jornada"))==null||c.addEventListener("click",()=>this.cerrarModal()),(i=this.modal.querySelector("#form-nuevo-cajero-inicio"))==null||i.addEventListener("submit",async t=>{t.preventDefault();const r=this.modal.querySelector("#in-nuevo-cajero-inicio"),s=(r==null?void 0:r.value.trim())||"";s&&(await D.crearOperador(s),this.cerrarModal(),this.abrirModalIniciarJornada())}),(b=this.modal.querySelector("#btn-abrir-turno"))==null||b.addEventListener("click",async()=>{const t=this.modal.querySelectorAll("input[data-inicio-cajero-chk]:checked"),r=[];if(t.forEach(d=>{d.value&&r.push(d.value)}),r.length===0){this.mostrarToast("Selecciona al menos un operador para abrir el turno.","error");return}const s=r[0];await D.abrirJornada(s,r),this.cerrarModal(),this.renderDinamico()})})()}async abrirModalCobro(){var x,h,g,v;if(this.vm.lineasCarrito.length===0)return;let e=[];try{e=await D.listarMetodosPago()}catch{e=[{nombre:"PUNTOD.VENTA",moneda:"BS"},{nombre:"BIOPAGO",moneda:"BS"},{nombre:"PAGO MOVIL",moneda:"BS"},{nombre:"TRANSF.BS.",moneda:"BS"},{nombre:"BS.EFEC.",moneda:"BS"},{nombre:"DOL.CASH",moneda:"USD"},{nombre:"ZELLE",moneda:"USD"},{nombre:"BINAN.USDT",moneda:"USD"}]}const o=Number(this.vm.totalUsd.toFixed(2)),a=this.vm.tasaTicket,n=Number((o*a).toFixed(2)),c=e.find(E=>E.nombre==="BIOPAGO")||e.find(E=>E.moneda==="BS")||e[0]||{nombre:"BIOPAGO",moneda:"BS"};let i;if(this.pagosBorrador&&this.pagosBorrador.length>0){if(i=this.pagosBorrador.map(E=>({...E})),i.length===1&&i[0]){const E=i[0];E.monto=E.moneda==="USD"?o:n,E.tasaCambio=a}else if(i.length>1){let E=0;for(let _=0;_<i.length-1;_++){const f=i[_],w=f.tasaCambio>0?f.tasaCambio:a;E+=f.moneda==="USD"?f.monto*w:f.monto}const I=Math.max(0,Math.round((n-E)*100)/100),L=i[i.length-1];L&&(L.monto=L.moneda==="USD"?a>0?Number((I/a).toFixed(2)):0:I,L.tasaCambio=a)}}else i=[{id:"p-1",metodo:c.nombre,moneda:c.moneda,monto:c.moneda==="USD"?o:n,tasaCambio:a,referencia:""}];let b=((x=this.resolucionVueltoBorrador)==null?void 0:x.estado)||"PAGADO",t=((h=this.resolucionVueltoBorrador)==null?void 0:h.metodo)||"BS.EFEC.",r=((g=this.resolucionVueltoBorrador)==null?void 0:g.tasa)||a;e.some(E=>E.nombre===t)||(t=((v=e[0])==null?void 0:v.nombre)||"BS.EFEC.");let s=!1;const d=()=>{let E=0;for(const P of i)if(P.moneda==="USD"){const T=P.tasaCambio>0?P.tasaCambio:a;E+=P.monto*T}else E+=P.monto;E=Math.round(E*100)/100;const I=a>0?Math.round(E/a*100)/100:0,L=Math.max(0,Math.round((n-E)*100)/100),_=a>0?Math.max(0,Math.round(L/a*100)/100):0,f=Math.max(0,Math.round((E-n)*100)/100),w=a>0?Math.max(0,Math.round(f/a*100)/100):0,y=E>=n-.01&&i.every(P=>P.monto>0);return{cubiertoUsd:I,cubiertoBs:E,faltanteUsd:_,faltanteBs:L,vueltoUsd:w,vueltoBs:f,puedeConfirmar:y}},l=()=>{const E=d(),I=e.filter(C=>C.moneda==="USD"),L=e.filter(C=>C.moneda==="BS"),_=i.map((C,G)=>{const q=I.map(k=>`<option value="${k.nombre}" ${k.nombre===C.metodo?"selected":""}>${k.nombre} ($ - Tasa Dinámica)</option>`).join(""),B=`
                        <optgroup label="MÉTODOS EN BOLÍVARES (Bs.)">
                            ${L.map(k=>`<option value="${k.nombre}" ${k.nombre===C.metodo?"selected":""}>${k.nombre} (Bs.)</option>`).join("")}
                        </optgroup>
                        <optgroup label="MÉTODOS EN DÓLARES ($ - TASA PROPIA)">
                            ${q}
                        </optgroup>
                    `,N=C.tasaCambio>0?C.tasaCambio:a,S=C.moneda==="USD"?C.monto*N:C.monto,j=a>0?S/a:0;return`
                    <div class="border-2 border-brand-black rounded-lg p-3 bg-gray-50 mb-2.5">
                        ...
                        `}
                    </div>`}).join(""),f=e.find(C=>C.nombre===t),w=(f==null?void 0:f.moneda)==="USD",y=r>0?r:a,P=w&&y>0?Number((E.vueltoBs/y).toFixed(2)):E.vueltoUsd,T=E.faltanteBs>.009?`
                    <div class="bg-amber-50 border-2 border-amber-500 rounded p-3 text-center mb-3">
                        ...
                        <p class="font-heading font-black text-xl text-amber-700">Faltan Bs. ${Y(E.faltanteBs)} · <span class="text-base text-amber-900">$ ${Y(E.faltanteUsd)} (Equiv. Oficial)</span></p>
                    </div>`:E.vueltoBs>.009?`
                    <div class="border-2 border-emerald-700 bg-emerald-50/70 rounded-lg p-3 mb-3">
                        <div class="flex flex-wrap items-center justify-between gap-1 mb-2">
                            <div>
                                ...
                                <span class="font-heading font-black text-xl text-emerald-800">Bs. ${Y(E.vueltoBs)}</span>
                            </div>
                            <span class="text-xs font-bold text-emerald-900 font-mono">$ ${Y(E.vueltoUsd)} equiv. oficial</span>
                        </div>
                        ...
                        <div class="bg-white border border-emerald-600 rounded p-2.5 space-y-2">
                            ...
                            <div class="flex items-center justify-between pt-1 border-t border-gray-100 text-xs">
                                ...
                                <span class="font-heading font-black text-base text-brand-black">Bs. ${Y(E.vueltoBs)}</span>
                            </div>
                            `}
                        </div>
                        `:`
                        <div class="bg-white border border-gray-300 rounded p-2 text-xs text-gray-700">
                            ...
                            <p class="text-[11px] text-gray-600 leading-tight">
                                El excedente de <b>Bs. ${Y(E.vueltoBs)} ($ ${Y(E.vueltoUsd)})</b> no genera egreso físico de caja y se asienta en el comprobante como saldo a favor o ganancia por excedente del negocio.
                            </p>
                        </div>
                        `}
                ...
                <div>
                    ...
                    <div class="flex flex-wrap items-center justify-between gap-2 pt-2 border-t-2 border-brand-black">
                        ...
                        <button id="btn-confirmar-cobro" ${E.puedeConfirmar?"":"disabled"}
                ...
                </div>`,"max-w-xl"),p()},p=()=>{var _,f,w,y,P,T,C,G,q,z;const E=()=>{this.pagosBorrador=i,this.resolucionVueltoBorrador={estado:b,metodo:t,tasa:r},this.cerrarModal()};this.modal.querySelectorAll("[data-cerrar]").forEach(B=>B.addEventListener("click",E)),(_=this.modal.querySelector("[data-cancelar]"))==null||_.addEventListener("click",E),(f=this.modal.querySelector("#btn-volver-caja"))==null||f.addEventListener("click",()=>{this.pagosBorrador=i,this.resolucionVueltoBorrador={estado:b,metodo:t,tasa:r},this.cerrarModal();const B=document.getElementById("caja-buscar");B==null||B.focus()}),(w=this.modal.querySelector("#btn-vuelto-pagado"))==null||w.addEventListener("click",()=>{b="PAGADO",this.resolucionVueltoBorrador={estado:b,metodo:t,tasa:r},l()}),(y=this.modal.querySelector("#btn-vuelto-retenido"))==null||y.addEventListener("click",()=>{b="RETENIDO",this.resolucionVueltoBorrador={estado:b,metodo:t,tasa:r},l()}),(P=this.modal.querySelector("#select-metodo-vuelto"))==null||P.addEventListener("change",B=>{t=B.target.value,this.resolucionVueltoBorrador={estado:b,metodo:t,tasa:r},l()});const I=this.modal.querySelector("#input-tasa-vuelto");I&&(I.addEventListener("input",B=>{const N=Z(B.target.value);r=N>0?N:a,this.resolucionVueltoBorrador={estado:b,metodo:t,tasa:r}}),I.addEventListener("blur",()=>{l()})),(T=this.modal.querySelector("#btn-reset-tasa-vuelto"))==null||T.addEventListener("click",()=>{r=a,this.resolucionVueltoBorrador={estado:b,metodo:t,tasa:r},l()}),this.modal.querySelectorAll("select[data-pago-metodo]").forEach(B=>{B.addEventListener("change",N=>{const S=B.dataset.pagoMetodo,j=N.target.value,k=i.find(M=>M.id===S);if(k){const M=e.find(te=>te.nombre===j),J=(M==null?void 0:M.moneda)||"BS";k.moneda=J,k.tasaCambio=a,i.length===1?k.monto=k.moneda==="USD"?o:n:J!==k.moneda&&(J==="USD"?k.monto=Number((a>0?k.monto/a:0).toFixed(2)):k.monto=Number((k.monto*a).toFixed(2))),k.metodo=j,this.pagosBorrador=i,l()}})}),this.modal.querySelectorAll("input[data-pago-tasa]").forEach(B=>{B.addEventListener("input",N=>{const S=B.dataset.pagoTasa,j=Z(N.target.value),k=i.find(M=>M.id===S);if(k){k.tasaCambio=j>0?j:a,this.pagosBorrador=i;const M=d(),J=this.modal.querySelector("#btn-confirmar-cobro");J&&(J.disabled=!M.puedeConfirmar)}}),B.addEventListener("blur",()=>{l()})}),this.modal.querySelectorAll("button[data-reset-tasa]").forEach(B=>{B.addEventListener("click",()=>{const N=B.dataset.resetTasa,S=i.find(j=>j.id===N);S&&(S.tasaCambio=a,this.pagosBorrador=i,l())})}),this.modal.querySelectorAll("input[data-pago-monto]").forEach(B=>{B.addEventListener("input",N=>{const S=B.dataset.pagoMonto,j=Z(N.target.value),k=i.find(M=>M.id===S);if(k){k.monto=j,this.pagosBorrador=i;const M=d(),J=this.modal.querySelector("#btn-confirmar-cobro");J&&(J.disabled=!M.puedeConfirmar)}}),B.addEventListener("blur",()=>{l()})}),this.modal.querySelectorAll("input[data-pago-ref]").forEach(B=>{B.addEventListener("input",N=>{const S=B.dataset.pagoRef,j=N.target.value,k=i.find(M=>M.id===S);k&&(k.referencia=j,this.pagosBorrador=i)})}),this.modal.querySelectorAll("button[data-eliminar-pago]").forEach(B=>{B.addEventListener("click",()=>{const N=B.dataset.eliminarPago;i=i.filter(S=>S.id!==N),this.pagosBorrador=i,l()})}),(C=this.modal.querySelector("#btn-agregar-pago"))==null||C.addEventListener("click",()=>{let B=d();const N="p-"+Math.random().toString(36).slice(2,7),S=e.find(k=>!i.some(M=>M.metodo===k.nombre))||e[0]||{nombre:"BS.EFEC.",moneda:"BS"};if(B.faltanteBs<=.01&&i.length===1&&i[0]){const k=i[0],M=k.tasaCambio>0?k.tasaCambio:a,J=k.moneda==="USD"?k.monto*M:k.monto,te=Math.round(J/2*100)/100;k.monto=k.moneda==="USD"?M>0?Number((te/M).toFixed(2)):0:te,B=d()}const j=B.faltanteBs>0?B.faltanteBs:0;i.push({id:N,metodo:S.nombre,moneda:S.moneda,monto:S.moneda==="USD"?a>0?Number((j/a).toFixed(2)):0:j,tasaCambio:a,referencia:""}),this.pagosBorrador=i,l()}),(G=this.modal.querySelector("#btn-toggle-nuevo-metodo"))==null||G.addEventListener("click",()=>{s=!s,l()}),(q=this.modal.querySelector("#btn-guardar-nuevo-metodo"))==null||q.addEventListener("click",()=>{(async()=>{const B=this.modal.querySelector("#input-nuevo-metodo-nombre"),N=(B==null?void 0:B.value.trim())||"",S=this.modal.querySelector('input[name="radio-nuevo-metodo-moneda"][value="USD"]'),j=S!=null&&S.checked?"USD":"BS";if(N)try{e=await D.crearMetodoPago(N,j),s=!1,l()}catch{}})()}),this.modal.querySelectorAll("button[data-caja-eliminar-metodo]").forEach(B=>{B.addEventListener("click",()=>{(async()=>{const N=B.dataset.cajaEliminarMetodo||"";N&&window.confirm(`¿Seguro que deseas eliminar el método de pago "${N}"?`)&&(e=await D.eliminarMetodoPago(N),i.forEach(S=>{if(S.metodo===N){const j=e[0]||{nombre:"PUNTOD.VENTA",moneda:"BS"};S.metodo=j.nombre,S.moneda=j.moneda}}),this.pagosBorrador=i,l())})()})});let L=!1;(z=this.modal.querySelector("#btn-confirmar-cobro"))==null||z.addEventListener("click",()=>{L||(L=!0,(async()=>{const B=this.modal.querySelector("#cobro-error");try{const N=i.filter($=>$.monto>0).map($=>{const V=$.moneda==="USD"&&$.tasaCambio>0?$.tasaCambio:a,O=$.moneda==="USD"?$.monto*V:$.monto,Q=a>0?O/a:0;return{metodo:$.metodo,moneda:$.moneda,montoUsd:($.moneda==="USD"?$.monto:Q).toFixed(2),montoBs:O.toFixed(2),tasaCambio:$.moneda==="USD"?V.toFixed(2):void 0,referencia:$.referencia.trim()||void 0}});if(N.length===0)throw new Error("Debe especificar al menos un pago válido.");const S=d(),j=N.reduce(($,V)=>$+Number(V.montoBs),0),k=e.find($=>$.nombre===t),M=(k==null?void 0:k.moneda)==="USD",J=M?r>0?r:a:void 0,te=M&&J?(S.vueltoBs/J).toFixed(2):S.vueltoUsd.toFixed(2),H={estado:S.vueltoBs>.009?b:"SIN_VUELTO",metodo:S.vueltoBs>.009&&b==="PAGADO"?t:void 0,montoBs:S.vueltoBs>.009?S.vueltoBs.toFixed(2):void 0,montoUsd:S.vueltoBs>.009?te:void 0,tasa:S.vueltoBs>.009&&b==="PAGADO"&&J?J.toFixed(2):void 0},U=await this.vm.cobrar(j.toFixed(2),N,H);this.pagosBorrador=null,this.resolucionVueltoBorrador=null,this.cerrarModal(),this.modalTicketExito(U,j)}catch(N){L=!1,B&&(B.textContent=N instanceof Error?N.message.replace(/"/g,""):String(N),B.classList.remove("hidden"))}})())})};l()}modalTicketExito(e,o){const a=e.pagos&&e.pagos.length>0?e.pagos.map(c=>`
            ...
            </tr>`}).join("")}async render(){var _;const[e,o,a,n,c,i]=await Promise.all([D.panel(),D.respaldos(),D.ventas(),D.productos(),D.categorias(),D.obtenerJornadaActual()]);this.vm.setDatos(e),this.respaldos=o,this.transacciones=a,this.productos=n,this.categorias=c,this.jornadaActual=i,this.calcularMargenes();const b=new Date,t=new Date(b.getTime()-864e5),r=f=>f.toLocaleString("es-VE",{year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit"});this.rangoDetalleFechas||(this.rangoDetalleFechas=`Del ${r(t)} al ${r(b)}`);const s=this.modelo.bs(e.valorInventarioUsd),d=this.modelo.tieneCapacidad(8)||this.modelo.tieneRubro(4),l=e.dineroPorCategoria&&e.dineroPorCategoria.length>0?e.dineroPorCategoria:this.calcularDineroPorCategoria(),p=l.reduce((f,w)=>f+w.cantidadProductos,0),x=l.reduce((f,w)=>f+w.unidadesStock,0),h=l.reduce((f,w)=>f+ee(w.dineroBrutoUsd),0),g=l.reduce((f,w)=>f+ee(w.dineroBrutoBs),0),v=l.reduce((f,w)=>f+ee(w.dineroVentaUsd),0),E=Math.max(0,v-h),I=v>0?(E/v*100).toFixed(1):"0.0",L=l.map(f=>{const w=ee(f.porcentajeCapital);return`
            <tr class="hover:bg-gray-50 transition-colors">
                ...
                <td class="py-2.5 px-3 text-right font-black font-mono text-brand-black">$ ${W(f.dineroBrutoUsd)}</td>
                <td class="py-2.5 px-3 text-right font-black font-mono text-brand-purple">Bs. ${W(f.dineroBrutoBs)}</td>
                <td class="py-2.5 px-3 text-right font-mono text-gray-700">$ ${W(f.dineroVentaUsd)}</td>
                <td class="py-2.5 px-3 text-right font-mono text-emerald-700">$ ${W(f.margenBrutoProyectadoUsd)} (${f.margenBrutoPct}%)</td>
                ...
                <table class="w-full text-left text-xs">
                    ...
                    <tfoot class="border-t-2 border-brand-black bg-gray-100 font-black font-heading text-xs">
                        <tr>
                            ...
                            <td class="py-2.5 px-3 text-right text-brand-black">$ ${W(v)}</td>
                            <td class="py-2.5 px-3 text-right text-emerald-700">$ ${W(E)} (${I}%)</td>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
            ...
            <div class="bg-white border-2 border-brand-black rounded-lg shadow-brutal p-4 sm:p-6 flex flex-col justify-between">
                ...
                <div class="mt-4 pt-3 border-t border-brand-black">
        ...
        </div>`,this.conectarEventos(e),this.pintarGrafico(e)}conectarEventos(e){var p,x,h,g,v,E,I,L,_;const o=document.getElementById("btn-crit-vol"),a=document.getElementById("btn-crit-ing"),n=document.getElementById("filtro-fecha-desde"),c=document.getElementById("filtro-fecha-hasta"),i=document.getElementById("btn-aplicar-fechas"),b=document.getElementById("btn-limpiar-fechas"),t=document.getElementById("label-estado-fechas"),r=document.getElementById("tabla-transacciones-body"),s=document.getElementById("conteo-transacciones-badge"),d=this.contenedor.querySelectorAll("[data-rango]");d.forEach(f=>{f.addEventListener("click",()=>{n&&(n.value=""),c&&(c.value=""),t&&t.classList.add("hidden"),r&&(r.innerHTML=this.renderFilasTransacciones(this.transacciones)),s&&(s.textContent=`${this.transacciones.length} transacciones registradas`),d.forEach(y=>{y.className="w-12 sm:w-14 h-8 flex items-center justify-center font-heading font-bold text-xs rounded text-brand-black hover:bg-gray-100 transition-colors shrink-0"}),f.className="w-12 sm:w-14 h-8 flex items-center justify-center font-heading font-black text-xs rounded bg-brand-black text-white shrink-0";const w=f.dataset.rango||"24h";this.aplicarRango(w,e)})}),i==null||i.addEventListener("click",()=>{this.aplicarFiltroFechas(e)}),b==null||b.addEventListener("click",()=>{var w,y;n&&(n.value=""),c&&(c.value=""),t&&t.classList.add("hidden"),r&&(r.innerHTML=this.renderFilasTransacciones(this.transacciones)),s&&(s.textContent=`${this.transacciones.length} transacciones registradas`),this.paginaTrx=1,(w=document.getElementById("trx-pag-ant"))==null||w.addEventListener("click",()=>{this.paginaTrx>1&&(this.paginaTrx--,r&&(r.innerHTML=this.renderFilasTransacciones(this.transacciones)))}),(y=document.getElementById("trx-pag-sig"))==null||y.addEventListener("click",()=>{const P=Math.ceil(this.transacciones.length/this.POR_PAGINA_TRX);this.paginaTrx<P&&(this.paginaTrx++,r&&(r.innerHTML=this.renderFilasTransacciones(this.transacciones)))}),d.forEach(P=>{P.className="w-12 sm:w-14 h-8 flex items-center justify-center font-heading font-bold text-xs rounded text-brand-black hover:bg-gray-100 transition-colors shrink-0"});const f=this.contenedor.querySelector('[data-rango="24h"]');f&&(f.className="w-12 sm:w-14 h-8 flex items-center justify-center font-heading font-black text-xs rounded bg-brand-black text-white shrink-0"),this.aplicarRango("24h",e)}),o==null||o.addEventListener("click",()=>{this.criterioGrafica="volumen",o.className="px-2.5 py-1 text-xs font-heading font-black rounded bg-brand-black text-white",a.className="px-2.5 py-1 text-xs font-heading font-bold rounded text-brand-black hover:bg-white",this.pintarGrafico(this.vm.panel||e)}),a==null||a.addEventListener("click",()=>{this.criterioGrafica="ingreso",a.className="px-2.5 py-1 text-xs font-heading font-black rounded bg-brand-black text-white",o.className="px-2.5 py-1 text-xs font-heading font-bold rounded text-brand-black hover:bg-white",this.pintarGrafico(this.vm.panel||e)}),(p=document.getElementById("trx-pag-ant"))==null||p.addEventListener("click",()=>{this.paginaTrx>1&&(this.paginaTrx--,r&&(r.innerHTML=this.renderFilasTransacciones(this.transacciones)))}),(x=document.getElementById("trx-pag-sig"))==null||x.addEventListener("click",()=>{const f=Math.ceil(this.transacciones.length/this.POR_PAGINA_TRX);this.paginaTrx<f&&(this.paginaTrx++,r&&(r.innerHTML=this.renderFilasTransacciones(this.transacciones)))}),(h=document.getElementById("btn-crear-backup"))==null||h.addEventListener("click",async()=>{await D.crearRespaldo(),await this.render()});const l=async()=>{var J,te,H;const f=await D.obtenerJornadaActual(),w=await D.listarOperadores(),y=await D.obtenerSemaforoStock(),P=document.getElementById("jornada-estado-texto"),T=document.getElementById("jornada-fechas-texto"),C=document.getElementById("jornada-operador-activo"),G=document.getElementById("jornada-balance-texto"),q=document.getElementById("jornada-tickets-texto"),z=document.getElementById("box-acciones-jornada"),B=document.getElementById("lista-operadores-chips"),N=(f==null?void 0:f.estado)==="abierta";P&&(P.textContent=N?"Jornada en Curso":"Jornada Cerrada"),T&&(T.textContent=N?`Inicio: ${(f==null?void 0:f.inicioStr)||"Reciente"}`:`Último cierre: ${(f==null?void 0:f.finStr)||"Sin jornadas"}`);const S=f!=null&&f.operadoresActivos&&f.operadoresActivos.length>0?f.operadoresActivos.join(", "):((J=f==null?void 0:f.operadorActual)==null?void 0:J.trim())||"Sin operador asignado";C&&(C.textContent=S),G&&(G.textContent=`$ ${W((f==null?void 0:f.ventasTotalUsd)||0)} USD (Bs. ${W((f==null?void 0:f.ventasTotalBs)||0)})`),q&&(q.textContent=`${(f==null?void 0:f.ticketsEmitidos)||0} tickets emitidos · ${(f==null?void 0:f.entradasStockReg)||0} reposiciones`),z&&(N?z.innerHTML=`
                    ...
                    `,(te=z.querySelector("#btn-abrir-jornada"))==null||te.addEventListener("click",async()=>{var U;try{const $=(await D.listarOperadores()).filter(O=>O.activo);if($.length===0){this.mostrarToast("No hay operadores activos. Registre uno primero.","error");return}const V=((U=$[0])==null?void 0:U.nombre)||"Cajero Principal";await D.abrirJornada(V,$.map(O=>O.nombre)),this.mostrarToast("Jornada abierta exitosamente.","success"),l()}catch($){this.mostrarToast("Error al abrir jornada: "+($ instanceof Error?$.message:String($)),"error")}}),(H=z.querySelector("#btn-cerrar-jornada"))==null||H.addEventListener("click",async()=>{try{if(window.confirm("¿Confirmas el cierre de la jornada operativa actual? Se generará el balance consolidado del turno.")){const $=await D.cerrarJornada();this.mostrarToast(`Jornada cerrada. ID: ${$.id} — $${$.ventasTotalUsd} USD (Bs. ${$.ventasTotalBs})`,"success"),l()}}catch(U){this.mostrarToast("Error al cerrar jornada: "+(U instanceof Error?U.message:String(U)),"error")}})),B&&(B.innerHTML=w.map(U=>{var V,O;return`
                    <div class="inline-flex items-center gap-1.5 border-2 border-brand-black rounded px-2 py-1 text-xs font-bold ${((V=f==null?void 0:f.operadoresActivos)==null?void 0:V.includes(U.nombre))||((O=f==null?void 0:f.operadorActual)==null?void 0:O.includes(U.nombre))?"bg-purple-100 text-brand-purple border-brand-purple":"bg-gray-100 text-brand-black"}">
                        <span>${U.nombre}</span>
                ...
                `}).join(""),B.querySelectorAll("button[data-op-edit]").forEach(U=>{U.addEventListener("click",async()=>{const $=U.dataset.opEdit||"",V=U.dataset.opNom||"",O=prompt("Editar nombre del operador:",V);O&&O.trim()&&O.trim()!==V&&(await D.editarOperador($,O.trim()),l())})}),B.querySelectorAll("button[data-op-del]").forEach(U=>{U.addEventListener("click",async()=>{const $=U.dataset.opDel||"",V=U.dataset.opNom||"";confirm(`¿Eliminar al operador "${V}"?`)&&(await D.eliminarOperador($),l())})}));const j=document.getElementById("lista-metodos-pago-panel");if(j){const U=await D.listarMetodosPago();j.innerHTML=U.map($=>`
                ...
                `).join(""),j.querySelectorAll("button[data-metodo-del]").forEach($=>{$.addEventListener("click",async()=>{const V=$.dataset.metodoDel||"";window.confirm(`¿Confirmas eliminar el método de pago "${V}"?`)&&(await D.eliminarMetodoPago(V),l())})})}const k=document.getElementById("in-semaforo-rojo"),M=document.getElementById("in-semaforo-amarillo");k&&!k.dataset.modificado&&(k.value=String(y.rojoMax)),M&&!M.dataset.modificado&&(M.value=String(y.amarilloMax))};l(),(g=document.getElementById("btn-relevar-operador"))==null||g.addEventListener("click",async()=>{var G,q,z;const f=(await D.listarOperadores()).filter(B=>B.activo);if(f.length===0){this.mostrarToast("No hay operadores activos disponibles. Registre uno a continuación.","error");return}const w=await D.obtenerJornadaActual(),y=(w==null?void 0:w.operadoresActivos)||[(w==null?void 0:w.operadorActual)||""],P=document.getElementById("modal-root")||document.body,T=document.createElement("div");T.id="modal-asignar-operadores",T.innerHTML=`
                <div class="fixed inset-0 bg-black/40 flex items-center justify-center z-[100] p-4">
                    <div class="bg-white border-2 border-brand-black rounded-lg shadow-brutal p-6 w-full max-w-md max-h-[90vh] overflow-y-auto">
                        ...
                        <div class="space-y-2 max-h-56 overflow-y-auto pr-1 mb-4">
                            ${f.map(B=>{const N=y.some(S=>S.toLowerCase()===B.nombre.toLowerCase());return`
                ...
                </div>`,P.appendChild(T);const C=()=>T.remove();(G=T.querySelector("#modal-asig-close"))==null||G.addEventListener("click",C),(q=T.querySelector("#modal-asig-cancelar"))==null||q.addEventListener("click",C),(z=T.querySelector("#modal-asig-guardar"))==null||z.addEventListener("click",async()=>{const B=T.querySelectorAll("input[data-asig-chk]:checked"),N=[];if(B.forEach(S=>{S.dataset.asigChk&&N.push(S.dataset.asigChk)}),N.length===0){this.mostrarToast("Selecciona al menos un operador para el turno.","error");return}await D.asignarOperadoresTurno(N),C(),l()})}),(v=document.getElementById("form-crear-metodo-panel"))==null||v.addEventListener("submit",async f=>{f.preventDefault();const w=document.getElementById("in-nuevo-metodo-nombre"),y=document.getElementById("select-nuevo-metodo-moneda"),P=(w==null?void 0:w.value.trim())||"",T=(y==null?void 0:y.value)==="USD"?"USD":"BS";P&&(await D.crearMetodoPago(P,T),w&&(w.value=""),l())}),(E=document.getElementById("form-crear-operador"))==null||E.addEventListener("submit",async f=>{f.preventDefault();const w=document.getElementById("in-nuevo-operador"),y=(w==null?void 0:w.value.trim())||"";y&&(await D.crearOperador(y),w&&(w.value=""),l())}),(I=document.getElementById("form-semaforo-stock"))==null||I.addEventListener("submit",async f=>{f.preventDefault();const w=document.getElementById("in-semaforo-rojo"),y=document.getElementById("in-semaforo-amarillo"),P=document.getElementById("msg-semaforo-stock"),T=parseInt((w==null?void 0:w.value)||"5",10),C=parseInt((y==null?void 0:y.value)||"15",10);if(T>=C){this.mostrarToast("El umbral rojo debe ser estrictamente menor que el umbral amarillo.","error");return}await D.guardarSemaforoStock(T,C),P&&(P.textContent="Umbrales del semáforo guardados y aplicados a todo el sistema.",P.classList.remove("hidden"),setTimeout(()=>P.classList.add("hidden"),2500))}),(L=document.getElementById("btn-reset-semaforo"))==null||L.addEventListener("click",async()=>{await D.guardarSemaforoStock(5,15),l()}),(_=document.getElementById("form-cambiar-clave"))==null||_.addEventListener("submit",async f=>{f.preventDefault();const w=document.getElementById("pin-actual"),y=document.getElementById("pin-nuevo"),P=document.getElementById("msg-cambio-clave");if(P)try{await D.cambiarPinDueno((w==null?void 0:w.value.trim())||"",(y==null?void 0:y.value.trim())||""),await this.modelo.cargarConfig(),P.textContent=y!=null&&y.value.trim()?"Clave actualizada exitosamente. El panel requerirá esta nueva clave para ingresar.":"Clave eliminada. El panel ha quedado en modo abierto sin clave.",P.style.color="#00823B",P.classList.remove("hidden"),setTimeout(()=>void this.render(),1200)}catch(T){P.textContent=T instanceof Error?T.message:String(T),P.style.color="#C60C15",P.classList.remove("hidden")}})}aplicarFiltroFechas(e){const o=document.getElementById("filtro-fecha-desde"),a=document.getElementById("filtro-fecha-hasta"),n=document.getElementById("label-estado-fechas"),c=document.getElementById("tabla-transacciones-body"),i=document.getElementById("conteo-transacciones-badge"),b=(o==null?void 0:o.value.trim())||"",t=(a==null?void 0:a.value.trim())||"";if(!b&&!t){n&&(n.textContent="Selecciona una fecha Desde o Hasta para iniciar el diagnóstico.",n.classList.remove("hidden"));return}let r=0,s=Number.MAX_SAFE_INTEGER;if(b){const[k,M,J]=b.split("-").map(Number);r=Math.floor(new Date(k,M-1,J,0,0,0,0).getTime()/1e3)}if(t){const[k,M,J]=t.split("-").map(Number);s=Math.floor(new Date(k,M-1,J,23,59,59,999).getTime()/1e3)}if(r>s){n&&(n.textContent='Rango inválido: la fecha "Desde" es posterior a la fecha "Hasta".',n.classList.remove("hidden"));return}const d=this.transacciones.filter(k=>{const M=this.obtenerTimestampTicket(k);return M>=r&&M<=s}),l=d.reduce((k,M)=>k+ee(M.totalUsd),0),p=d.reduce((k,M)=>k+ee(M.totalBs),0),x=d.length;let h=0;d.forEach(k=>{(k.lineas||[]).forEach(M=>{const J=this.productos.find(te=>te.sku===M.sku);J&&(h+=ee(J.precioBrutoUsd)*ee(M.cantidad))})});const g=Math.max(0,l-h),v=g*.95,E=v*(this.modelo.tasaActual||807.39),I=x>0?(l/x).toFixed(2):"0.00",L=document.getElementById("kpi-ventas-usd"),_=document.getElementById("kpi-ventas-bs"),f=document.getElementById("kpi-tickets"),w=document.getElementById("kpi-gan-bruta"),y=document.getElementById("kpi-gan-neta"),P=document.getElementById("kpi-gan-neta-bs"),T=document.getElementById("kpi-ticket-prom");L&&(L.textContent="$ "+W(l)),_&&(_.textContent="Bs. "+se(p),_.title="Bs. "+W(p)),f&&(f.textContent=String(x)),w&&(w.textContent="$ "+W(g)),y&&(y.textContent="$ "+W(v)),P&&(P.textContent="Bs. "+se(E),P.title="Bs. "+W(E)),T&&(T.textContent="$ "+I),c&&(c.innerHTML=this.renderFilasTransacciones(d)),i&&(i.textContent=`${d.length} transacciones en rango`),this.contenedor.querySelectorAll("[data-rango]").forEach(k=>{k.className="w-12 sm:w-14 h-8 flex items-center justify-center font-heading font-bold text-xs rounded text-brand-black hover:bg-gray-100 transition-colors shrink-0"});const G={};d.forEach(k=>{(k.lineas||[]).forEach(M=>{G[M.nombre]||(G[M.nombre]={nombre:M.nombre,cantidad:0,totalUsd:0}),G[M.nombre].cantidad+=Number(M.cantidad)||0,G[M.nombre].totalUsd+=ee(M.subtotalUsd)||Number(M.cantidad)*ee(M.precioUsd)})});const q=Object.values(G).reduce((k,M)=>k+M.totalUsd,0)||1,z=Object.values(G).sort((k,M)=>M.totalUsd-k.totalUsd).map(k=>({nombre:k.nombre,cantidad:String(k.cantidad),totalUsd:k.totalUsd.toFixed(2),porcentaje:(k.totalUsd/q*100).toFixed(1)})),B={...e,ventas24hUsd:l.toFixed(2),ventas24hBs:p.toFixed(2),gananciaBrutaUsd:g.toFixed(2),gananciaNetaUsd:v.toFixed(2),tickets24h:x,topProductos:z.length>0?z:d.length===0?[]:e.topProductos};this.vm.setDatos(B),this.pintarGrafico(B);const N=k=>k.toLocaleString("es-VE",{year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit"}),S=b?new Date(r*1e3):new Date(2020,0,1),j=t?new Date(s*1e3):new Date;if(this.rangoDetalleFechas=`Del ${N(S)} al ${N(j)}`,n){const k=b||"Inicio",M=t||"Actualidad";n.textContent=`Diagnóstico activo (${k} al ${M}): ${d.length} transacciones registradas`,n.classList.remove("hidden")}}aplicarRango(e,o){const a=new Date,n=w=>w.toLocaleString("es-VE",{year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit"});let c=864e5;e==="7d"?c=7*864e5:e==="30d"?c=30*864e5:e==="1a"?c=365*864e5:e==="todo"&&(c=5*365*864e5);const i=new Date(a.getTime()-c);this.rangoDetalleFechas=`Del ${n(i)} al ${n(a)}`;const b=Math.floor(i.getTime()/1e3),t=this.transacciones.filter(w=>this.obtenerTimestampTicket(w)>=b),r=this.modelo.tasaActual||807.39;let s=0,d=0;t.forEach(w=>{s+=ee(w.totalUsd),d+=ee(w.totalBs)});const l=t.length,p=document.getElementById("kpi-ventas-usd"),x=document.getElementById("kpi-ventas-bs"),h=document.getElementById("kpi-tickets"),g=document.getElementById("kpi-gan-bruta"),v=document.getElementById("kpi-gan-neta"),E=document.getElementById("kpi-gan-neta-bs"),I=document.getElementById("kpi-ticket-prom");if(p&&(p.textContent="$ "+W(s)),x&&(x.textContent="Bs. "+se(d),x.title="Bs. "+W(d)),h&&(h.textContent=String(l)),g&&(g.textContent="$ "+W(s*this.margenBrutoPct/100)),v&&(v.textContent="$ "+W(s*this.margenNetoPct/100)),E){const w=s*this.margenNetoPct/100*r;E.textContent="Bs. "+se(w),E.title="Bs. "+W(w)}I&&(I.textContent="$ "+(l>0?(s/l).toFixed(2):"0.00"));const L=new Map;t.forEach(w=>{var y;(y=w.lineas)==null||y.forEach(P=>{const T=L.get(P.sku);T?(T.cantidad+=ee(P.cantidad),T.totalUsd+=ee(P.subtotalUsd)):L.set(P.sku,{nombre:P.nombre,cantidad:ee(P.cantidad),totalUsd:ee(P.subtotalUsd)})})});const _=Array.from(L.values()).sort((w,y)=>y.totalUsd-w.totalUsd).slice(0,10).map(w=>({nombre:w.nombre,cantidad:String(Math.round(w.cantidad)),totalUsd:w.totalUsd>0?String(w.totalUsd.toFixed(2)):void 0,porcentaje:s>0?(w.totalUsd/s*100).toFixed(1):"0.0"})),f={...o,topProductos:_};this.vm.setDatos(f),this.pintarGrafico(f)}kpi(e,o,a,n,c=""){return`
        <div class="border-2 border-brand-black rounded-lg shadow-brutal-sm p-4 ${n} flex flex-col justify-between min-w-0 overflow-hidden" title="${c}">
            ...
            ${c?`<div class="text-[10px] text-gray-500 font-bold truncate mt-0.5 border-t border-gray-200/60 pt-1">${c}</div>`:""}
                ...
                `,t+=E});const p=b.map((h,g)=>{const v=a[g%a.length],E=n?`${h.valor} un.`:`$ ${W(h.valor)}`;return`
                <tr class="border-b border-gray-100 last:border-none">
                    <td class="py-2 pr-3 flex items-center gap-2">
                        <span class="w-3 h-3 rounded-full inline-block border border-brand-black shrink-0" style="background-color: ${v}"></span>
                        <span class="font-bold text-xs truncate max-w-[180px]" title="${h.nombre}">${h.nombre}</span>
                    </td>
                    <td class="py-2 text-right font-black text-xs pr-2">${E}</td>
                    <td class="py-2 text-right font-mono text-xs font-bold text-gray-500">${h.porcentaje}%</td>
                </tr>
            `}).join(""),x=n?`${Math.round(i)} un.`:`$ ${W(i)}`;o.innerHTML=`
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6 items-start">
            ...
            <div class="space-y-3">
                ...
                <div id="cuentas-lista" class="space-y-2 max-h-[65vh] overflow-y-auto pr-1">
                    ${n.length===0?`<div class="bg-white border-2 border-brand-black rounded-lg p-6 text-center text-brand-text font-bold">
                        ...
                        </div>`:(()=>{const s=Math.ceil(n.length/this.POR_PAGINA_CUENTAS);this.paginaCuentas>s&&(this.paginaCuentas=s),this.paginaCuentas<1&&(this.paginaCuentas=1);const d=(this.paginaCuentas-1)*this.POR_PAGINA_CUENTAS;return n.slice(d,d+this.POR_PAGINA_CUENTAS).map(p=>{var f;const x=p.tipo==="deuda",h=X(p.totalParcialUsd),g=X(p.abonosUsd),v=Math.max(0,h-g),E=Math.max(0,g-h),I=((f=this.cuentaSeleccionada)==null?void 0:f.ventaId)===p.ventaId,L=Se(p.fechaCreacionUnix);let _="cursor-pointer border-2 border-brand-black rounded-lg p-3 sm:p-4 transition-all ";return I?_+=x?"bg-amber-50 text-brand-black border-l-[6px] border-l-amber-600 shadow-brutal ring-1 ring-amber-500/20":"bg-blue-50 text-brand-black border-l-[6px] border-l-blue-600 shadow-brutal ring-1 ring-blue-500/20":_+=x?"bg-stone-50 border-l-4 border-l-amber-300 text-brand-black shadow-brutal-sm hover:bg-amber-50/60 hover:-translate-y-0.5":"bg-stone-50 border-l-4 border-l-blue-200 text-brand-black shadow-brutal-sm hover:bg-blue-50/50 hover:-translate-y-0.5",`
                        <div data-cuenta-id="${p.ventaId}" class="${_}">
                            <div class="flex justify-between items-start gap-2">
                                ...
                                <div class="text-right shrink-0">
                                    ...
                                    ${E>0?`<p class="text-[11px] font-black text-emerald-700">A favor: +$${R(E)}</p>`:v>0?`<p class="text-xs font-bold ${x?"text-amber-900":"text-blue-900"}">Pend: $${R(v)}</p>`:'<p class="text-[11px] font-bold text-gray-400">Cubierta ($0,00)</p>'}
                                </div>
                            </div>
            ...
            <div class="lg:col-span-2">
                ${this.cuentaSeleccionada?(()=>{var g;const s=this.cuentaSeleccionada.tipo==="deuda",d=X(this.cuentaSeleccionada.totalParcialUsd),l=X(this.cuentaSeleccionada.abonosUsd),p=Math.max(0,d-l),x=Math.max(0,l-d),h=Se(this.cuentaSeleccionada.fechaCreacionUnix);return`
                <div class="bg-white border-2 border-brand-black rounded-lg shadow-brutal p-4 sm:p-6 flex flex-col space-y-4">
                    ...
                    <div class="border-2 border-brand-black rounded-lg p-3 sm:p-4 bg-gray-50 shadow-sm flex flex-col">
                        ...
                        <div id="cta-consumos-lista" class="space-y-2 overflow-y-auto max-h-[300px] pr-1">
                            ${!this.cuentaSeleccionada.consumos||this.cuentaSeleccionada.consumos.length===0?`<div class="py-6 text-center text-xs text-gray-400 font-bold">
                                ...
                                </div>`:this.cuentaSeleccionada.consumos.map(v=>`
                                <div class="flex items-center justify-between border-2 border-brand-black rounded-lg bg-white px-3 py-2 text-xs font-bold shadow-brutal-sm hover:bg-amber-50/50 transition-colors gap-2">
                                    <div class="flex-1 min-w-0 pr-2">
                                        <p class="truncate text-brand-black font-heading font-black text-sm" title="${v.nombre}">${v.nombre}</p>
                                        <p class="text-[11px] text-gray-500 font-semibold">${v.cantidad} un. &times; $${R(v.precioUsd)} <span class="text-gray-400 font-normal">· Bs. ${this.modelo.bs(X(v.precioUsd))} c/u</span></p>
                                    </div>
                                    <div class="flex items-center gap-3 shrink-0">
                                        <div class="text-right">
                                            <p class="font-heading font-black text-sm sm:text-base text-brand-black leading-tight">$${R(v.subtotalUsd)}</p>
                                            <p class="text-[10px] font-bold text-gray-500 leading-tight">Bs. ${this.modelo.bs(X(v.subtotalUsd))}</p>
                                        </div>
                                        <button data-del-consumo="${v.id}" title="Eliminar consumo y reintegrar stock al inventario" class="w-7 h-7 flex items-center justify-center rounded border-2 border-brand-black bg-red-100 hover:bg-red-500 hover:text-white text-red-800 text-sm font-black transition-colors active:scale-95">&times;</button>
                                    </div>
                                </div>
                            `).join("")}
                        </div>
                    ...
                    <div>
                        ...
                        <div id="cta-grid-prod" class="grid grid-cols-2 sm:grid-cols-3 gap-2 max-h-48 overflow-y-auto p-1">
                            ${this.productos.map(v=>{const E=!v.sinStock&&Number(v.stock)<=0,I=v.precioPaqueteUsd&&v.nombrePaquete||v.esCaja&&v.unidadesPorCaja&&v.unidadesPorCaja>1;return`
                                <div class="relative" data-producto-row="${v.sku}">
                                    <button data-add-sku="${v.sku}" data-modo="unidad" data-agotado="${E?"1":"0"}" class="text-left border-2 border-brand-black rounded p-2 transition-all text-xs font-bold ${E?"bg-gray-100 opacity-60 cursor-not-allowed":"bg-gray-50 hover:bg-white shadow-sm hover:shadow-brutal-sm"} w-full">
                                        <div class="flex justify-between items-start gap-1">
                                            <p class="truncate flex-1" title="${v.nombre}">${v.nombre}</p>
                                            ${E?'<span class="bg-red-200 text-red-900 border border-red-400 text-[9px] font-black px-1 rounded">AGOTADO</span>':""}
                                        </div>
                                        <div class="flex justify-between items-center mt-1">
                                            <p class="text-brand-black font-black">$${R(v.precioUsd)}</p>
                                            <span class="text-[10px] ${E?"text-red-700 font-black":"text-gray-500"}">${we(v)}</span>
                                        </div>
                                    </button>
                                    ${I?`
                                    <button data-add-sku="${v.sku}" data-modo="paquete" data-agotado="${E?"1":"0"}" class="text-left border-2 border-brand-purple rounded p-1 transition-all text-[10px] font-bold mt-1 w-full ${E?"bg-gray-100 opacity-60 cursor-not-allowed":"bg-purple-50 hover:bg-purple-100 shadow-sm hover:shadow-brutal-sm"}">
                                        <div class="flex justify-between items-center">
                                            <span class="text-purple-800">${v.nombrePaquete} ($${Number(v.precioPaqueteUsd).toFixed(2)})</span>
                                            <span class="text-purple-600">${v.unidadesPorCaja||1} un.</span>
                                        </div>
                                    </button>`:""}
                                </div>`}).join("")}
                        </div>
                    ...
                    <div class="border-t-2 border-brand-black pt-4 grid grid-cols-1 sm:grid-cols-3 gap-3">
                        ...
                        <button id="btn-cerrar-cuenta" class="${x>0?"bg-emerald-600 hover:bg-emerald-700":"bg-green-700 hover:bg-green-800"} text-white font-heading font-black text-xs sm:text-sm py-3.5 rounded border-2 border-brand-black shadow-brutal active:translate-y-0.5">
        ...
        </div>`,this.vincularEventos()}vincularEventos(){var a,n,c,i,b,t,r,s,d,l;(a=document.getElementById("btn-nueva-cuenta"))==null||a.addEventListener("click",()=>this.modalNuevaCuenta("activa")),(n=document.getElementById("btn-nueva-deuda"))==null||n.addEventListener("click",()=>this.modalNuevaCuenta("deuda")),(c=document.getElementById("tab-todas"))==null||c.addEventListener("click",()=>{this.filtroTab="todas",this.paginaCuentas=1,this.render()}),(i=document.getElementById("tab-activas"))==null||i.addEventListener("click",()=>{this.filtroTab="activa",this.paginaCuentas=1,this.render()}),(b=document.getElementById("tab-deudas"))==null||b.addEventListener("click",()=>{this.filtroTab="deuda",this.paginaCuentas=1,this.render()}),(t=document.getElementById("cta-pag-ant"))==null||t.addEventListener("click",()=>{this.paginaCuentas>1&&(this.paginaCuentas--,this.render())}),(r=document.getElementById("cta-pag-sig"))==null||r.addEventListener("click",()=>{this.paginaCuentas++,this.render()}),this.contenedor.querySelectorAll("[data-cuenta-id]").forEach(p=>{p.addEventListener("click",()=>{const x=p.dataset.cuentaId;this.cuentaSeleccionada=this.cuentas.find(h=>h.ventaId===x)||null,this.render()})}),this.contenedor.querySelectorAll("[data-del-consumo]").forEach(p=>{p.addEventListener("click",async()=>{if(!this.cuentaSeleccionada)return;const x=p.dataset.delConsumo||"";try{const h=await D.eliminarConsumo(this.cuentaSeleccionada.ventaId,x);this.cuentaSeleccionada=h,this.render()}catch(h){this.mostrarToast(h instanceof Error?h.message:String(h),"error")}})});const e=this.contenedor.querySelector("#cta-buscar-prod");e&&he(e,40),e==null||e.addEventListener("input",()=>{const p=e.value.trim().toLowerCase(),x=this.contenedor.querySelector("#cta-grid-prod");if(x){const h=this.productos.filter(g=>!p||g.nombre.toLowerCase().includes(p));h.length===0?x.innerHTML='<p class="col-span-full text-center text-xs text-gray-400 py-4 font-bold">No se encontraron productos coincidentes.</p>':(x.innerHTML=h.map(g=>{const v=!g.sinStock&&Number(g.stock)<=0,E=g.precioPaqueteUsd&&g.nombrePaquete;return`
                        <div class="relative" data-producto-row="${g.sku}">
                            <button data-add-sku="${g.sku}" data-modo="unidad" data-agotado="${v?"1":"0"}" class="text-left border-2 border-brand-black rounded p-2 transition-all text-xs font-bold ${v?"bg-gray-100 opacity-60 cursor-not-allowed":"bg-gray-50 hover:bg-white shadow-sm hover:shadow-brutal-sm"} w-full">
                                <div class="flex justify-between items-start gap-1">
                                ...
                                <div class="flex justify-between items-center mt-1">
                                    ...
                                    <span class="text-[10px] ${v?"text-red-700 font-black":"text-gray-500"}">${we(g)}</span>
                                </div>
                            </button>
                            ${E?`
                            <button data-add-sku="${g.sku}" data-modo="paquete" data-agotado="${v?"1":"0"}" class="text-left border-2 border-brand-purple rounded p-1 transition-all text-[10px] font-bold mt-1 w-full ${v?"bg-gray-100 opacity-60 cursor-not-allowed":"bg-purple-50 hover:bg-purple-100 shadow-sm hover:shadow-brutal-sm"}">
                                <div class="flex justify-between items-center">
            ...
            `,L=t.moneda==="USD",_=L&&s>0?s:c,f=L?r*_:r,w=c>0?f/c:0,y=a+w,P=y-o,T=P*c;let C="";if(r<=1e-4)C=`
                ...
                `;else if(P<-.001){const q=Math.abs(P),z=q*c;C=`
            ...
            <div class="fixed inset-0 bg-black/40 flex items-center justify-center z-[100] p-4">
                    ...
                    <div class="space-y-3 mb-4">
                        ...
                        <div class="bg-purple-50 border border-brand-purple rounded p-2 text-xs font-bold space-y-0.5">
                            <div class="flex justify-between">
                                ...
                                <span data-aporte-bs class="text-brand-purple font-mono font-black">Bs. ${R(f)}</span>
                            </div>
                            <div class="flex justify-between">
                        ...
                        <div>
                            ...
                            <p class="text-[11px] text-gray-500 mt-1">Equivalente oficial acreditado: <span class="font-bold text-brand-black">$ ${R(w)} USD</span> (Tasa BCV: ${R(c)})</p>
                        </div>
                        `}
            ...
            </div>`,x()},p=()=>{const h=t.moneda==="USD",g=h&&s>0?s:c,v=h?r*g:r,E=c>0?v/c:0,I=a+E,L=I-o,_=L*c;let f="";if(r<=1e-4)f=`
                    <div class="rounded p-2.5 border-2 border-brand-black bg-gray-50 text-xs text-gray-700 font-bold">
                        <div class="flex justify-between items-center">
                            <span>Aporte de este abono:</span>
                            <span class="font-heading font-black text-sm text-gray-800">$0.00 USD (Bs. 0.00)</span>
                ...
                `;else if(L>.001)f=`
                    <div class="rounded p-2.5 border-2 border-emerald-600 bg-emerald-50 text-xs text-emerald-950 font-bold">
                        ...
                        <div class="flex justify-between items-center text-emerald-900 border-t border-emerald-300 pt-1">
                            <span>Saldo a favor resultante:</span>
                            <span class="font-heading font-black text-base text-emerald-600">+$${R(L)} USD (Bs. ${R(_)})</span>
                        </div>
                        <p class="text-[10px] text-emerald-800 mt-1">✓ Abono superior al consumo: este excedente se convertirá en ganancia o crédito al liquidar.</p>
                    </div>
                `;else if(L<-.001){const P=Math.abs(L),T=P*c;f=`
                    <div class="rounded p-2.5 border-2 border-brand-black bg-gray-50 text-xs text-gray-800 font-bold">
                        <div class="flex justify-between items-center mb-1">
                            <span>Nuevo Total Abonado:</span>
                ...
                `;const w=this.modal.querySelector("#abono-preview-container");w&&(w.innerHTML=f);const y=this.modal.querySelector("#modal-abono-ok");if(y&&(y.disabled=r<=0),h){const P=this.modal.querySelector("[data-aporte-bs]");P&&(P.textContent=`Bs. ${R(v)}`);const T=this.modal.querySelector("[data-abono-computable]");T&&(T.textContent=`$ ${R(E)} USD`)}},x=()=>{var g,v,E,I,L,_,f;const h=()=>{this.modal.innerHTML=""};(g=this.modal.querySelector("#modal-abono-cancel"))==null||g.addEventListener("click",h),(v=this.modal.querySelector("#select-metodo-abono"))==null||v.addEventListener("change",w=>{const y=w.target.value,P=e.find(T=>T.nombre===y);if(P){const T=t.moneda==="USD";t=P,P.moneda==="USD"?(s=c,T||(r=n)):T&&(r=Number(i)),l()}}),(E=this.modal.querySelector("#input-monto-abono"))==null||E.addEventListener("input",w=>{r=X(w.target.value),p()}),(I=this.modal.querySelector("#input-tasa-abono"))==null||I.addEventListener("input",w=>{s=X(w.target.value),p()}),(L=this.modal.querySelector("#btn-reset-tasa-abono"))==null||L.addEventListener("click",()=>{s=c,l()}),(_=this.modal.querySelector("#input-ref-abono"))==null||_.addEventListener("input",w=>{d=w.target.value}),(f=this.modal.querySelector("#modal-abono-ok"))==null||f.addEventListener("click",async()=>{if(!this.cuentaSeleccionada)return;const w=t.moneda==="USD",y=w&&s>0?s:c,P=w?r*y:r,T=c>0?P/c:0;if(T<=0||!Number.isFinite(T)){this.mostrarToast("Por favor ingrese un monto válido.","error");return}const C=await D.abonarCuenta(this.cuentaSeleccionada.ventaId,T,P);C&&(this.cuentaSeleccionada=C),h(),this.render()})};l()}modalEditarAbono(){var g,v,E,I;if(!this.cuentaSeleccionada)return;const e=X(this.cuentaSeleccionada.totalParcialUsd),o=X(this.cuentaSeleccionada.abonosUsd),a=this.modelo.tasaActual||807.39;this.modal.innerHTML=`
        <div class="fixed inset-0 bg-black/40 flex items-center justify-center z-[100] p-4">
            <div class="bg-white border-2 border-brand-black rounded-lg shadow-brutal p-6 w-full max-w-md">
                ...
                <div class="grid grid-cols-2 gap-3">
        ...
        </div>`;const n=this.modal.querySelector("#tab-edit-usd"),c=this.modal.querySelector("#tab-edit-bs"),i=this.modal.querySelector("#sec-edit-usd"),b=this.modal.querySelector("#sec-edit-bs"),t=this.modal.querySelector("#edit-abono-usd-val"),r=this.modal.querySelector("#edit-abono-bs-val"),s=this.modal.querySelector("#edit-usd-equiv"),d=this.modal.querySelector("#edit-bs-equiv"),l=this.modal.querySelector("#edit-preview-box");let p="usd";const x=()=>{let L=0;p==="usd"?(L=X(t==null?void 0:t.value),s&&(s.textContent=`Bs. ${(L*a).toFixed(2)}`)):(L=X(r==null?void 0:r.value)/a,d&&(d.textContent=`$${L.toFixed(2)} USD`)),Number.isFinite(L)||(L=0);const _=L-e;if(l)if(_>.001)l.className="rounded p-2.5 border-2 border-emerald-600 bg-emerald-50 text-xs text-emerald-950 font-bold",l.innerHTML=`
                        <div class="flex justify-between items-center mb-1">
                            <span>Nuevo Saldo Abonado:</span>
                            <span class="font-heading font-black text-sm text-emerald-700">$${R(L)}</span>
                        </div>
                        <div class="flex justify-between items-center text-emerald-900 border-t border-emerald-300 pt-1">
                            <span>Saldo a favor resultante:</span>
                    ...
                    `;else if(_<-.001){const f=Math.abs(_);l.className="rounded p-2.5 border-2 border-brand-black bg-gray-50 text-xs text-gray-800 font-bold",l.innerHTML=`
                        <div class="flex justify-between items-center mb-1">
                            <span>Nuevo Saldo Abonado:</span>
                            <span class="font-heading font-black text-sm text-brand-black">$${R(L)}</span>
                        </div>
                        <div class="flex justify-between items-center text-gray-900 border-t border-gray-300 pt-1">
                            <span>Saldo pendiente restante:</span>
                            <span class="font-heading font-black text-base text-brand-black">$${R(f)} USD</span>
                        </div>
                    `}else l.className="rounded p-2.5 border-2 border-green-600 bg-green-50 text-xs text-green-950 font-bold",l.innerHTML=`
                        <div class="flex justify-between items-center">
                            <span>Registro saldado exactamente:</span>
                            <span class="font-heading font-black text-base text-green-700">$${R(L)} USD</span>
                        </div>
                    `};t&&fe(t,5e4,8,x),r&&fe(r,5e7,11,x),x(),(g=this.modal.querySelector("#btn-preset-cero"))==null||g.addEventListener("click",()=>{const L=X(p==="usd"?t==null?void 0:t.value:r==null?void 0:r.value);(o>0||L>0)&&!window.confirm("¿Desea restablecer el saldo abonado a $0.00? Esta acción dejará los pagos registrados en cero.")||(t&&(t.value="0.00"),r&&(r.value="0.00"),x())}),(v=this.modal.querySelector("#btn-preset-exacto"))==null||v.addEventListener("click",()=>{t&&(t.value=e.toFixed(2)),r&&(r.value=(e*a).toFixed(2)),x()}),n==null||n.addEventListener("click",()=>{p="usd",n.className="flex-1 py-1.5 font-heading font-black text-xs rounded bg-brand-black text-white",c.className="flex-1 py-1.5 font-heading font-bold text-xs rounded text-brand-black hover:bg-white",i==null||i.classList.remove("hidden"),b==null||b.classList.add("hidden"),x()}),c==null||c.addEventListener("click",()=>{p="bs",c.className="flex-1 py-1.5 font-heading font-black text-xs rounded bg-brand-black text-white",n.className="flex-1 py-1.5 font-heading font-bold text-xs rounded text-brand-black hover:bg-white",b==null||b.classList.remove("hidden"),i==null||i.classList.add("hidden"),x()});const h=()=>{this.modal.innerHTML=""};(E=this.modal.querySelector("#modal-edit-cancel"))==null||E.addEventListener("click",h),(I=this.modal.querySelector("#modal-edit-ok"))==null||I.addEventListener("click",async()=>{if(!this.cuentaSeleccionada)return;let L=0;if(p==="usd"?L=X(t==null?void 0:t.value):L=X(r==null?void 0:r.value)/a,!Number.isFinite(L)||L<0||L>5e4){this.mostrarToast("Monto inválido. Ingrese un valor entre $0.00 y $50,000.00 USD.","error");return}if(Math.abs(L-o)>.009){const w=L===0?`¿Confirmas reiniciar el saldo abonado a $0.00? (Monto previo registrado: $${R(o)})`:`¿Confirmas modificar el saldo abonado de esta cuenta a $${R(L)} USD (Bs. ${R(L*a)})? (Monto previo registrado: $${R(o)})`;if(!window.confirm(w))return}const f=await D.editarAbonoCuenta(this.cuentaSeleccionada.ventaId,L);f&&(this.cuentaSeleccionada=f),h(),this.render()})}async modalCierre(){var _,f,w;if(!this.cuentaSeleccionada)return;const e=this.cuentaSeleccionada.tipo==="deuda",o=X(this.cuentaSeleccionada.totalParcialUsd),a=X(this.cuentaSeleccionada.abonosUsd),n=Math.max(0,Math.round((o-a)*100)/100),c=Math.max(0,Math.round((a-o)*100)/100),i=this.modelo.tasaActual||807.39;if(n<=.009){this.modal.innerHTML=`
            <div class="fixed inset-0 bg-black/40 flex items-center justify-center z-[100] p-4">
                <div class="bg-white border-2 border-brand-black rounded-lg shadow-brutal p-6 w-full max-w-md">
            ...
            </div>`;const y=()=>{this.modal.innerHTML=""};(_=this.modal.querySelector("#modal-liq-cancel"))==null||_.addEventListener("click",y);let P=!1;(f=this.modal.querySelector("#modal-liq-ok"))==null||f.addEventListener("click",async()=>{if(!P&&this.cuentaSeleccionada){P=!0;try{await D.cerrarCuenta(this.cuentaSeleccionada.ventaId,"0.00",i.toFixed(4)),this.cuentaSeleccionada=null,y(),this.render()}catch{P=!1}}});return}let b=[];try{b=await D.listarMetodosPago()}catch{b=[{nombre:"PUNTOD.VENTA",moneda:"BS"},{nombre:"BIOPAGO",moneda:"BS"},{nombre:"PAGO MOVIL",moneda:"BS"},{nombre:"TRANSF.BS.",moneda:"BS"},{nombre:"BS.EFEC.",moneda:"BS"},{nombre:"DOL.CASH",moneda:"USD"},{nombre:"ZELLE",moneda:"USD"},{nombre:"BINAN.USDT",moneda:"USD"}]}let t=i;const r=e?b.find(y=>y.nombre==="TRANSF.BS.")||b[0]||{nombre:"PUNTOD.VENTA",moneda:"BS"}:b.find(y=>y.nombre==="PUNTOD.VENTA")||b[0]||{nombre:"PUNTOD.VENTA",moneda:"BS"},s=this.cuentaSeleccionada.ventaId,d=this.pagosBorradorPorCuenta.get(s);let l;if(d&&d.length>0){if(l=d.map(y=>({...y})),l.length===1&&l[0]){const y=l[0];y.monto=y.moneda==="USD"?n:Number((n*t).toFixed(2)),y.tasaCambio=t}else if(l.length>1){const y=Number((n*t).toFixed(2));let P=0;for(let G=0;G<l.length-1;G++){const q=l[G],z=q.tasaCambio>0?q.tasaCambio:t;P+=q.moneda==="USD"?q.monto*z:q.monto}const T=Math.max(0,Math.round((y-P)*100)/100),C=l[l.length-1];C&&(C.monto=C.moneda==="USD"?t>0?Number((T/t).toFixed(2)):0:T,C.tasaCambio=t)}}else l=[{id:"p-1",metodo:r.nombre,moneda:r.moneda,monto:r.moneda==="USD"?n:Number((n*t).toFixed(2)),tasaCambio:t,referencia:""}];const p=this.resolucionVueltoPorCuenta.get(s);let x=(p==null?void 0:p.estado)||"PAGADO",h=(p==null?void 0:p.metodo)||(e?"TRANSF.BS.":"BS.EFEC."),g=(p==null?void 0:p.tasa)||t;b.some(y=>y.nombre===h)||(h=((w=b[0])==null?void 0:w.nombre)||(e?"TRANSF.BS.":"BS.EFEC."));let v=!1;const E=()=>{let y=0;for(const N of l)if(N.moneda==="USD"){const S=N.tasaCambio>0?N.tasaCambio:t;y+=N.monto*S}else y+=N.monto;y=Math.round(y*100)/100;const P=Math.round(n*t*100)/100,T=t>0?Math.round(y/t*100)/100:0,C=Math.max(0,Math.round((P-y)*100)/100),G=t>0?Math.max(0,Math.round(C/t*100)/100):0,q=Math.max(0,Math.round((y-P)*100)/100),z=t>0?Math.max(0,Math.round(q/t*100)/100):0,B=y>=P-.01&&l.every(N=>N.monto>0);return{cubiertoUsd:T,cubiertoBs:y,totalEsperadoBs:P,faltanteUsd:G,faltanteBs:C,vueltoUsd:z,vueltoBs:q,puedeConfirmar:B}},I=()=>{var S,j;const y=E(),P=b.filter(k=>k.moneda==="USD"),T=b.filter(k=>k.moneda==="BS"),C=l.map((k,M)=>{const J=P.map(O=>`<option value="${O.nombre}" ${O.nombre===k.metodo?"selected":""}>${O.nombre} ($ - Tasa Dinámica)</option>`).join(""),H=`
                        <optgroup label="MÉTODOS EN BOLÍVARES (Bs.)">
                            ${T.map(O=>`<option value="${O.nombre}" ${O.nombre===k.metodo?"selected":""}>${O.nombre} (Bs.)</option>`).join("")}
                        </optgroup>
                        <optgroup label="MÉTODOS EN DÓLARES ($ - TASA PROPIA)">
                            ${J}
                        </optgroup>
                    `,U=k.tasaCambio>0?k.tasaCambio:t,$=k.moneda==="USD"?k.monto*U:k.monto,V=t>0?$/t:0;return`
                    <div class="border-2 border-brand-black rounded-lg p-2.5 bg-gray-50 mb-2">
                        <div class="flex flex-wrap items-center justify-between gap-1 mb-1.5">
                            <span class="font-heading font-black text-xs uppercase text-gray-700">
                                Pago #${M+1} · <span class="${k.moneda==="USD"?"text-brand-purple":"text-brand-black"}">${k.moneda==="USD"?"Divisa ($ USD)":"Moneda Nacional (Bs.)"}</span>
                            </span>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-12 gap-2 items-center">
                            ...
                            <div class="sm:col-span-3">
                                <div class="flex items-center justify-between mb-0.5">
                                    <label class="block text-[10px] font-bold text-gray-500 uppercase">Tasa (Bs./$)</label>
                                    <button type="button" data-reset-tasa-cuenta="${k.id}" class="text-[9px] font-black uppercase text-brand-purple hover:underline" title="Restablecer a tasa oficial">Base: ${R(t)}</button>
                                </div>
                                <input type="number" step="0.01" min="1" max="100000" data-pago-tasa="${k.id}" value="${U>0?U:t}"
                        ...
                        <div class="flex flex-wrap items-center justify-between gap-1 mt-1 text-[11px]">
                            <span class="font-bold text-brand-black">Aporte directo: Bs. ${R(k.monto)}</span>
                            <span class="text-gray-500 font-mono">Equiv. base: $ ${R(V)}</span>
                        </div>
                        `}
                    </div>`}).join(""),G=b.find(k=>k.nombre===h),q=(G==null?void 0:G.moneda)==="USD",z=g>0?g:t,B=q&&z>0?Number((y.vueltoBs/z).toFixed(2)):y.vueltoUsd,N=y.faltanteBs>.009?`
                    ...
                    <div class="border-2 border-emerald-700 bg-emerald-50/70 rounded-lg p-3 mb-3">
                        <div class="flex flex-wrap items-center justify-between gap-1 mb-2">
                            ...
                            <span class="text-xs font-bold text-emerald-900 font-mono">$ ${R(y.vueltoUsd)} equiv.</span>
                        </div>
            ...
            </div>`,L()},L=()=>{var G,q,z,B,N,S,j,k,M,J,te;const y=()=>{this.cuentaSeleccionada&&(this.pagosBorradorPorCuenta.set(this.cuentaSeleccionada.ventaId,l),this.resolucionVueltoPorCuenta.set(this.cuentaSeleccionada.ventaId,{estado:x,metodo:h,tasa:g}))},P=()=>{y(),this.modal.innerHTML=""};(G=this.modal.querySelector("#modal-liq-cerrar"))==null||G.addEventListener("click",P),(q=this.modal.querySelector("#modal-liq-cancel"))==null||q.addEventListener("click",P),(z=this.modal.querySelector("#modal-liq-agregar-mas"))==null||z.addEventListener("click",()=>{y(),this.modal.innerHTML=""}),(B=this.modal.querySelector("#btn-vuelto-pagado-cuenta"))==null||B.addEventListener("click",()=>{x="PAGADO",y(),I()}),(N=this.modal.querySelector("#btn-vuelto-retenido-cuenta"))==null||N.addEventListener("click",()=>{x="RETENIDO",y(),I()}),(S=this.modal.querySelector("#select-metodo-vuelto-cuenta"))==null||S.addEventListener("change",H=>{h=H.target.value,y(),I()});const T=this.modal.querySelector("#input-tasa-vuelto-cuenta");if(T&&(T.addEventListener("input",H=>{const U=X(H.target.value);g=U>0?U:t,y()}),T.addEventListener("blur",()=>{I()})),(j=this.modal.querySelector("#btn-reset-tasa-vuelto-cuenta"))==null||j.addEventListener("click",()=>{g=t,y(),I()}),e){const H=this.modal.querySelector("#deuda-tasa-input"),U=this.modal.querySelector("#btn-tasa-bcv");H&&fe(H,1e5,9,()=>{const $=X(H.value);t=Number.isFinite($)&&$>0?$:i,y(),I()}),U==null||U.addEventListener("click",()=>{t=i,y(),I()})}this.modal.querySelectorAll("select[data-pago-metodo]").forEach(H=>{H.addEventListener("change",U=>{const $=H.dataset.pagoMetodo,V=U.target.value,O=l.find(Q=>Q.id===$);if(O){const Q=b.find(de=>de.nombre===V),oe=(Q==null?void 0:Q.moneda)||"BS";O.moneda=oe,O.tasaCambio=t,l.length===1?O.monto=O.moneda==="USD"?n:Number((n*t).toFixed(2)):oe!==O.moneda&&(oe==="USD"?O.monto=Number((t>0?O.monto/t:0).toFixed(2)):O.monto=Number((O.monto*t).toFixed(2))),O.metodo=V,y(),I()}})}),this.modal.querySelectorAll("input[data-pago-tasa]").forEach(H=>{H.addEventListener("input",U=>{const $=H.dataset.pagoTasa,V=X(U.target.value),O=l.find(Q=>Q.id===$);if(O){O.tasaCambio=V>0?V:t,y();const Q=E(),oe=this.modal.querySelector("#modal-liq-ok");oe&&(oe.disabled=!Q.puedeConfirmar)}}),H.addEventListener("blur",()=>{I()})}),this.modal.querySelectorAll("button[data-reset-tasa-cuenta]").forEach(H=>{H.addEventListener("click",()=>{const U=H.dataset.resetTasaCuenta,$=l.find(V=>V.id===U);$&&($.tasaCambio=t,y(),I())})}),this.modal.querySelectorAll("input[data-pago-monto]").forEach(H=>{H.addEventListener("input",U=>{const $=H.dataset.pagoMonto,V=X(U.target.value),O=l.find(Q=>Q.id===$);if(O){O.monto=V,y();const Q=E(),oe=this.modal.querySelector("#modal-liq-ok");oe&&(oe.disabled=!Q.puedeConfirmar)}}),H.addEventListener("blur",()=>{I()})}),this.modal.querySelectorAll("input[data-pago-ref]").forEach(H=>{H.addEventListener("input",U=>{const $=H.dataset.pagoRef,V=U.target.value,O=l.find(Q=>Q.id===$);O&&(O.referencia=V,y())})}),this.modal.querySelectorAll("button[data-eliminar-pago]").forEach(H=>{H.addEventListener("click",()=>{const U=H.dataset.eliminarPago;l=l.filter($=>$.id!==U),y(),I()})}),(k=this.modal.querySelector("#btn-agregar-pago-cuenta"))==null||k.addEventListener("click",()=>{let H=E();const U="p-"+Math.random().toString(36).slice(2,7),$=b.find(O=>!l.some(Q=>Q.metodo===O.nombre))||b[0]||{nombre:"BS.EFEC.",moneda:"BS"};if(H.faltanteBs<=.01&&l.length===1&&l[0]){const O=l[0],Q=O.tasaCambio>0?O.tasaCambio:t,oe=O.moneda==="USD"?O.monto*Q:O.monto,de=Math.round(oe/2*100)/100;O.monto=O.moneda==="USD"?Q>0?Number((de/Q).toFixed(2)):0:de,H=E()}const V=H.faltanteBs>0?H.faltanteBs:0;l.push({id:U,metodo:$.nombre,moneda:$.moneda,monto:$.moneda==="USD"?t>0?Number((V/t).toFixed(2)):0:V,tasaCambio:t,referencia:""}),y(),I()}),(M=this.modal.querySelector("#btn-toggle-nuevo-metodo-cuenta"))==null||M.addEventListener("click",()=>{v=!v,I()}),(J=this.modal.querySelector("#btn-guardar-nuevo-metodo-cuenta"))==null||J.addEventListener("click",()=>{(async()=>{const H=this.modal.querySelector("#input-nuevo-metodo-nombre-cuenta"),U=(H==null?void 0:H.value.trim())||"",$=this.modal.querySelector('input[name="radio-nuevo-metodo-moneda-cuenta"][value="USD"]'),V=$!=null&&$.checked?"USD":"BS";if(U)try{b=await D.crearMetodoPago(U,V),v=!1,I()}catch{}})()});let C=!1;(te=this.modal.querySelector("#modal-liq-ok"))==null||te.addEventListener("click",()=>{C||(C=!0,(async()=>{if(!this.cuentaSeleccionada)return;const H=this.modal.querySelector("#cobro-error-cuenta");try{const U=l.filter(ne=>ne.monto>0).map(ne=>{const ue=ne.moneda==="USD"&&ne.tasaCambio>0?ne.tasaCambio:t,Ee=ne.moneda==="USD"?ne.monto*ue:ne.monto,Ie=t>0?Ee/t:0;return{metodo:ne.metodo,moneda:ne.moneda,montoUsd:(ne.moneda==="USD"?ne.monto:Ie).toFixed(2),montoBs:Ee.toFixed(2),tasaCambio:ne.moneda==="USD"?ue.toFixed(2):void 0,referencia:ne.referencia.trim()||void 0}});if(U.length===0)throw new Error("Debe especificar al menos un pago válido.");const $=E(),V=U.reduce((ne,ue)=>ne+Number(ue.montoBs),0),O=b.find(ne=>ne.nombre===h),Q=(O==null?void 0:O.moneda)==="USD",oe=Q?g>0?g:t:void 0,de=Q&&oe?($.vueltoBs/oe).toFixed(2):$.vueltoUsd.toFixed(2),Te={estado:$.vueltoBs>.009?x:"SIN_VUELTO",metodo:$.vueltoBs>.009&&x==="PAGADO"?h:void 0,montoBs:$.vueltoBs>.009?$.vueltoBs.toFixed(2):void 0,montoUsd:$.vueltoBs>.009?de:void 0,tasa:$.vueltoBs>.009&&x==="PAGADO"&&oe?oe.toFixed(2):void 0},xe=this.cuentaSeleccionada.ventaId;await D.cerrarCuenta(xe,V.toFixed(2),t.toFixed(4),U,Te),this.pagosBorradorPorCuenta.delete(xe),this.resolucionVueltoPorCuenta.delete(xe),this.cuentaSeleccionada=null,this.modal.innerHTML="",this.render()}catch(U){C=!1,H&&(H.textContent=U instanceof Error?U.message.replace(/"/g,""):String(U),H.classList.remove("hidden"))}})())})};I()}mostrarToast(e,o="info"){var c;const a={success:"bg-emerald-600 text-white",error:"bg-red-600 text-white",info:"bg-brand-black text-white"},n=document.createElement("div");n.className=`fixed bottom-6 right-6 ${a[o]} border-2 border-brand-black rounded shadow-brutal px-5 py-4 font-heading font-bold max-w-md z-[110]`,n.innerHTML=`${e.replace(/"/g,"")} <button class="ml-3 underline font-black">cerrar</button>`,(c=n.querySelector("button"))==null||c.addEventListener("click",()=>n.remove()),document.body.appendChild(n),setTimeout(()=>n.remove(),8e3)}}const ie=(u,e,o,a)=>{u.maxLength=o,u.addEventListener("input",()=>{let n=u.value.replace(/[^0-9.,]/g,"");const c=n.split(/[.,]/);c.length>2?n=c[0]+"."+c.slice(1).join(""):c.length===2&&(n=c[0]+"."+c[1].slice(0,2)),n.length>o&&(n=n.slice(0,o));const i=parseFloat(n.replace(",","."));Number.isFinite(i)&&i>e&&(n=String(e)),u.value!==n&&(u.value=n),a&&a()})},Ce=(u,e,o,a,n)=>{u.maxLength=a,u.addEventListener("input",()=>{let c=u.value.replace(/[^0-9]/g,"");c.length>a&&(c=c.slice(0,a));const i=parseInt(c,10);Number.isFinite(i)&&i>o&&(c=String(o)),u.value!==c&&(u.value=c),n&&n()}),u.addEventListener("blur",()=>{const c=parseInt(u.value,10);Number.isFinite(c)&&c<e&&(u.value=String(e),n&&n())})},le=(u,e,o=!1)=>{u.maxLength=e,u.addEventListener("input",()=>{let a=o?u.value.replace(/[<>{}\\]/g,""):u.value.replace(/[^a-zA-Z0-9áéíóúÁÉÍÓÚñÑüÜ .,/_#-]/g,"");a.length>e&&(a=a.slice(0,e)),u.value!==a&&(u.value=a)})};class at{constructor(e,o,a=!0){A(this,"contenedor");A(this,"modelo");A(this,"categorias",[]);A(this,"tasasImpuestos",[]);A(this,"productos",[]);A(this,"busquedaRepo","");A(this,"busquedaCatalogo","");A(this,"filtroCategoriaRepo","todas");A(this,"filtroCategoriaCatalogo","todas");A(this,"tabActiva","alta");A(this,"semaforoStock",{rojoMax:5,amarilloMax:15});A(this,"POR_PAGINA_CAT",40);A(this,"POR_PAGINA_REPO",24);A(this,"paginaCat",1);A(this,"paginaRepo",1);this.duenoAutenticado=a,this.contenedor=e,this.modelo=o,this.modelo}getUnitLabel(e){return e.unidad==="kg"?"kg":e.unidad==="ml"?"ml":e.unidad==="un"?"un.":e.capacidades&2?"kg":"un."}getNombreCategoria(e){if(!e)return"General";const o=this.categorias.find(a=>a.id===e);return o?o.nombre:"General"}getConteoCategorias(){const e={todas:this.productos.length};return this.categorias.forEach(o=>{e[o.id]=0}),e["cat-general"]===void 0&&(e["cat-general"]=0),this.productos.forEach(o=>{const a=o.categoriaId||"cat-general";e[a]=(e[a]||0)+1}),e}async render(e){var s;e&&(this.tabActiva=e);const[o,a,n,c]=await Promise.all([D.categorias(),D.tasasImpuestos(),D.productos(),D.obtenerSemaforoStock()]);this.categorias=o,this.tasasImpuestos=a,this.productos=n,this.semaforoStock=c;const i=this.getConteoCategorias(),b="Ej: Artículo Comercial 1L, Presentación 500g, Pack Estándar...",r=((s=this.modelo.getConfig())==null?void 0:s.privacidadInventario)&&!this.duenoAutenticado;this.contenedor.innerHTML=`
        <div class="mb-4 sm:mb-6 flex flex-wrap items-center justify-between gap-3">
            <div>
        ...
        <div id="inv-seccion-ajustes" class="${this.tabActiva==="ajustes"?"block":"hidden"} grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="border border-brand-black rounded p-2 bg-gray-50 flex justify-between items-center text-xs">
                <div class="min-w-0 flex-1 pr-2">
                    ...
                    <p class="text-[10px] text-gray-500 font-bold">
                        <span class="text-brand-purple font-black">${x}</span> · ${g?"•••":`$${Number(s.precioUsd).toFixed(2)}`} · IVA ${s.impuestoPct}%
                    </p>
                </div>
                <div class="text-right shrink-0">
            ...
            <div class="border-2 border-brand-black rounded-lg p-3 bg-white shadow-sm flex flex-col justify-between h-[175px] min-h-[175px] max-h-[175px] box-border overflow-hidden">
                <div class="min-w-0">
                    ...
                    <p class="text-xs font-bold text-gray-500 mt-0.5">${E?'Stock: <span class="font-black text-gray-400">•••</span>':`Precio: $${Number(d.precioUsd).toFixed(2)} · Stock: <span class="font-black ${p}">${g}</span>`}</p>
                </div>
                ...
                <div class="flex items-center gap-1 mt-auto pt-2 border-t border-gray-100">
                    ...
                    `:""}
                    <input type="number" step="${x==="kg"||x==="ml"?"0.1":"1"}" min="${x==="kg"||x==="ml"?"0.01":"1"}" max="9999" maxlength="5" placeholder="Cant." data-repo-cant="${d.sku}" class="w-14 h-8 border-2 border-brand-black rounded px-1.5 py-0.5 font-bold text-xs shrink-0" />
                ...
                </div>`,e.insertAdjacentElement("afterend",d),(r=document.getElementById("repo-pag-ant"))==null||r.addEventListener("click",()=>{this.paginaRepo>1&&(this.paginaRepo--,this.renderRepoLista())}),(s=document.getElementById("repo-pag-sig"))==null||s.addEventListener("click",()=>{this.paginaRepo<c&&(this.paginaRepo++,this.renderRepoLista())})}e.querySelectorAll("input[data-repo-cant]").forEach(d=>{ie(d,9999,5)}),e.querySelectorAll("[data-repo-del]").forEach(d=>{d.addEventListener("click",()=>{const l=d.dataset.repoDel,p=this.productos.find(h=>h.sku===l);if(!l||!p)return;window.confirm(`¿Seguro que deseas eliminar permanentemente el producto "${p.nombre}" del catálogo?`)&&D.eliminarProducto(l).then(()=>{D.productos().then(h=>{this.productos=h,this.renderRepoLista(),this.renderCatalogoLista();const g=document.getElementById("repo-contador-estado");g&&(g.textContent=`${this.productos.length} artículos en depósito`)})})})}),e.querySelectorAll("[data-repo-in]").forEach(d=>{d.addEventListener("click",()=>{const l=d.dataset.repoIn,p=e.querySelector(`input[data-repo-cant="${l}"]`),x=p==null?void 0:p.value;if(l&&x&&Number(x)>0&&Number(x)<=9999){const h=e.querySelector(`select[data-repo-tipo="${l}"]`),g=(h==null?void 0:h.value)||"unidad",v=this.productos.find(I=>I.sku===l);let E=Number(x);g==="caja"&&(v!=null&&v.esCaja)&&(v!=null&&v.unidadesPorCaja)&&v.unidadesPorCaja>1&&(E=E*v.unidadesPorCaja),D.compraStock(l,String(E)).then(()=>{D.productos().then(I=>{this.productos=I,this.renderRepoLista(),this.renderCatalogoLista()})})}})}),e.querySelectorAll("[data-repo-red]").forEach(d=>{d.addEventListener("click",()=>{const l=d.dataset.repoRed,p=e.querySelector(`input[data-repo-cant="${l}"]`),x=p==null?void 0:p.value;if(l&&x&&Number(x)>0&&Number(x)<=9999){const h=e.querySelector(`select[data-repo-tipo="${l}"]`),g=(h==null?void 0:h.value)||"unidad",v=this.productos.find(I=>I.sku===l);let E=Number(x);g==="caja"&&(v!=null&&v.esCaja)&&(v!=null&&v.unidadesPorCaja)&&v.unidadesPorCaja>1&&(E=E*v.unidadesPorCaja),D.reducirStock(l,String(E)).then(()=>{D.productos().then(I=>{this.productos=I,this.renderRepoLista(),this.renderCatalogoLista()})})}})}),e.querySelectorAll("[data-repo-out]").forEach(d=>{d.addEventListener("click",()=>{const l=d.dataset.repoOut,p=e.querySelector(`input[data-repo-cant="${l}"]`),x=p==null?void 0:p.value;if(l&&x&&Number(x)>0&&Number(x)<=9999){const h=e.querySelector(`select[data-repo-tipo="${l}"]`),g=(h==null?void 0:h.value)||"unidad",v=this.productos.find(I=>I.sku===l);let E=Number(x);g==="caja"&&(v!=null&&v.esCaja)&&(v!=null&&v.unidadesPorCaja)&&v.unidadesPorCaja>1&&(E=E*v.unidadesPorCaja),D.merma(l,String(E),null).then(()=>{D.productos().then(I=>{this.productos=I,this.renderRepoLista(),this.renderCatalogoLista()})})}})})}async guardarProducto(){var G;const e=document.getElementById("prod-nombre"),o=document.getElementById("prod-categoria"),a=document.getElementById("prod-impuesto"),n=document.getElementById("prod-precio-bruto"),c=document.getElementById("prod-margen"),i=document.getElementById("prod-precio-neto"),b=document.getElementById("prod-stock"),t=((G=document.getElementById("prod-unidad"))==null?void 0:G.value)||"un",r=document.getElementById("prod-sin-stock"),s=document.getElementById("alta-prod-msg"),d=q=>{if(!q)return 0;const z=q.trim().replace(",","."),B=parseFloat(z);return Number.isFinite(B)&&!isNaN(B)?B:0};if(s&&(s.classList.add("hidden"),s.textContent=""),!(e!=null&&e.value.trim())){s&&(s.textContent="El nombre del producto o servicio es obligatorio.",s.style.color="#C60C15",s.classList.remove("hidden"),e==null||e.focus());return}const l=d(i==null?void 0:i.value);if(l<=0||l>99999.99){s&&(s.textContent="El precio neto final debe ser un monto positivo entre $0.01 y $99,999.99 USD.",s.style.color="#C60C15",s.classList.remove("hidden"),i==null||i.focus());return}const p=d(b==null?void 0:b.value),x=(r==null?void 0:r.checked)||!1;if(!x&&(p<0||p>99999)){s&&(s.textContent="El stock inicial debe estar comprendido entre 0 y 99,999 unidades.",s.style.color="#C60C15",s.classList.remove("hidden"),b==null||b.focus());return}if(!x&&t==="un"&&(!Number.isInteger(p)||p%1!==0)){s&&(s.textContent='Los productos vendidos por Unidad (un.) deben tener un stock entero exacto (ej: 10). Para registrar fracciones o decimales, seleccione Tipo de Medida "Por Peso (kg)" o "Por Volumen (ml)".',s.style.color="#C60C15",s.classList.remove("hidden"),b==null||b.focus());return}const h=d((a==null?void 0:a.value)||"0"),g=d((c==null?void 0:c.value)||"30");let v=d(n==null?void 0:n.value);v<=0&&(v=l/(1+h/100)/(1+g/100));const E=document.getElementById("prod-es-caja"),I=document.getElementById("prod-unidades-caja"),L=(E==null?void 0:E.checked)||!1,_=L&&I?Math.max(2,parseInt(I.value,10)||12):void 0,f=document.getElementById("prod-tiene-paquete"),w=document.getElementById("prod-nombre-paquete"),y=document.getElementById("prod-precio-paquete"),P=(f==null?void 0:f.checked)||!1,T=P&&w?w.value.trim():void 0,C=P&&y?d(y.value).toFixed(2):void 0;try{await D.crearProducto({nombre:e.value.trim(),precioBrutoUsd:v.toFixed(2),margenPct:String(g),precioUsd:l.toFixed(2),impuestoPct:String(h),stockInicial:r!=null&&r.checked?"0":String(p),categoriaId:(o==null?void 0:o.value)||"",sinStock:(r==null?void 0:r.checked)||!1,pesable:t==="kg"||t==="ml",unidad:t,alcoholica:!1,esCaja:L,unidadesPorCaja:_,precioPaqueteUsd:C,nombrePaquete:T}),s&&(s.textContent="Producto o servicio registrado exitosamente.",s.style.color="#00823B",s.classList.remove("hidden")),setTimeout(()=>void this.render("alta"),800)}catch(q){s&&(s.textContent=q instanceof Error?q.message:String(q),s.style.color="#C60C15",s.classList.remove("hidden"))}}}const be=u=>{if(typeof u=="number")return Number.isFinite(u)?u:0;if(u==null)return 0;let e=String(u).trim();if(!e)return 0;e.includes(",")&&!e.includes(".")?e=e.replace(",","."):e.includes(".")&&e.includes(",")&&(e=e.replace(/\./g,"").replace(",","."));const o=parseFloat(e);return Number.isFinite(o)&&!isNaN(o)?o:0},K=u=>be(u).toLocaleString("es-VE",{minimumFractionDigits:2,maximumFractionDigits:2}),Ue=u=>{const e=u.finUnix||Math.floor(Date.now()/1e3),o=Math.max(0,e-u.inicioUnix),a=Math.floor(o/3600),n=Math.floor(o%3600/60);return`${a}h ${n}m`},ot=(u,e)=>{const o=u.operadoresRelevo.length>0?u.operadoresRelevo.join(" / "):u.operadorActual,a=e.filter(d=>d.operador&&u.operadoresRelevo.some(l=>d.operador===l)),n=[["INFORME DE JORNADA",u.id],["Estado",u.estado==="abierta"?"En curso":"Cerrada"],["Apertura",u.inicioStr],["Cierre",u.finStr||"-"],["Duracion",Ue(u)],["Operadores",o],["Total USD",`$${K(u.ventasTotalUsd)}`],["Total Bs",`Bs. ${K(u.ventasTotalBs)}`],["Tickets emitidos",String(u.ticketsEmitidos)],["Vuelto pagado Bs",`Bs. ${K(u.vueltoPagadoBs)}`],["Vuelto retenido Bs",`Bs. ${K(u.vueltoRetenidoBs)}`],["Deudas liquidadas USD",`$${K(u.deudasLiquidadasUsd)}`],["Entradas stock",String(u.entradasStockReg)],["Mermas",String(u.mermasStockReg)],["Cambios de precio",String(u.cambiosPrecioReg)],["Tasa apertura",`Bs. ${u.tasaInicio}`],["Tasa cierre",u.tasaFin?`Bs. ${u.tasaFin}`:"-"],...u.checksumSha256?[["SHA-256",u.checksumSha256]]:[],[],["TICKETS DE LA JORNADA"],["ID Ticket","Fecha/Hora","Canal","Operador","Metodo(s) de Pago","Total USD","Total Bs"]],c=a.map(d=>{const l=d.pagos&&d.pagos.length>0?d.pagos.map(p=>`${p.metodo}:$${K(p.montoUsd)}`).join(" | "):"DIRECTO";return[d.ventaId,d.fechaHora||"",d.canal||"VENTA DIRECTA",d.operador||"Caja",l,`$${K(d.totalUsd)}`,`Bs. ${K(d.totalBs)}`]}),b=[...n,...c].map(d=>d.map(l=>`"${String(l).replace(/"/g,'""')}"`).join(",")).join(`\r
`),t=new Blob(["\uFEFF"+b],{type:"text/csv;charset=utf-8;"}),r=URL.createObjectURL(t),s=document.createElement("a");s.href=r,s.download=`jornada_${u.id}.csv`,document.body.appendChild(s),s.click(),document.body.removeChild(s),URL.revokeObjectURL(r)};class nt{constructor(e,o,a){A(this,"contenedor");A(this,"modalRoot");A(this,"modelo");A(this,"jornadas",[]);A(this,"jornadaActual",null);A(this,"todosLosTickets",[]);A(this,"busqueda","");A(this,"expandidos",new Set);A(this,"paginaActual",1);A(this,"POR_PAGINA",20);this.contenedor=e,this.modalRoot=o,this.modelo=a}parseFechaUnix(e){if(!e)return 0;const o=e.match(/(\d{4})-(\d{2})-(\d{2})[T\s](\d{2}):(\d{2}):(\d{2})/);if(o&&o[1]&&o[2]&&o[3]&&o[4]&&o[5]&&o[6]){const a=Date.UTC(parseInt(o[1],10),parseInt(o[2],10)-1,parseInt(o[3],10),parseInt(o[4],10),parseInt(o[5],10),parseInt(o[6],10));if(!isNaN(a))return Math.floor(a/1e3)}return 0}async render(){const[e,o,a]=await Promise.all([D.obtenerJornadaActual(),D.listarHistoricoJornadas(),D.ventas()]);this.jornadaActual=e,this.jornadas=o,this.todosLosTickets=a;const n=(e?1:0)+o.length,c=[...e?[e]:[],...o],i=c.reduce((t,r)=>t+be(r.ventasTotalUsd),0),b=c.reduce((t,r)=>t+r.ticketsEmitidos,0);this.contenedor.innerHTML=`
        <div class="mb-4 sm:mb-6 flex flex-wrap items-center justify-between gap-3">
            <div>
        ...
        <div id="lista-jornadas" class="flex flex-col gap-2"></div>`,this.conectarEventosBuscador(),this.renderLista()}conectarEventosBuscador(){var e;(e=document.getElementById("jornadas-buscar"))==null||e.addEventListener("input",o=>{this.busqueda=o.target.value,this.paginaActual=1,this.renderLista()})}renderLista(){var s,d;const e=document.getElementById("lista-jornadas");if(!e)return;const o=[...this.jornadaActual?[this.jornadaActual]:[],...this.jornadas],a=this.busqueda.trim().toLowerCase(),n=a?o.filter(l=>l.id.toLowerCase().includes(a)||l.operadorActual.toLowerCase().includes(a)||l.operadoresRelevo.some(p=>p.toLowerCase().includes(a))||l.inicioStr.toLowerCase().includes(a)||(l.finStr||"").toLowerCase().includes(a)):o,c=document.getElementById("paginacion-jornadas");if(c&&c.remove(),n.length===0){e.innerHTML=`
            ...
            </div>`;return}const i=Math.max(1,Math.ceil(n.length/this.POR_PAGINA));this.paginaActual>i&&(this.paginaActual=i),this.paginaActual<1&&(this.paginaActual=1);const b=this.paginaActual,t=n.slice((b-1)*this.POR_PAGINA,b*this.POR_PAGINA);e.innerHTML=t.map(l=>this.renderFila(l)).join(""),t.forEach(l=>{if(this.expandidos.has(l.id)){const p=document.getElementById(`detalle-${l.id}`);p==null||p.classList.remove("hidden");const x=document.getElementById(`icon-${l.id}`);x&&(x.textContent="▲")}}),e.querySelectorAll("[data-toggle-jornada]").forEach(l=>{l.addEventListener("click",()=>{const p=l.dataset.toggleJornada,x=document.getElementById(`detalle-${p}`),h=document.getElementById(`icon-${p}`);if(x){const g=x.classList.contains("hidden");x.classList.toggle("hidden",!g),h&&(h.textContent=g?"▲":"▼"),g?this.expandidos.add(p):this.expandidos.delete(p)}})}),e.querySelectorAll("[data-exportar-jornada]").forEach(l=>{l.addEventListener("click",p=>{p.stopPropagation();const x=l.dataset.exportarJornada,h=n.find(g=>g.id===x);h&&ot(h,this.todosLosTickets)})}),e.querySelectorAll("[data-ver-ticket]").forEach(l=>{l.addEventListener("click",()=>{const p=l.dataset.verTicket,x=this.todosLosTickets.find(h=>h.ventaId===p);x&&this.mostrarModalTicket(x)})});const r=document.createElement("div");r.id="paginacion-jornadas",r.innerHTML=`
        ...
        </div>`;const n=()=>{this.modalRoot.innerHTML=""};(c=document.getElementById("modal-ticket-cerrar"))==null||c.addEventListener("click",n),(i=document.getElementById("modal-ticket-ok"))==null||i.addEventListener("click",n)}}class rt{constructor(e){A(this,"contenedor");this.contenedor=e}render(){this.contenedor.innerHTML=`
        ...
        </div>`}}const ve=4e3,st=3e5;class dt{constructor(){A(this,"modelo",new Ke);A(this,"cajaVm",null);A(this,"panelVm",new Ye);A(this,"widget",new Je);A(this,"root");A(this,"modalRoot");A(this,"peerConnection",null);A(this,"dataChannel",null);A(this,"wsSignaling",null);A(this,"offerResendTimer",null);A(this,"p2pPin","");A(this,"signalingRetries",0);A(this,"p2pConnected",!1);A(this,"roomId","");A(this,"reconectando",!1);A(this,"STUN_SERVERS",{iceServers:[{urls:"stun:stun.cloudflare.com:3478"},{urls:"stun:stun.l.google.com:19302"},{urls:"turn:openrelay.metered.ca:80",username:"openrelayproject",credential:"openrelayproject"},{urls:"turn:openrelay.metered.ca:443",username:"openrelayproject",credential:"openrelayproject"},{urls:"turn:openrelay.metered.ca:443?transport=tcp",username:"openrelayproject",credential:"openrelayproject"}]});A(this,"p2pSessionCookie","");A(this,"keepaliveTimer",null);A(this,"keepalivePending",!1)}async arrancar(){var c;const e=document.getElementById("app-root");if(!e)return;this.root=e,this.modalRoot=this.crearModalRoot(),this.widget.iniciar();const a=new URLSearchParams(window.location.search).get("room");if(a){await this.arrancarMovil(e,a);return}window.addEventListener("tasa_actualizada",i=>{const b=i;b.detail&&this.modelo.setTasa(b.detail)}),await this.refrescarTasa();let n=null;try{n=await this.modelo.cargarConfig()}catch{n=await this.modelo.cargarConfig()}if(window.setInterval(()=>void this.refrescarTasa(),st),!n){new Ze(this.root,this.modelo,()=>void this.arrancar()).render();return}this.pintarBotonesRol(n),(c=document.getElementById("btn-conectar-movil"))==null||c.addEventListener("click",()=>this.abrirModalQrMovil()),this.ejecutarBackupAutomatico(),await this.arrancarCaja()}async arrancarMovil(e,o){e.innerHTML=`
        <div class="flex flex-col items-center justify-center h-screen bg-gray-50">
            <div class="text-center">
                ...
                <p id="movil-status" class="text-xs text-gray-400 mt-1">Conectando al servidor de senalizacion...</p>
                        ...
                        </div>`},d.onmessage=p=>{try{const x=JSON.parse(p.data);if(x.type==="ping"){try{d.send(JSON.stringify({type:"pong",ts:x.ts}))}catch{}return}const h=window.__P2P_PENDING__;if(h&&x.id&&h.has(x.id)){const g=h.get(x.id);h.delete(x.id),x.error?g.reject(new Error(x.error)):g.resolve(x.body)}}catch{}},await s.setRemoteDescription(new RTCSessionDescription(t.sdp));const l=await s.createAnswer();await s.setLocalDescription(l),i.readyState===WebSocket.OPEN&&i.send(JSON.stringify({type:"answer",sdp:s.localDescription}))}else t.type==="error"&&(this.root.innerHTML=`
                    <div class="flex flex-col items-center justify-center h-screen bg-gray-50">
                        <div class="text-center">
        ...
        </div>`;const o=()=>{this.modalRoot.innerHTML=""};(n=document.getElementById("pin-cancelar"))==null||n.addEventListener("click",o);const a=()=>void(async()=>{var r;const b=document.getElementById("pin-input").value;await D.validarPin(b).catch(()=>!1)?(o(),await this.abrirPanel()):(r=document.getElementById("pin-error"))==null||r.classList.remove("hidden")})();(c=document.getElementById("pin-ok"))==null||c.addEventListener("click",a),(i=document.getElementById("pin-input"))==null||i.addEventListener("keydown",b=>{b.key==="Enter"&&a()})}async abrirPanel(){this.marcarActivo("panel");const e=new et(this.root,this.modalRoot,this.panelVm,this.modelo);try{await e.render()}catch(o){this.toastError(o instanceof Error?o.message:String(o))}}async abrirModalQrMovil(){var i,b,t;let e=null;try{e=await D.generarQr()}catch{e=null}const o=(e==null?void 0:e.url)||`http://127.0.0.1:${ve}/panel`,a=(e==null?void 0:e.qrBase64)||"",n=(e==null?void 0:e.roomId)||"";this.modalRoot.innerHTML=`
        <div class="fixed inset-0 bg-black/40 flex items-center justify-center z-[100] p-4">
            <div class="bg-white border-2 border-brand-black rounded-lg shadow-brutal p-6 w-full max-w-lg">
                <div class="flex justify-between items-center border-b-2 border-brand-black pb-3 mb-4">
                    <div>
