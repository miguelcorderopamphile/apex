            ...
            <div class="bg-white border-2 border-brand-black rounded-lg shadow-brutal p-6 w-full max-w-lg">
                ...
                <div>
                    ...
                    <div class="space-y-1.5 max-h-48 overflow-y-auto pr-1">
                        ${a.length===0?'<p class="text-xs text-gray-400 font-bold py-4 text-center">Sin registros históricos previos.</p>':a.map(r=>`
...
Esta tasa afectará todos los nuevos cobros y tickets a partir de este momento.`)){const g=await H("fijar_tasa_manual",{tasa:c.toFixed(2)});g&&this.pintar(g),n()}})}async actualizar(){const e=await this.obtenerTasa("obtener_tasa_bcv");e&&this.pintar(e)}async forzar(){const e=document.getElementById("bcv-refresh");e&&e.classList.add("animate-spin");const a=await this.obtenerTasa("forzar_actualizacion_tasa");e&&e.classList.remove("animate-spin"),a&&this.pintar(a)}async obtenerTasa(e){try{return await H(e)}catch{return null}}pintar(e){const a=document.getElementById("bcv-valor"),o=document.getElementById("bcv-fluctuacion");if(!a||!o)return;const n=Number(e.valor);a.textContent=Number.isFinite(n)&&n>0?n.toLocaleString("es-VE",{minimumFractionDigits:2,maximumFractionDigits:2}):"--",Number.isFinite(n)&&n>0&&window.dispatchEvent(new CustomEvent("tasa_actualizada",{detail:n}));const l=Number(e.fluctuacionPct);if(e.direccion&&Number.isFinite(l)){const i=e.direccion==="subio"?Re:e.direccion==="bajo"?Ne:Me,s=l>0?"+":"";o.textContent=`${s}${l.toFixed(2)}%`,o.style.color=i}else o.textContent=""}}class Fe{constructor(){w(this,"config",null);w(this,"rol","caja");w(this,"tasa",0);w(this,"oyentes",[])}suscribir(e){this.oyentes.push(e)}notificar(){this.oyentes.forEach(e=>e())}async cargarConfig(){return this.config=await D.config(),this.config}getConfig(){return this.config}async inicializar(e,a,o){await D.inicializar(e,a,o),await this.cargarConfig(),this.notificar()}get rubroAbasto(){var e,a;return(((e=this.config)==null?void 0:e.rubros)??0&pe)!==0||(((a=this.config)==null?void 0:a.rubros)??0)===pe}tieneRubro(e){var a;return((((a=this.config)==null?void 0:a.rubros)??0)&e)!==0}tieneCapacidad(e){var a;return((((a=this.config)==null?void 0:a.capacidades)??0)&e)!==0}setRol(e){this.rol=e,this.notificar()}get rolActual(){return this.rol}setTasa(e){this.tasa=e,this.notificar()}get tasaActual(){return this.tasa}bs(e){return(Number(e)*this.tasa).toLocaleString("es-VE",{minimumFractionDigits:2,maximumFractionDigits:2})}}class qe{constructor(e,a,o){w(this,"contenedor");w(this,"modelo");w(this,"alTerminar");w(this,"modo","nuevo");w(this,"bloquearPanel",!0);this.contenedor=e,this.modelo=a,this.alTerminar=o}render(){this.contenedor.innerHTML=`
        ...
        <div class="fixed inset-0 bg-black/40 flex items-center justify-center z-[100] p-4">
            ...
            `,"max-w-md"),(n=this.modal.querySelector("[data-cerrar]"))==null||n.addEventListener("click",()=>this.cerrarModal()),(l=this.modal.querySelector("#btn-cancelar-inicio-jornada"))==null||l.addEventListener("click",()=>this.cerrarModal()),(i=this.modal.querySelector("#form-nuevo-cajero-inicio"))==null||i.addEventListener("submit",async t=>{t.preventDefault();const r=this.modal.querySelector("#in-nuevo-cajero-inicio"),d=(r==null?void 0:r.value.trim())||"";d&&(await D.crearOperador(d),this.cerrarModal(),this.abrirModalIniciarJornada())}),(s=this.modal.querySelector("#btn-abrir-turno"))==null||s.addEventListener("click",async()=>{const t=this.modal.querySelectorAll("input[data-inicio-cajero-chk]:checked"),r=[];if(t.forEach(c=>{c.value&&r.push(c.value)}),r.length===0){alert("Selecciona al menos un operador para abrir el turno.");return}const d=r[0];await D.abrirJornada(d,r),this.cerrarModal(),this.renderDinamico()})})()}async abrirModalCobro(){var h,y,k,U;if(this.vm.lineasCarrito.length===0)return;let e=[];try{e=await D.listarMetodosPago()}catch{e=[{nombre:"PUNTOD.VENTA",moneda:"BS"},{nombre:"BIOPAGO",moneda:"BS"},{nombre:"PAGO MOVIL",moneda:"BS"},{nombre:"TRANSF.BS.",moneda:"BS"},{nombre:"BS.EFEC.",moneda:"BS"},{nombre:"DOL.CASH",moneda:"USD"},{nombre:"ZELLE",moneda:"USD"},{nombre:"BINAN.USDT",moneda:"USD"}]}const a=Number(this.vm.totalUsd.toFixed(2)),o=this.vm.tasaTicket,n=Number((a*o).toFixed(2)),l=e.find(A=>A.nombre==="PUNTOD.VENTA")||e.find(A=>A.moneda==="BS")||e[0]||{nombre:"PUNTOD.VENTA",moneda:"BS"};let i;if(this.pagosBorrador&&this.pagosBorrador.length>0){if(i=this.pagosBorrador.map(A=>({...A})),i.length===1&&i[0]){const A=i[0];A.monto=A.moneda==="USD"?a:n,A.tasaCambio=o}else if(i.length>1){let A=0;for(let F=0;F<i.length-1;F++){const C=i[F],f=C.tasaCambio>0?C.tasaCambio:o;A+=C.moneda==="USD"?C.monto*f:C.monto}const O=Math.max(0,Math.round((n-A)*100)/100),R=i[i.length-1];R&&(R.monto=R.moneda==="USD"?o>0?Number((O/o).toFixed(2)):0:O,R.tasaCambio=o)}}else i=[{id:"p-1",metodo:l.nombre,moneda:l.moneda,monto:l.moneda==="USD"?a:n,tasaCambio:o,referencia:""}];let s=((h=this.resolucionVueltoBorrador)==null?void 0:h.estado)||"PAGADO",t=((y=this.resolucionVueltoBorrador)==null?void 0:y.metodo)||"BS.EFEC.",r=((k=this.resolucionVueltoBorrador)==null?void 0:k.tasa)||o;e.some(A=>A.nombre===t)||(t=((U=e[0])==null?void 0:U.nombre)||"BS.EFEC.");let d=!1;const c=()=>{let A=0;for(const S of i)if(S.moneda==="USD"){const M=S.tasaCambio>0?S.tasaCambio:o;A+=S.monto*M}else A+=S.monto;A=Math.round(A*100)/100;const O=o>0?Math.round(A/o*100)/100:0,R=Math.max(0,Math.round((n-A)*100)/100),F=o>0?Math.max(0,Math.round(R/o*100)/100):0,C=Math.max(0,Math.round((A-n)*100)/100),f=o>0?Math.max(0,Math.round(C/o*100)/100):0,m=A>=n-.01&&i.every(S=>S.monto>0);return{cubiertoUsd:O,cubiertoBs:A,faltanteUsd:F,faltanteBs:R,vueltoUsd:f,vueltoBs:C,puedeConfirmar:m}},b=()=>{const A=c(),O=e.filter(B=>B.moneda==="USD"),R=e.filter(B=>B.moneda==="BS"),F=i.map((B,_)=>{const x=O.map(E=>`<option value="${E.nombre}" ${E.nombre===B.metodo?"selected":""}>${E.nombre} ($ - Tasa Dinámica)</option>`).join(""),T=`
                        <optgroup label="MÉTODOS EN BOLÍVARES (Bs.)">
                            ${R.map(E=>`<option value="${E.nombre}" ${E.nombre===B.metodo?"selected":""}>${E.nombre} (Bs.)</option>`).join("")}
                        </optgroup>
                        <optgroup label="MÉTODOS EN DÓLARES ($ - TASA PROPIA)">
                            ${x}
                        </optgroup>
                    `,I=B.tasaCambio>0?B.tasaCambio:o,G=B.moneda==="USD"?B.monto*I:B.monto,P=o>0?G/o:0;return`
                    <div class="border-2 border-brand-black rounded-lg p-3 bg-gray-50 mb-2.5">
                        <div class="flex flex-wrap items-center justify-between gap-2 mb-2">
                            <span class="font-heading font-black text-xs uppercase text-gray-700">
                                Pago #${_+1} · <span class="${B.moneda==="USD"?"text-brand-purple":"text-brand-black"}">${B.moneda==="USD"?"Divisa ($ USD)":"Moneda Nacional (Bs.)"}</span>
                            </span>
                            ${i.length>1?`<button data-eliminar-pago="${B.id}" class="text-xs font-black text-red-600 hover:text-red-800 bg-red-50 border border-red-300 rounded px-2 py-0.5">Eliminar</button>`:""}
                        </div>
                        ${B.moneda==="USD"?`
                        <div class="grid grid-cols-1 sm:grid-cols-12 gap-2 items-center">
                            <div class="sm:col-span-4">
                                <label class="block text-[10px] font-bold text-gray-500 uppercase mb-0.5">Medio en Divisa</label>
                                <select data-pago-metodo="${B.id}" class="w-full border-2 border-brand-black rounded px-2.5 py-1.5 font-heading font-black text-xs bg-white focus:outline-none focus:ring-2 focus:ring-brand-purple">
                                    ${T}
                                </select>
                            </div>
                            <div class="sm:col-span-3">
                                ...
                                <div class="flex items-center">
                                    ...
                                    <input type="number" step="0.01" min="0.01" max="999999" data-pago-monto="${B.id}" value="${B.monto>0?B.monto:""}" placeholder="0.00"
                            ...
                            <div class="sm:col-span-3">
                                <div class="flex items-center justify-between mb-0.5">
                                    ...
                                    <button type="button" data-reset-tasa="${B.id}" class="text-[9px] font-black uppercase text-brand-purple hover:underline" title="Restablecer a tasa BCV oficial">BCV: ${Y(o)}</button>
                                </div>
                                <input type="number" step="0.01" min="1" max="100000" data-pago-tasa="${B.id}" value="${I>0?I:o}"
                            ...
                            <div class="sm:col-span-2">
                                ...
                                <input type="text" maxlength="25" data-pago-ref="${B.id}" value="${B.referencia||""}" placeholder="Opcional"
                        ...
                        <div class="flex flex-wrap items-center justify-between gap-1 mt-1.5 text-[11px]">
                            ...
                            <span class="text-gray-500 font-mono">Equiv. oficial BCV: $ ${Y(P)} ${I!==o?`(Tasa: Bs. ${Y(I)} vs BCV: Bs. ${Y(o)})`:""}</span>
                        </div>
                        `:`
                        <div class="grid grid-cols-1 sm:grid-cols-12 gap-2 items-center">
                            ...
                            <div class="sm:col-span-4">
                                ...
                                <div class="flex items-center">
                                    ...
                                    <input type="number" step="0.01" min="0.01" max="999999999" data-pago-monto="${B.id}" value="${B.monto>0?B.monto:""}" placeholder="0.00"
                                        class="w-full border-2 border-brand-black rounded-r px-2 py-1.5 text-xs font-mono font-black focus:outline-none focus:ring-2 focus:ring-brand-purple text-right" />
                                </div>
                            </div>
                            <div class="sm:col-span-3">
                        ...
                        <div class="flex flex-wrap items-center justify-between gap-1 mt-1.5 text-[11px]">
                            <span class="font-bold text-brand-black">Aporte directo: Bs. ${Y(B.monto)}</span>
                            <span class="text-gray-500 font-mono">Equiv. oficial BCV: $ ${Y(P)}</span>
                        </div>
                        `}
                    </div>`}).join(""),C=e.find(B=>B.nombre===t),f=(C==null?void 0:C.moneda)==="USD",m=r>0?r:o,S=f&&m>0?Number((A.vueltoBs/m).toFixed(2)):A.vueltoUsd,M=A.faltanteBs>.009?`
                    <div class="bg-amber-50 border-2 border-amber-500 rounded p-3 text-center mb-3">
                        ...
                        <p class="font-heading font-black text-xl text-amber-700">Faltan Bs. ${Y(A.faltanteBs)} · <span class="text-base text-amber-900">$ ${Y(A.faltanteUsd)} (Equiv. Oficial)</span></p>
                    </div>`:A.vueltoBs>.009?`
                    <div class="border-2 border-emerald-700 bg-emerald-50/70 rounded-lg p-3 mb-3">
                        <div class="flex flex-wrap items-center justify-between gap-1 mb-2">
                            <div>
                                ...
                                <span class="font-heading font-black text-xl text-emerald-800">Bs. ${Y(A.vueltoBs)}</span>
                            </div>
                            <span class="text-xs font-bold text-emerald-900 font-mono">$ ${Y(A.vueltoUsd)} equiv. oficial</span>
                        </div>
                        ...
                        <div class="bg-white border border-emerald-600 rounded p-2.5 space-y-2">
                            <div class="flex flex-wrap items-center justify-between gap-2">
                                ...
                                <select id="select-metodo-vuelto" class="border-2 border-brand-black rounded px-2 py-1 text-xs font-bold bg-white focus:outline-none">
                                    ${e.map(B=>`
                                        <option value="${B.nombre}" ${B.nombre===t?"selected":""}>
                                            ${B.nombre} (${B.moneda==="USD"?"$ Dólares":"Bs. Bolívares"})
                                        </option>
                                    `).join("")}
                                </select>
                            </div>
                            ...
                            <div class="flex items-center justify-between pt-1 border-t border-gray-100 text-xs">
                                ...
                                <span class="font-heading font-black text-base text-brand-black">Bs. ${Y(A.vueltoBs)}</span>
                            </div>
                            `}
                        </div>
                        `:`
                        <div class="bg-white border border-gray-300 rounded p-2 text-xs text-gray-700">
                            ...
                            <p class="text-[11px] text-gray-600 leading-tight">
                                El excedente de <b>Bs. ${Y(A.vueltoBs)} ($ ${Y(A.vueltoUsd)})</b> no genera egreso físico de caja y se asienta en el comprobante como saldo a favor o ganancia por excedente del negocio.
                            </p>
                        </div>
                        `}
                                ...
                                <div class="flex flex-wrap gap-1.5 max-h-24 overflow-y-auto">
                                    ${e.map(B=>`
                                        <span class="inline-flex items-center gap-1 border border-brand-black rounded px-2 py-0.5 text-[11px] font-bold bg-white">
                                            <span>${B.nombre}</span>
                                            <span class="text-[9px] text-gray-500">(${B.moneda})</span>
                                            <button data-caja-eliminar-metodo="${B.nombre}" title="Eliminar método" class="text-red-600 hover:text-red-800 font-bold ml-0.5">✕</button>
                                        </span>
                                    `).join("")}
                                </div>
                            </div>
                ...
                </div>`,"max-w-xl"),g()},g=()=>{var R,F,C,f,m,S,M,B,_,x;const A=()=>{this.pagosBorrador=i,this.resolucionVueltoBorrador={estado:s,metodo:t,tasa:r},this.cerrarModal()};this.modal.querySelectorAll("[data-cerrar]").forEach(v=>v.addEventListener("click",A)),(R=this.modal.querySelector("[data-cancelar]"))==null||R.addEventListener("click",A),(F=this.modal.querySelector("#btn-volver-caja"))==null||F.addEventListener("click",()=>{this.pagosBorrador=i,this.resolucionVueltoBorrador={estado:s,metodo:t,tasa:r},this.cerrarModal();const v=document.getElementById("caja-buscar");v==null||v.focus()}),(C=this.modal.querySelector("#btn-vuelto-pagado"))==null||C.addEventListener("click",()=>{s="PAGADO",this.resolucionVueltoBorrador={estado:s,metodo:t,tasa:r},b()}),(f=this.modal.querySelector("#btn-vuelto-retenido"))==null||f.addEventListener("click",()=>{s="RETENIDO",this.resolucionVueltoBorrador={estado:s,metodo:t,tasa:r},b()}),(m=this.modal.querySelector("#select-metodo-vuelto"))==null||m.addEventListener("change",v=>{t=v.target.value,this.resolucionVueltoBorrador={estado:s,metodo:t,tasa:r},b()});const O=this.modal.querySelector("#input-tasa-vuelto");O&&(O.addEventListener("input",v=>{const T=Q(v.target.value);r=T>0?T:o,this.resolucionVueltoBorrador={estado:s,metodo:t,tasa:r}}),O.addEventListener("blur",()=>{b()})),(S=this.modal.querySelector("#btn-reset-tasa-vuelto"))==null||S.addEventListener("click",()=>{r=o,this.resolucionVueltoBorrador={estado:s,metodo:t,tasa:r},b()}),this.modal.querySelectorAll("select[data-pago-metodo]").forEach(v=>{v.addEventListener("change",T=>{const I=v.dataset.pagoMetodo,G=T.target.value,P=i.find(E=>E.id===I);if(P){const E=e.find(W=>W.nombre===G),K=(E==null?void 0:E.moneda)||"BS";P.moneda=K,P.tasaCambio=o,i.length===1?P.monto=P.moneda==="USD"?a:n:K!==P.moneda&&(K==="USD"?P.monto=Number((o>0?P.monto/o:0).toFixed(2)):P.monto=Number((P.monto*o).toFixed(2))),P.metodo=G,this.pagosBorrador=i,b()}})}),this.modal.querySelectorAll("input[data-pago-tasa]").forEach(v=>{v.addEventListener("input",T=>{const I=v.dataset.pagoTasa,G=Q(T.target.value),P=i.find(E=>E.id===I);if(P){P.tasaCambio=G>0?G:o,this.pagosBorrador=i;const E=c(),K=this.modal.querySelector("#btn-confirmar-cobro");K&&(K.disabled=!E.puedeConfirmar)}}),v.addEventListener("blur",()=>{b()})}),this.modal.querySelectorAll("button[data-reset-tasa]").forEach(v=>{v.addEventListener("click",()=>{const T=v.dataset.resetTasa,I=i.find(G=>G.id===T);I&&(I.tasaCambio=o,this.pagosBorrador=i,b())})}),this.modal.querySelectorAll("input[data-pago-monto]").forEach(v=>{v.addEventListener("input",T=>{const I=v.dataset.pagoMonto,G=Q(T.target.value),P=i.find(E=>E.id===I);if(P){P.monto=G,this.pagosBorrador=i;const E=c(),K=this.modal.querySelector("#btn-confirmar-cobro");K&&(K.disabled=!E.puedeConfirmar)}}),v.addEventListener("blur",()=>{b()})}),this.modal.querySelectorAll("input[data-pago-ref]").forEach(v=>{v.addEventListener("input",T=>{const I=v.dataset.pagoRef,G=T.target.value,P=i.find(E=>E.id===I);P&&(P.referencia=G,this.pagosBorrador=i)})}),this.modal.querySelectorAll("button[data-eliminar-pago]").forEach(v=>{v.addEventListener("click",()=>{const T=v.dataset.eliminarPago;i=i.filter(I=>I.id!==T),this.pagosBorrador=i,b()})}),(M=this.modal.querySelector("#btn-agregar-pago"))==null||M.addEventListener("click",()=>{let v=c();const T="p-"+Math.random().toString(36).slice(2,7),I=e.find(P=>!i.some(E=>E.metodo===P.nombre))||e[0]||{nombre:"BS.EFEC.",moneda:"BS"};if(v.faltanteBs<=.01&&i.length===1&&i[0]){const P=i[0],E=P.tasaCambio>0?P.tasaCambio:o,K=P.moneda==="USD"?P.monto*E:P.monto,W=Math.round(K/2*100)/100;P.monto=P.moneda==="USD"?E>0?Number((W/E).toFixed(2)):0:W,v=c()}const G=v.faltanteBs>0?v.faltanteBs:0;i.push({id:T,metodo:I.nombre,moneda:I.moneda,monto:I.moneda==="USD"?o>0?Number((G/o).toFixed(2)):0:G,tasaCambio:o,referencia:""}),this.pagosBorrador=i,b()}),(B=this.modal.querySelector("#btn-toggle-nuevo-metodo"))==null||B.addEventListener("click",()=>{d=!d,b()}),(_=this.modal.querySelector("#btn-guardar-nuevo-metodo"))==null||_.addEventListener("click",()=>{(async()=>{const v=this.modal.querySelector("#input-nuevo-metodo-nombre"),T=(v==null?void 0:v.value.trim())||"",I=this.modal.querySelector('input[name="radio-nuevo-metodo-moneda"][value="USD"]'),G=I!=null&&I.checked?"USD":"BS";if(T)try{e=await D.crearMetodoPago(T,G),d=!1,b()}catch{}})()}),this.modal.querySelectorAll("button[data-caja-eliminar-metodo]").forEach(v=>{v.addEventListener("click",()=>{(async()=>{const T=v.dataset.cajaEliminarMetodo||"";T&&window.confirm(`¿Seguro que deseas eliminar el método de pago "${T}"?`)&&(e=await D.eliminarMetodoPago(T),i.forEach(I=>{if(I.metodo===T){const G=e[0]||{nombre:"PUNTOD.VENTA",moneda:"BS"};I.metodo=G.nombre,I.moneda=G.moneda}}),this.pagosBorrador=i,b())})()})}),(x=this.modal.querySelector("#btn-confirmar-cobro"))==null||x.addEventListener("click",()=>{(async()=>{const v=this.modal.querySelector("#cobro-error");try{const T=i.filter($=>$.monto>0).map($=>{const q=$.moneda==="USD"&&$.tasaCambio>0?$.tasaCambio:o,V=$.moneda==="USD"?$.monto*q:$.monto,z=o>0?V/o:0;return{metodo:$.metodo,moneda:$.moneda,montoUsd:($.moneda==="USD"?$.monto:z).toFixed(2),montoBs:V.toFixed(2),tasaCambio:$.moneda==="USD"?q.toFixed(2):void 0,referencia:$.referencia.trim()||void 0}});if(T.length===0)throw new Error("Debe especificar al menos un pago válido.");const I=c(),G=T.reduce(($,q)=>$+Number(q.montoBs),0),P=e.find($=>$.nombre===t),E=(P==null?void 0:P.moneda)==="USD",K=E?r>0?r:o:void 0,W=E&&K?(I.vueltoBs/K).toFixed(2):I.vueltoUsd.toFixed(2),L={estado:I.vueltoBs>.009?s:"SIN_VUELTO",metodo:I.vueltoBs>.009&&s==="PAGADO"?t:void 0,montoBs:I.vueltoBs>.009?I.vueltoBs.toFixed(2):void 0,montoUsd:I.vueltoBs>.009?W:void 0,tasa:I.vueltoBs>.009&&s==="PAGADO"&&K?K.toFixed(2):void 0},j=await this.vm.cobrar(G.toFixed(2),T,L);this.pagosBorrador=null,this.resolucionVueltoBorrador=null,this.cerrarModal(),this.modalTicketExito(j,G)}catch(T){v&&(v.textContent=T instanceof Error?T.message.replace(/"/g,""):String(T),v.classList.remove("hidden"))}})()})};b()}modalTicketExito(e,a){const o=e.pagos&&e.pagos.length>0?e.pagos.map(l=>`
            ...
            <button data-cerrar class="w-full bg-brand-black text-white font-heading font-black text-lg py-3 rounded border-2 border-brand-black shadow-brutal-sm hover:bg-gray-800">
                LISTO
            </button>`,"max-w-md")}renderDinamico(){const e=this.contenedor.querySelector("#caja-grid");if(!e)return;const a=this.semaforoStock;e.innerHTML=this.vm.visibles.map(i=>{const s=(i.capacidades&ie)!==0,t=Number(i.stock),r=t<=0,d=i.sinStock?"text-purple-700":t<=a.rojoMax?"text-red-600":t<=a.amarilloMax?"text-amber-600":"text-emerald-700",c=i.unidad?i.unidad.toLowerCase()==="un"?"un.":i.unidad.toLowerCase():s?"kg":"un.",b=c==="kg"?"kg":c==="ml"?"ml":"un.";let g="";if(i.sinStock)g="LIBRE";else if(r)g="AGOTADO";else if(i.esCaja&&i.unidadesPorCaja&&i.unidadesPorCaja>1){const h=Math.floor(t/i.unidadesPorCaja),y=t%i.unidadesPorCaja;g=`${h} cajas y ${y} un. (${t} un.)`}else g=`${t} ${c}`;return`
                <button data-sku="${i.sku}" ${r&&!i.sinStock?"disabled":""}
            ...
            </tr>`}).join("")}async render(){var C;const[e,a,o,n,l,i,s]=await Promise.all([D.panel(),D.respaldos(),D.ventas(),D.productos(),D.categorias(),D.cuentas(),D.obtenerJornadaActual()]);this.vm.setDatos(e),this.respaldos=a,this.transacciones=o,this.productos=n,this.categorias=l,this.cuentas=i,this.jornadaActual=s;const t=new Date,r=new Date(t.getTime()-864e5),d=f=>f.toLocaleString("es-VE",{year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit"});if(!this.rangoDetalleFechas){this.rangoDescripcion="Últimas 24 Horas",this.rangoDetalleFechas=`Del ${d(r)} al ${d(t)}`;const f=Math.floor(r.getTime()/1e3),m=this.transacciones.filter(S=>this.obtenerTimestampTicket(S)>=f);this.transaccionesFiltradasRango=m.length>0?m:this.transacciones.slice(0,15)}const c=this.modelo.bs(e.valorInventarioUsd),b=this.modelo.tieneCapacidad(8)||this.modelo.tieneRubro(4),g=e.dineroPorCategoria&&e.dineroPorCategoria.length>0?e.dineroPorCategoria:this.calcularDineroPorCategoria(),h=g.reduce((f,m)=>f+m.cantidadProductos,0),y=g.reduce((f,m)=>f+m.unidadesStock,0),k=g.reduce((f,m)=>f+te(m.dineroBrutoUsd),0),U=g.reduce((f,m)=>f+te(m.dineroBrutoBs),0),A=g.reduce((f,m)=>f+te(m.dineroVentaUsd),0),O=Math.max(0,A-k),R=A>0?(O/A*100).toFixed(1):"0.0",F=g.map(f=>{const m=te(f.porcentajeCapital);return`
                ...
                <td class="py-2.5 px-3 text-right">
                    <div class="flex items-center justify-end gap-1.5">
                        <div class="w-16 bg-gray-200 border border-brand-black rounded-full h-2 overflow-hidden shrink-0">
                            <div class="bg-brand-purple h-full" style="width: ${Math.min(100,m)}%"></div>
                        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-4 sm:mb-6">
            ...
            ${this.kpi("Inventario Total",`$ ${J(e.valorInventarioUsd)}`,`<span class="break-words" title="Bs. ${c}">Bs. ${se(c)}</span>`,"bg-purple-50","Valoración monetaria total del stock físico actual disponible")}
        </div>
        ...
        <div class="bg-white border-2 border-brand-black rounded-lg shadow-brutal p-4 sm:p-6 mb-4 sm:mb-6">
            <div class="flex flex-wrap justify-between items-center border-b-2 border-brand-black pb-3 mb-4 gap-2">
                <div>
                    <div class="flex items-center gap-2">
                        <span class="w-3 h-3 rounded-full ${((C=this.jornadaActual)==null?void 0:C.estado)==="abierta"?"bg-emerald-500 animate-pulse":"bg-gray-400"} border border-brand-black inline-block"></span>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
            ...
            <div class="bg-white border-2 border-brand-black rounded-lg shadow-brutal p-4 sm:p-6 flex flex-col justify-between">
                ...
                <div class="mt-4 pt-3 border-t border-brand-black">
        ...
        </div>`,this.conectarEventos(e),this.pintarGrafico(e)}conectarEventos(e){var g,h,y,k,U,A,O,R,F,C;const a=document.getElementById("btn-crit-vol"),o=document.getElementById("btn-crit-ing"),n=document.getElementById("filtro-fecha-desde"),l=document.getElementById("filtro-fecha-hasta"),i=document.getElementById("btn-aplicar-fechas"),s=document.getElementById("btn-limpiar-fechas"),t=document.getElementById("label-estado-fechas"),r=document.getElementById("tabla-transacciones-body"),d=document.getElementById("conteo-transacciones-badge"),c=this.contenedor.querySelectorAll("[data-rango]");c.forEach(f=>{f.addEventListener("click",()=>{n&&(n.value=""),l&&(l.value=""),t&&t.classList.add("hidden"),r&&(r.innerHTML=this.renderFilasTransacciones(this.transacciones)),d&&(d.textContent=`${this.transacciones.length} transacciones registradas`),c.forEach(S=>{S.className="w-12 sm:w-14 h-8 flex items-center justify-center font-heading font-bold text-xs rounded text-brand-black hover:bg-gray-100 transition-colors shrink-0"}),f.className="w-12 sm:w-14 h-8 flex items-center justify-center font-heading font-black text-xs rounded bg-brand-black text-white shrink-0";const m=f.dataset.rango||"24h";this.aplicarRango(m,e)})}),i==null||i.addEventListener("click",()=>{this.aplicarFiltroFechas(e)}),s==null||s.addEventListener("click",()=>{var m,S;n&&(n.value=""),l&&(l.value=""),t&&t.classList.add("hidden"),r&&(r.innerHTML=this.renderFilasTransacciones(this.transacciones)),d&&(d.textContent=`${this.transacciones.length} transacciones registradas`),this.paginaTrx=1,(m=document.getElementById("trx-pag-ant"))==null||m.addEventListener("click",()=>{this.paginaTrx>1&&(this.paginaTrx--,r&&(r.innerHTML=this.renderFilasTransacciones(this.transacciones)))}),(S=document.getElementById("trx-pag-sig"))==null||S.addEventListener("click",()=>{const M=Math.ceil(this.transacciones.length/this.POR_PAGINA_TRX);this.paginaTrx<M&&(this.paginaTrx++,r&&(r.innerHTML=this.renderFilasTransacciones(this.transacciones)))}),c.forEach(M=>{M.className="w-12 sm:w-14 h-8 flex items-center justify-center font-heading font-bold text-xs rounded text-brand-black hover:bg-gray-100 transition-colors shrink-0"});const f=this.contenedor.querySelector('[data-rango="24h"]');f&&(f.className="w-12 sm:w-14 h-8 flex items-center justify-center font-heading font-black text-xs rounded bg-brand-black text-white shrink-0"),this.aplicarRango("24h",e)}),a==null||a.addEventListener("click",()=>{this.criterioGrafica="volumen",a.className="px-2.5 py-1 text-xs font-heading font-black rounded bg-brand-black text-white",o.className="px-2.5 py-1 text-xs font-heading font-bold rounded text-brand-black hover:bg-white",this.pintarGrafico(this.vm.panel||e)}),o==null||o.addEventListener("click",()=>{this.criterioGrafica="ingreso",o.className="px-2.5 py-1 text-xs font-heading font-black rounded bg-brand-black text-white",a.className="px-2.5 py-1 text-xs font-heading font-bold rounded text-brand-black hover:bg-white",this.pintarGrafico(this.vm.panel||e)}),(g=document.getElementById("trx-pag-ant"))==null||g.addEventListener("click",()=>{this.paginaTrx>1&&(this.paginaTrx--,r&&(r.innerHTML=this.renderFilasTransacciones(this.transacciones)))}),(h=document.getElementById("trx-pag-sig"))==null||h.addEventListener("click",()=>{const f=Math.ceil(this.transacciones.length/this.POR_PAGINA_TRX);this.paginaTrx<f&&(this.paginaTrx++,r&&(r.innerHTML=this.renderFilasTransacciones(this.transacciones)))}),(y=document.getElementById("btn-crear-backup"))==null||y.addEventListener("click",async()=>{await D.crearRespaldo(),await this.render()}),(k=document.getElementById("btn-exportar-pdf"))==null||k.addEventListener("click",()=>{this.exportarResumenPdf(this.vm.panel||e)});const b=async()=>{var L,j;const f=await D.obtenerJornadaActual(),m=await D.listarOperadores(),S=await D.obtenerSemaforoStock(),M=document.getElementById("jornada-estado-texto"),B=document.getElementById("jornada-fechas-texto"),_=document.getElementById("jornada-operador-activo"),x=document.getElementById("jornada-balance-texto"),v=document.getElementById("jornada-tickets-texto"),T=document.getElementById("box-acciones-jornada"),I=document.getElementById("lista-operadores-chips"),G=(f==null?void 0:f.estado)==="abierta";M&&(M.textContent=G?"Jornada en Curso":"Jornada Cerrada"),B&&(B.textContent=G?`Inicio: ${(f==null?void 0:f.inicioStr)||"Reciente"}`:`Último cierre: ${(f==null?void 0:f.finStr)||"Sin jornadas"}`);const P=f!=null&&f.operadoresActivos&&f.operadoresActivos.length>0?f.operadoresActivos.join(", "):(f==null?void 0:f.operadorActual)||"Sin operador asignado";_&&(_.textContent=P),x&&(x.textContent=`$ ${J((f==null?void 0:f.ventasTotalUsd)||0)} USD (Bs. ${J((f==null?void 0:f.ventasTotalBs)||0)})`),v&&(v.textContent=`${(f==null?void 0:f.ticketsEmitidos)||0} tickets emitidos · ${(f==null?void 0:f.entradasStockReg)||0} reposiciones`),T&&(G?T.innerHTML=`
                    ...
                    `,(L=T.querySelector("#btn-abrir-jornada"))==null||L.addEventListener("click",async()=>{var V;const $=(await D.listarOperadores()).filter(z=>z.activo),q=((V=$[0])==null?void 0:V.nombre)||"Cajero Principal";await D.abrirJornada(q,$.map(z=>z.nombre)),b()}),(j=T.querySelector("#btn-cerrar-jornada"))==null||j.addEventListener("click",async()=>{if(window.confirm("¿Confirmas el cierre de la jornada operativa actual? Se generará el balance consolidado del turno.")){const q=await D.cerrarJornada();alert(`Jornada cerrada con éxito.
Identificador: ${q.id}
Total Recaudado: $${q.ventasTotalUsd} USD (Bs. ${q.ventasTotalBs})`),b()}})),I&&(I.innerHTML=m.map($=>{var V,z;return`
                    <div class="inline-flex items-center gap-1.5 border-2 border-brand-black rounded px-2 py-1 text-xs font-bold ${((V=f==null?void 0:f.operadoresActivos)==null?void 0:V.includes($.nombre))||((z=f==null?void 0:f.operadorActual)==null?void 0:z.includes($.nombre))?"bg-purple-100 text-brand-purple border-brand-purple":"bg-gray-100 text-brand-black"}">
                        <span>${$.nombre}</span>
                ...
                `}).join(""),I.querySelectorAll("button[data-op-edit]").forEach($=>{$.addEventListener("click",async()=>{const q=$.dataset.opEdit||"",V=$.dataset.opNom||"",z=window.prompt("Editar nombre del operador:",V);z&&z.trim()&&z.trim()!==V&&(await D.editarOperador(q,z.trim()),b())})}),I.querySelectorAll("button[data-op-del]").forEach($=>{$.addEventListener("click",async()=>{const q=$.dataset.opDel||"",V=$.dataset.opNom||"";window.confirm(`¿Seguro que deseas eliminar al operador "${V}" del sistema?`)&&(await D.eliminarOperador(q),b())})}));const E=document.getElementById("lista-metodos-pago-panel");if(E){const $=await D.listarMetodosPago();E.innerHTML=$.map(q=>`
                ...
                `).join(""),E.querySelectorAll("button[data-metodo-del]").forEach(q=>{q.addEventListener("click",async()=>{const V=q.dataset.metodoDel||"";window.confirm(`¿Confirmas eliminar el método de pago "${V}"?`)&&(await D.eliminarMetodoPago(V),b())})})}const K=document.getElementById("in-semaforo-rojo"),W=document.getElementById("in-semaforo-amarillo");K&&!K.dataset.modificado&&(K.value=String(S.rojoMax)),W&&!W.dataset.modificado&&(W.value=String(S.amarilloMax))};b(),(U=document.getElementById("btn-relevar-operador"))==null||U.addEventListener("click",async()=>{var _;const f=(await D.listarOperadores()).filter(x=>x.activo);if(f.length===0){alert("No hay operadores activos disponibles. Registre uno a continuación.");return}const m=await D.obtenerJornadaActual(),S=(m==null?void 0:m.operadoresActivos)||[(m==null?void 0:m.operadorActual)||((_=f[0])==null?void 0:_.nombre)||""],M=f.map(x=>x.nombre).join(", "),B=window.prompt(`Indique el o los operadores en turno (separados por coma si son varios).
Disponibles: ${M}`,S.join(", "));if(B!==null){const x=B.split(",").map(v=>v.trim()).filter(v=>f.some(T=>T.nombre.toLowerCase()===v.toLowerCase()));x.length>0?(await D.asignarOperadoresTurno(x),b()):B.trim()&&alert("Ninguno de los nombres coincide con operadores registrados.")}}),(A=document.getElementById("form-crear-metodo-panel"))==null||A.addEventListener("submit",async f=>{f.preventDefault();const m=document.getElementById("in-nuevo-metodo-nombre"),S=document.getElementById("select-nuevo-metodo-moneda"),M=(m==null?void 0:m.value.trim())||"",B=(S==null?void 0:S.value)==="USD"?"USD":"BS";M&&(await D.crearMetodoPago(M,B),m&&(m.value=""),b())}),(O=document.getElementById("form-crear-operador"))==null||O.addEventListener("submit",async f=>{f.preventDefault();const m=document.getElementById("in-nuevo-operador"),S=(m==null?void 0:m.value.trim())||"";S&&(await D.crearOperador(S),m&&(m.value=""),b())}),(R=document.getElementById("form-semaforo-stock"))==null||R.addEventListener("submit",async f=>{f.preventDefault();const m=document.getElementById("in-semaforo-rojo"),S=document.getElementById("in-semaforo-amarillo"),M=document.getElementById("msg-semaforo-stock"),B=parseInt((m==null?void 0:m.value)||"5",10),_=parseInt((S==null?void 0:S.value)||"15",10);if(B>=_){alert("El umbral rojo debe ser estrictamente menor que el umbral amarillo.");return}await D.guardarSemaforoStock(B,_),M&&(M.textContent="Umbrales del semáforo guardados y aplicados a todo el sistema.",M.classList.remove("hidden"),setTimeout(()=>M.classList.add("hidden"),2500))}),(F=document.getElementById("btn-reset-semaforo"))==null||F.addEventListener("click",async()=>{await D.guardarSemaforoStock(5,15),b()}),(C=document.getElementById("form-cambiar-clave"))==null||C.addEventListener("submit",async f=>{f.preventDefault();const m=document.getElementById("pin-actual"),S=document.getElementById("pin-nuevo"),M=document.getElementById("msg-cambio-clave");if(M)try{await D.cambiarPinDueno((m==null?void 0:m.value.trim())||"",(S==null?void 0:S.value.trim())||""),await this.modelo.cargarConfig(),M.textContent=S!=null&&S.value.trim()?"Clave actualizada exitosamente. El panel requerirá esta nueva clave para ingresar.":"Clave eliminada. El panel ha quedado en modo abierto sin clave.",M.style.color="#00823B",M.classList.remove("hidden"),setTimeout(()=>void this.render(),1200)}catch(B){M.textContent=B instanceof Error?B.message:String(B),M.style.color="#C60C15",M.classList.remove("hidden")}})}aplicarFiltroFechas(e){const a=document.getElementById("filtro-fecha-desde"),o=document.getElementById("filtro-fecha-hasta"),n=document.getElementById("label-estado-fechas"),l=document.getElementById("tabla-transacciones-body"),i=document.getElementById("conteo-transacciones-badge"),s=(a==null?void 0:a.value.trim())||"",t=(o==null?void 0:o.value.trim())||"";if(!s&&!t){n&&(n.textContent="Selecciona una fecha Desde o Hasta para iniciar el diagnóstico.",n.classList.remove("hidden"));return}let r=0,d=Number.MAX_SAFE_INTEGER;if(s){const[P,E,K]=s.split("-").map(Number);r=Math.floor(new Date(P,E-1,K,0,0,0,0).getTime()/1e3)}if(t){const[P,E,K]=t.split("-").map(Number);d=Math.floor(new Date(P,E-1,K,23,59,59,999).getTime()/1e3)}if(r>d){n&&(n.textContent='Rango inválido: la fecha "Desde" es posterior a la fecha "Hasta".',n.classList.remove("hidden"));return}const c=this.transacciones.filter(P=>{const E=this.obtenerTimestampTicket(P);return E>=r&&E<=d}),b=c.reduce((P,E)=>P+te(E.totalUsd),0),g=c.reduce((P,E)=>P+te(E.totalBs),0),h=c.length,y=b*.35,k=b*.23,U=k*(this.modelo.tasaActual||807.39),A=h>0?(b/h).toFixed(2):"0.00",O=document.getElementById("kpi-ventas-usd"),R=document.getElementById("kpi-ventas-bs"),F=document.getElementById("kpi-tickets"),C=document.getElementById("kpi-gan-bruta"),f=document.getElementById("kpi-gan-neta"),m=document.getElementById("kpi-gan-neta-bs"),S=document.getElementById("kpi-ticket-prom");O&&(O.textContent="$ "+J(b)),R&&(R.textContent="Bs. "+se(g),R.title="Bs. "+J(g)),F&&(F.textContent=String(h)),C&&(C.textContent="$ "+J(y)),f&&(f.textContent="$ "+J(k)),m&&(m.textContent="Bs. "+se(U),m.title="Bs. "+J(U)),S&&(S.textContent="$ "+A),l&&(l.innerHTML=this.renderFilasTransacciones(c)),i&&(i.textContent=`${c.length} transacciones en rango`),this.contenedor.querySelectorAll("[data-rango]").forEach(P=>{P.className="w-12 sm:w-14 h-8 flex items-center justify-center font-heading font-bold text-xs rounded text-brand-black hover:bg-gray-100 transition-colors shrink-0"});const B={};c.forEach(P=>{(P.lineas||[]).forEach(E=>{B[E.nombre]||(B[E.nombre]={nombre:E.nombre,cantidad:0,totalUsd:0}),B[E.nombre].cantidad+=Number(E.cantidad)||0,B[E.nombre].totalUsd+=te(E.subtotalUsd)||Number(E.cantidad)*te(E.precioUsd)})});const _=Object.values(B).reduce((P,E)=>P+E.totalUsd,0)||1,x=Object.values(B).sort((P,E)=>E.totalUsd-P.totalUsd).map(P=>({nombre:P.nombre,cantidad:String(P.cantidad),totalUsd:P.totalUsd.toFixed(2),porcentaje:(P.totalUsd/_*100).toFixed(1)})),v={...e,ventas24hUsd:b.toFixed(2),ventas24hBs:g.toFixed(2),gananciaBrutaUsd:y.toFixed(2),gananciaNetaUsd:k.toFixed(2),tickets24h:h,topProductos:x.length>0?x:c.length===0?[]:e.topProductos};this.vm.setDatos(v),this.pintarGrafico(v);const T=P=>P.toLocaleString("es-VE",{year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit"}),I=s?new Date(r*1e3):new Date(2020,0,1),G=t?new Date(d*1e3):new Date;if(this.rangoDescripcion="Diagnóstico Personalizado",this.rangoDetalleFechas=`Del ${T(I)} al ${T(G)}`,this.transaccionesFiltradasRango=c,n){const P=s||"Inicio",E=t||"Actualidad";n.textContent=`Diagnóstico activo (${P} al ${E}): ${c.length} transacciones registradas`,n.classList.remove("hidden")}}aplicarRango(e,a){const o=new Date,n=C=>C.toLocaleString("es-VE",{year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit"});let l=864e5,i="Últimas 24 Horas",s=1;e==="7d"?(s=4.2,l=7*864e5,i="Últimos 7 Días"):e==="30d"?(s=14.5,l=30*864e5,i="Últimos 30 Días"):e==="1a"?(s=85,l=365*864e5,i="Último Año (1A)"):e==="todo"&&(s=120,l=5*365*864e5,i="Histórico Consolidado (TODO)");const t=new Date(o.getTime()-l);this.rangoDescripcion=i,this.rangoDetalleFechas=`Del ${n(t)} al ${n(o)}`;const r=Math.floor(t.getTime()/1e3),d=this.transacciones.filter(C=>this.obtenerTimestampTicket(C)>=r);this.transaccionesFiltradasRango=d.length>0?d:this.transacciones.slice(0,15);const c=Number(a.ventas24hUsd)*s,b=Number(a.ventas24hBs)*s,g=Math.round(a.tickets24h*s),h=document.getElementById("kpi-ventas-usd"),y=document.getElementById("kpi-ventas-bs"),k=document.getElementById("kpi-tickets"),U=document.getElementById("kpi-gan-bruta"),A=document.getElementById("kpi-gan-neta"),O=document.getElementById("kpi-gan-neta-bs"),R=document.getElementById("kpi-ticket-prom");if(h&&(h.textContent="$ "+J(c)),y&&(y.textContent="Bs. "+se(b),y.title="Bs. "+J(b)),k&&(k.textContent=String(g)),U&&(U.textContent="$ "+J(c*.35)),A&&(A.textContent="$ "+J(c*.23)),O){const C=c*.23*(this.modelo.tasaActual||807.39);O.textContent="Bs. "+se(C),O.title="Bs. "+J(C)}R&&(R.textContent="$ "+(g>0?(c/g).toFixed(2):"0.00"));const F={...a,topProductos:a.topProductos.map(C=>({nombre:C.nombre,cantidad:String(Math.round(Number(C.cantidad)*s)),totalUsd:C.totalUsd?String(Math.round(Number(C.totalUsd)*s)):void 0,porcentaje:C.porcentaje}))};this.vm.setDatos(F),this.pintarGrafico(F)}kpi(e,a,o,n,l=""){return`
        <div class="border-2 border-brand-black rounded-lg shadow-brutal-sm p-4 ${n} flex flex-col justify-between min-w-0 overflow-hidden" title="${l}">
            ...
            ${l?`<div class="text-[10px] text-gray-500 font-bold truncate mt-0.5 border-t border-gray-200/60 pt-1">${l}</div>`:""}
        </div>`}async pintarGrafico(e){const a=this.contenedor.querySelector("#panel-chart-container");if(!a)return;if(e.topProductos.length===0){a.innerHTML='<p class="font-body text-brand-text py-10 text-center text-sm">Aún no hay ventas registradas en este período.</p>';return}const o=["#2563EB","#D97706","#10B981","#8B5CF6","#EC4899","#06B6D4","#F97316","#84CC16","#6366F1","#14B8A6","#F43F5E","#A855F7","#EAB308","#64748B"],n=this.criterioGrafica==="volumen",l=e.topProductos.map(y=>Number(n?y.cantidad:y.totalUsd||Number(y.cantidad)*8.5)),i=l.reduce((y,k)=>y+k,0)||1,s=e.topProductos.map((y,k)=>({nombre:y.nombre,valor:l[k],porcentaje:(l[k]/i*100).toFixed(1)}));let t=0;const r=95,d=95,c=85;let b="";s.length===1?b=`<circle cx="${r}" cy="${d}" r="${c}" fill="${o[0]}" stroke="#1E232A" stroke-width="2" />`:s.forEach((y,k)=>{const A=y.valor/i*2*Math.PI,O=t,R=t+A,F=r+c*Math.cos(O),C=d+c*Math.sin(O),f=r+c*Math.cos(R),m=d+c*Math.sin(R),S=A>Math.PI?1:0,M=o[k%o.length];b+=`
                    <path d="M ${r} ${d} L ${F} ${C} A ${c} ${c} 0 ${S} 1 ${f} ${m} Z"
                ...
                `,t+=A});const g=s.map((y,k)=>{const U=o[k%o.length],A=n?`${y.valor} un.`:`$ ${J(y.valor)}`;return`
                <tr class="border-b border-gray-100 last:border-none">
        ...
        <div class="flex flex-col md:flex-row items-center justify-around gap-4 sm:gap-6 w-full h-full py-2 min-w-0">
        ...
        </div>`}exportarResumenPdf(e){var _;const a=((_=this.modelo.getConfig())==null?void 0:_.nombre)||"DatioLabs Enterprise",o=new Date().toLocaleString("es-VE",{year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit",second:"2-digit"}),n=this.modelo.tasaActual||807.39,l=e.gananciaBrutaUsd||Number(e.ventas24hUsd)*.35,i=e.gananciaNetaUsd||Number(e.ventas24hUsd)*.23,s=Number(e.ventas24hUsd)*.23*n,t=e.tickets24h>0?(Number(e.ventas24hUsd)/e.tickets24h).toFixed(2):"0.00",r=e.dineroPorCategoria&&e.dineroPorCategoria.length>0?e.dineroPorCategoria:this.calcularDineroPorCategoria(),d=r.reduce((x,v)=>x+v.cantidadProductos,0),c=r.reduce((x,v)=>x+v.unidadesStock,0),b=r.reduce((x,v)=>x+te(v.dineroBrutoUsd),0),g=r.reduce((x,v)=>x+te(v.dineroBrutoBs),0),h=r.reduce((x,v)=>x+te(v.dineroVentaUsd),0),y=Math.max(0,h-b),k=h>0?(y/h*100).toFixed(1):"0.0",U=this.cuentas.filter(x=>x.tipo==="deuda");let A=0,O=0,R=0;U.forEach(x=>{const v=te(x.totalParcialUsd),T=te(x.abonosUsd),I=Math.max(0,v-T);A+=v,O+=T,R+=I});const F=R*n,C=U.map((x,v)=>{const T=te(x.totalParcialUsd),I=te(x.abonosUsd),G=Math.max(0,T-I),P=G*n,E=x.cliente||x.etiqueta,K=x.nota||"-",W=x.fechaCreacionStr||"Registrada";return`
            <tr style="border-bottom: 1px solid #ddd; ${v%2===0?"background-color: #fafafa;":""}">
                ...
                <td style="padding: 6px 10px; text-align: right; color: #15803d; font-weight: bold;">$ ${J(I)}</td>
        ...
        `).join(""),m=e.topProductos.map((x,v)=>{const T=x.totalUsd?Number(x.totalUsd):0,I=T*n;return`
            <tr style="border-bottom: 1px solid #ddd; ${v%2===0?"background-color: #fafafa;":""}">
                ...
                <td style="padding: 7px 10px; text-align: right; font-weight: bold; color: #1E232A;">Bs. ${I.toLocaleString("es-VE",{minimumFractionDigits:2,maximumFractionDigits:2})}</td>
                <td style="padding: 7px 10px; text-align: right; color: #555;">${x.porcentaje}%</td>
            </tr>`}).join(""),S=this.transaccionesFiltradasRango.length>0?this.transaccionesFiltradasRango:this.transacciones,M=S.slice(0,30).map((x,v)=>{const T=x.tasaDelDia?Number(x.tasaDelDia).toFixed(2):n.toFixed(2);let I="PAGO DIRECTO";return x.pagos&&x.pagos.length>1?I=`MIXTO (${x.pagos.length})`:x.pagos&&x.pagos.length===1&&(I=x.pagos[0].metodo),`
            <tr style="border-bottom: 1px solid #ddd; ${v%2===0?"background-color: #fafafa;":""}">
                <td style="padding: 6px 10px; font-family: monospace; font-weight: bold; color: #1E232A;">${x.ventaId}</td>
                ...
                <td style="padding: 6px 10px; font-weight: bold; font-size: 10px; color: #222;">${I}</td>
            ...
            `}).join(""),B=window.open("","_blank","width=850,height=950");if(!B){alert("Por favor autoriza las ventanas emergentes para generar el documento PDF.");return}B.document.write(`
        <!DOCTYPE html>
        <html lang="es">
        <head>
            <meta charset="utf-8">
        ...
        <body>
            ...
            <table>
                ...
                <tbody>
                    ${m||'<tr><td colspan="5" style="text-align: center; padding: 10px;">Sin registros en el período.</td></tr>'}
                </tbody>
            </table>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-4 sm:mb-6">
            ...
            <div class="bg-white border-2 border-brand-black rounded-lg p-3 sm:p-4 shadow-brutal-sm">
                ...
                <p class="font-heading font-black text-2xl sm:text-3xl text-emerald-700 mt-1">$ ${N(r)}</p>
                <p class="text-xs font-bold text-gray-500">Bs. ${N(r*l)}</p>
            </div>
            ...
            <div class="bg-white border-2 border-brand-black rounded-lg p-3 sm:p-4 shadow-brutal-sm">
                ...
                <p class="font-heading font-black text-xl sm:text-2xl text-brand-black mt-1">Bs. ${N(l)}</p>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6 items-start">
            ...
            <div class="space-y-3">
                ...
                <div id="cuentas-lista" class="space-y-2 max-h-[65vh] overflow-y-auto pr-1">
                    ${n.length===0?`<div class="bg-white border-2 border-brand-black rounded-lg p-6 text-center text-brand-text font-bold">
                        ...
                        </div>`:(()=>{const d=Math.ceil(n.length/this.POR_PAGINA_CUENTAS);this.paginaCuentas>d&&(this.paginaCuentas=d),this.paginaCuentas<1&&(this.paginaCuentas=1);const c=(this.paginaCuentas-1)*this.POR_PAGINA_CUENTAS;return n.slice(c,c+this.POR_PAGINA_CUENTAS).map(g=>{var C;const h=g.tipo==="deuda",y=Z(g.totalParcialUsd),k=Z(g.abonosUsd),U=Math.max(0,y-k),A=Math.max(0,k-y),O=((C=this.cuentaSeleccionada)==null?void 0:C.ventaId)===g.ventaId,R=Ee(g.fechaCreacionUnix);let F="cursor-pointer border-2 border-brand-black rounded-lg p-3 sm:p-4 transition-all ";return O?F+=h?"bg-amber-50 text-brand-black border-l-[6px] border-l-amber-600 shadow-brutal ring-1 ring-amber-500/20":"bg-blue-50 text-brand-black border-l-[6px] border-l-blue-600 shadow-brutal ring-1 ring-blue-500/20":F+=h?"bg-stone-50 border-l-4 border-l-amber-300 text-brand-black shadow-brutal-sm hover:bg-amber-50/60 hover:-translate-y-0.5":"bg-stone-50 border-l-4 border-l-blue-200 text-brand-black shadow-brutal-sm hover:bg-blue-50/50 hover:-translate-y-0.5",`
                        <div data-cuenta-id="${g.ventaId}" class="${F}">
                            <div class="flex justify-between items-start gap-2">
                                <div>
                                    ...
                                    <p class="text-[11px] font-mono font-bold text-gray-500 mt-0.5">
                                        ${h?`${R} · `:""}${g.ventaId.slice(0,9)}
                                    </p>
                                    ${g.nota?`<p class="text-[11px] italic font-semibold text-gray-600 mt-0.5 line-clamp-1">Nota: ${g.nota}</p>`:""}
                                </div>
                                <div class="text-right shrink-0">
                                    <p class="font-heading font-black text-base sm:text-lg text-brand-black">$${N(g.totalParcialUsd)}</p>
                            ...
                            <div class="flex justify-between items-center mt-2 pt-2 border-t border-gray-200 text-[11px] font-bold text-gray-600">
                                ...
                                <span>Abonado: <strong class="text-brand-black">$${N(g.abonosUsd)}</strong></span>
                            </div>
                        </div>`}).join("")})()}
                </div>
            ...
            <div class="lg:col-span-2">
                ${this.cuentaSeleccionada?(()=>{var k;const d=this.cuentaSeleccionada.tipo==="deuda",c=Z(this.cuentaSeleccionada.totalParcialUsd),b=Z(this.cuentaSeleccionada.abonosUsd),g=Math.max(0,c-b),h=Math.max(0,b-c),y=Ee(this.cuentaSeleccionada.fechaCreacionUnix);return`
                        ...
                        <div class="text-right">
                            ${h>0?`
                                ...
                                <div class="flex items-center justify-end gap-1.5 mt-1">
                                    <span class="font-bold text-[10px] text-gray-500">
                                        Consumo: $${N(c)} · Abonado: <strong class="text-brand-black">$${N(b)}</strong>
                                    </span>
                            ...
                            `:`
                                ...
                                <p class="text-xs font-bold text-gray-600">
                                    Bs. ${N(g*l)} <span class="text-[10px] text-gray-400">(${d?"tasa hoy":"actual"})</span>
                                </p>
                                <div class="flex items-center justify-end gap-1.5 mt-1">
                                    <span class="font-bold text-xs text-gray-600">
                                        Total: $${N(c)} · Abonado: <strong class="text-brand-black">$${N(b)}</strong>
                                    </span>
                                    <button data-editar-abono title="Corregir monto abonado por error" class="text-[10px] font-black uppercase px-2 py-0.5 rounded border border-brand-black bg-amber-200 hover:bg-amber-300 text-brand-black">
                                        Editar
                                    </button>
                            ...
                            ${!this.cuentaSeleccionada.consumos||this.cuentaSeleccionada.consumos.length===0?`<div class="py-6 text-center text-xs text-gray-400 font-bold">
                                ...
                                <div class="flex items-center justify-between border-2 border-brand-black rounded-lg bg-white px-3 py-2 text-xs font-bold shadow-brutal-sm hover:bg-amber-50/50 transition-colors gap-2">
                                    <div class="flex-1 min-w-0 pr-2">
                                        ...
                                        <p class="text-[11px] text-gray-500 font-semibold">${U.cantidad} un. &times; $${N(U.precioUsd)} <span class="text-gray-400 font-normal">· Bs. ${this.modelo.bs(Z(U.precioUsd))} c/u</span></p>
                                    </div>
        ...
        </div>`,this.vincularEventos()}vincularEventos(){var o,n,l,i,s,t,r,d,c,b;(o=document.getElementById("btn-nueva-cuenta"))==null||o.addEventListener("click",()=>this.modalNuevaCuenta("activa")),(n=document.getElementById("btn-nueva-deuda"))==null||n.addEventListener("click",()=>this.modalNuevaCuenta("deuda")),(l=document.getElementById("tab-todas"))==null||l.addEventListener("click",()=>{this.filtroTab="todas",this.paginaCuentas=1,this.render()}),(i=document.getElementById("tab-activas"))==null||i.addEventListener("click",()=>{this.filtroTab="activa",this.paginaCuentas=1,this.render()}),(s=document.getElementById("tab-deudas"))==null||s.addEventListener("click",()=>{this.filtroTab="deuda",this.paginaCuentas=1,this.render()}),(t=document.getElementById("cta-pag-ant"))==null||t.addEventListener("click",()=>{this.paginaCuentas>1&&(this.paginaCuentas--,this.render())}),(r=document.getElementById("cta-pag-sig"))==null||r.addEventListener("click",()=>{this.paginaCuentas++,this.render()}),this.contenedor.querySelectorAll("[data-cuenta-id]").forEach(g=>{g.addEventListener("click",()=>{const h=g.dataset.cuentaId;this.cuentaSeleccionada=this.cuentas.find(y=>y.ventaId===h)||null,this.render()})}),this.contenedor.querySelectorAll("[data-del-consumo]").forEach(g=>{g.addEventListener("click",async()=>{if(!this.cuentaSeleccionada)return;const h=g.dataset.delConsumo||"";try{const y=await D.eliminarConsumo(this.cuentaSeleccionada.ventaId,h);this.cuentaSeleccionada=y,this.render()}catch(y){alert(y instanceof Error?y.message:String(y))}})});const e=this.contenedor.querySelector("#cta-buscar-prod");e&&fe(e,40),e==null||e.addEventListener("input",()=>{const g=e.value.trim().toLowerCase(),h=this.contenedor.querySelector("#cta-grid-prod");if(h){const y=this.productos.filter(k=>!g||k.nombre.toLowerCase().includes(g));y.length===0?h.innerHTML='<p class="col-span-full text-center text-xs text-gray-400 py-4 font-bold">No se encontraron productos coincidentes.</p>':(h.innerHTML=y.map(k=>{const U=!k.sinStock&&Number(k.stock)<=0;return`
            ...
            </div>`;const l=this.modal.querySelector("#modal-tab-activa"),i=this.modal.querySelector("#modal-tab-deuda");l==null||l.addEventListener("click",()=>{a="activa",o()}),i==null||i.addEventListener("click",()=>{a="deuda",o()});const s=this.modal.querySelector("#modal-cta-nombre"),t=this.modal.querySelector("#modal-cta-nota");s&&fe(s,40),t&&fe(t,60),setTimeout(()=>s==null?void 0:s.focus(),50);const r=()=>{this.modal.innerHTML=""};(d=this.modal.querySelector("#modal-cta-cancel"))==null||d.addEventListener("click",r),(c=this.modal.querySelector("#modal-cta-ok"))==null||c.addEventListener("click",async()=>{const b=s==null?void 0:s.value.trim().slice(0,40),g=t==null?void 0:t.value.trim().slice(0,60);if(b){const h=await D.abrirCuenta(b,a,g,b);this.cuentaSeleccionada=h,a==="deuda"&&(this.filtroTab="deuda"),r(),this.render()}else alert(n?"Debe ingresar el nombre del cliente o empresa deudora.":"Debe ingresar un identificador válido para la cuenta."),s==null||s.focus()})};o()}async modalAbono(){if(!this.cuentaSeleccionada)return;let e=[];try{e=await D.listarMetodosPago()}catch{e=[{nombre:"PUNTOD.VENTA",moneda:"BS"},{nombre:"BIOPAGO",moneda:"BS"},{nombre:"PAGO MOVIL",moneda:"BS"},{nombre:"TRANSF.BS.",moneda:"BS"},{nombre:"BS.EFEC.",moneda:"BS"},{nombre:"DOL.CASH",moneda:"USD"},{nombre:"ZELLE",moneda:"USD"},{nombre:"BINAN.USDT",moneda:"USD"}]}const a=Z(this.cuentaSeleccionada.totalParcialUsd),o=Z(this.cuentaSeleccionada.abonosUsd),n=Math.max(0,a-o),l=this.modelo.tasaActual||807.39,i=(n*l).toFixed(2),s=this.cuentaSeleccionada.tipo==="deuda";let t=e[0]||{nombre:"PUNTOD.VENTA",moneda:"BS"},r=t.moneda==="USD"?n:Number(i),d=l,c="";const b=()=>{var B;const h=e.filter(_=>_.moneda==="USD"),y=e.filter(_=>_.moneda==="BS"),k=h.map(_=>`<option value="${_.nombre}" ${_.nombre===t.nombre?"selected":""}>${_.nombre} ($ USD - Tasa Dinámica)</option>`).join(""),A=`
                <optgroup label="MÉTODOS EN BOLÍVARES (Bs.)">
            ...
            `,O=t.moneda==="USD",R=O&&d>0?d:l,F=O?r*R:r,C=l>0?F/l:0,f=o+C,m=f-a,S=m*l;let M="";if(r<=1e-4)M=`
                    <div class="rounded p-2.5 border-2 border-brand-black bg-gray-50 text-xs text-gray-700 font-bold">
                        ...
                        <div class="flex justify-between items-center text-gray-600 border-t border-gray-300 pt-1 mt-1">
                            ...
                            <span class="font-heading font-black text-sm text-brand-black">$${N(n)} USD (Bs. ${N(i)})</span>
                        </div>
                    </div>
                `;else if(m>.001)M=`
                    <div class="rounded p-2.5 border-2 border-emerald-600 bg-emerald-50 text-xs text-emerald-950 font-bold">
                        ...
                        <div class="flex justify-between items-center text-emerald-900 border-t border-emerald-300 pt-1">
                            ...
                            <span class="font-heading font-black text-base text-emerald-600">+$${N(m)} USD (Bs. ${N(S)})</span>
                        </div>
                ...
                `;else if(m<-.001){const _=Math.abs(m),x=_*l;M=`
            ...
            <div class="fixed inset-0 bg-black/40 flex items-center justify-center z-[100] p-4">
                <div class="bg-white border-2 border-brand-black rounded-lg shadow-brutal p-6 w-full max-w-md">
                    ...
                    <p class="font-body text-xs text-gray-700 mb-3">${(B=this.cuentaSeleccionada)==null?void 0:B.etiqueta} · El usuario decide libremente el monto y tasa del abono.</p>
                    ...
                    <div class="space-y-3 mb-4">
                        ...
                        <div class="bg-purple-50 border border-brand-purple rounded p-2 text-xs font-bold space-y-0.5">
                            ...
                            <div class="flex justify-between">
                                ...
                                <span class="text-brand-black font-mono font-black">$ ${N(C)} USD</span>
                            </div>
                        </div>
                        `:`
                        <div>
                            ...
                            <p class="text-[11px] text-gray-500 mt-1">Equivalente oficial acreditado: <span class="font-bold text-brand-black">$ ${N(C)} USD</span> (Tasa BCV: ${N(l)})</p>
                        </div>
                        `}
            ...
            </div>`,g()},g=()=>{var y,k,U,A,O,R,F;const h=()=>{this.modal.innerHTML=""};(y=this.modal.querySelector("#modal-abono-cancel"))==null||y.addEventListener("click",h),(k=this.modal.querySelector("#select-metodo-abono"))==null||k.addEventListener("change",C=>{const f=C.target.value,m=e.find(S=>S.nombre===f);if(m){const S=t.moneda==="USD";t=m,m.moneda==="USD"?(d=l,S||(r=n)):S&&(r=Number(i)),b()}}),(U=this.modal.querySelector("#input-monto-abono"))==null||U.addEventListener("input",C=>{r=Z(C.target.value),b();const f=this.modal.querySelector("#input-monto-abono");f==null||f.focus()}),(A=this.modal.querySelector("#input-tasa-abono"))==null||A.addEventListener("input",C=>{d=Z(C.target.value),b();const f=this.modal.querySelector("#input-tasa-abono");f==null||f.focus()}),(O=this.modal.querySelector("#btn-reset-tasa-abono"))==null||O.addEventListener("click",()=>{d=l,b()}),(R=this.modal.querySelector("#input-ref-abono"))==null||R.addEventListener("input",C=>{c=C.target.value}),(F=this.modal.querySelector("#modal-abono-ok"))==null||F.addEventListener("click",async()=>{if(!this.cuentaSeleccionada)return;const C=t.moneda==="USD",f=C&&d>0?d:l,m=C?r*f:r,S=l>0?m/l:0;if(S<=0||!Number.isFinite(S)){alert("Por favor ingrese un monto válido.");return}const M=await D.abonarCuenta(this.cuentaSeleccionada.ventaId,S,m);M&&(this.cuentaSeleccionada=M),h(),this.render()})};b()}modalEditarAbono(){var k,U,A,O;if(!this.cuentaSeleccionada)return;const e=Z(this.cuentaSeleccionada.totalParcialUsd),a=Z(this.cuentaSeleccionada.abonosUsd),o=this.modelo.tasaActual||807.39;this.modal.innerHTML=`
        <div class="fixed inset-0 bg-black/40 flex items-center justify-center z-[100] p-4">
            <div class="bg-white border-2 border-brand-black rounded-lg shadow-brutal p-6 w-full max-w-md">
                ...
                <div class="bg-gray-50 border-2 border-brand-black rounded p-3 mb-4 text-xs space-y-1">
                    ...
                    <div class="flex justify-between font-bold text-amber-800">
                        ...
                        <span>$${N(a)} (Bs. ${this.modelo.bs(a)})</span>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-3">
        ...
        </div>`;const n=this.modal.querySelector("#tab-edit-usd"),l=this.modal.querySelector("#tab-edit-bs"),i=this.modal.querySelector("#sec-edit-usd"),s=this.modal.querySelector("#sec-edit-bs"),t=this.modal.querySelector("#edit-abono-usd-val"),r=this.modal.querySelector("#edit-abono-bs-val"),d=this.modal.querySelector("#edit-usd-equiv"),c=this.modal.querySelector("#edit-bs-equiv"),b=this.modal.querySelector("#edit-preview-box");let g="usd";const h=()=>{let R=0;g==="usd"?(R=Z(t==null?void 0:t.value),d&&(d.textContent=`Bs. ${(R*o).toFixed(2)}`)):(R=Z(r==null?void 0:r.value)/o,c&&(c.textContent=`$${R.toFixed(2)} USD`)),Number.isFinite(R)||(R=0);const F=R-e;if(b)if(F>.001)b.className="rounded p-2.5 border-2 border-emerald-600 bg-emerald-50 text-xs text-emerald-950 font-bold",b.innerHTML=`
                        <div class="flex justify-between items-center mb-1">
                    ...
                    `;else if(F<-.001){const C=Math.abs(F);b.className="rounded p-2.5 border-2 border-brand-black bg-gray-50 text-xs text-gray-800 font-bold",b.innerHTML=`
                        ...
                        <div class="flex justify-between items-center text-gray-900 border-t border-gray-300 pt-1">
                            <span>Saldo pendiente restante:</span>
                            <span class="font-heading font-black text-base text-brand-black">$${N(C)} USD</span>
                        </div>
                    ...
                    `};t&&xe(t,5e4,8,h),r&&xe(r,5e7,11,h),h(),(k=this.modal.querySelector("#btn-preset-cero"))==null||k.addEventListener("click",()=>{const R=Z(g==="usd"?t==null?void 0:t.value:r==null?void 0:r.value);(a>0||R>0)&&!window.confirm("¿Desea restablecer el saldo abonado a $0.00? Esta acción dejará los pagos registrados en cero.")||(t&&(t.value="0.00"),r&&(r.value="0.00"),h())}),(U=this.modal.querySelector("#btn-preset-exacto"))==null||U.addEventListener("click",()=>{t&&(t.value=e.toFixed(2)),r&&(r.value=(e*o).toFixed(2)),h()}),n==null||n.addEventListener("click",()=>{g="usd",n.className="flex-1 py-1.5 font-heading font-black text-xs rounded bg-brand-black text-white",l.className="flex-1 py-1.5 font-heading font-bold text-xs rounded text-brand-black hover:bg-white",i==null||i.classList.remove("hidden"),s==null||s.classList.add("hidden"),h()}),l==null||l.addEventListener("click",()=>{g="bs",l.className="flex-1 py-1.5 font-heading font-black text-xs rounded bg-brand-black text-white",n.className="flex-1 py-1.5 font-heading font-bold text-xs rounded text-brand-black hover:bg-white",s==null||s.classList.remove("hidden"),i==null||i.classList.add("hidden"),h()});const y=()=>{this.modal.innerHTML=""};(A=this.modal.querySelector("#modal-edit-cancel"))==null||A.addEventListener("click",y),(O=this.modal.querySelector("#modal-edit-ok"))==null||O.addEventListener("click",async()=>{if(!this.cuentaSeleccionada)return;let R=0;if(g==="usd"?R=Z(t==null?void 0:t.value):R=Z(r==null?void 0:r.value)/o,!Number.isFinite(R)||R<0||R>5e4){alert("Monto inválido. Ingrese un valor entre $0.00 y $50,000.00 USD.");return}if(Math.abs(R-a)>.009){const f=R===0?`¿Confirmas reiniciar el saldo abonado a $0.00? (Monto previo registrado: $${N(a)})`:`¿Confirmas modificar el saldo abonado de esta cuenta a $${N(R)} USD (Bs. ${N(R*o)})? (Monto previo registrado: $${N(a)})`;if(!window.confirm(f))return}const C=await D.editarAbonoCuenta(this.cuentaSeleccionada.ventaId,R);C&&(this.cuentaSeleccionada=C),y(),this.render()})}async modalCierre(){var F,C,f;if(!this.cuentaSeleccionada)return;const e=this.cuentaSeleccionada.tipo==="deuda",a=Z(this.cuentaSeleccionada.totalParcialUsd),o=Z(this.cuentaSeleccionada.abonosUsd),n=Math.max(0,Math.round((a-o)*100)/100),l=Math.max(0,Math.round((o-a)*100)/100),i=this.modelo.tasaActual||807.39;if(n<=.009){this.modal.innerHTML=`
            <div class="fixed inset-0 bg-black/40 flex items-center justify-center z-[100] p-4">
                <div class="bg-white border-2 border-brand-black rounded-lg shadow-brutal p-6 w-full max-w-md">
                    ...
                    <div class="space-y-2 mb-4">
                        ...
                        <div class="bg-emerald-50 border-2 border-emerald-600 rounded p-4 text-center mt-3">
                            ...
                            <p class="font-heading font-black text-3xl text-emerald-600 mt-1">+$${N(l)} USD</p>
                            <p class="font-bold text-xs text-emerald-800">Bs. ${N(l*i)}</p>
                            <div class="mt-3 bg-white border border-emerald-500 rounded p-2.5 text-xs text-emerald-950 font-bold leading-tight text-left">
                                ✓ <strong>Consolidación como ganancia:</strong> Al liquidar y cerrar la cuenta, este excedente de <strong>$${N(l)}</strong> se consolidará automáticamente como ganancia del negocio.
                            </div>
                        </div>
                        `:`
            ...
            </div>`;const m=()=>{this.modal.innerHTML=""};(F=this.modal.querySelector("#modal-liq-cancel"))==null||F.addEventListener("click",m),(C=this.modal.querySelector("#modal-liq-ok"))==null||C.addEventListener("click",async()=>{this.cuentaSeleccionada&&(await D.cerrarCuenta(this.cuentaSeleccionada.ventaId,"0.00",i.toFixed(4)),this.cuentaSeleccionada=null,m(),this.render())});return}let s=[];try{s=await D.listarMetodosPago()}catch{s=[{nombre:"PUNTOD.VENTA",moneda:"BS"},{nombre:"BIOPAGO",moneda:"BS"},{nombre:"PAGO MOVIL",moneda:"BS"},{nombre:"TRANSF.BS.",moneda:"BS"},{nombre:"BS.EFEC.",moneda:"BS"},{nombre:"DOL.CASH",moneda:"USD"},{nombre:"ZELLE",moneda:"USD"},{nombre:"BINAN.USDT",moneda:"USD"}]}let t=i;const r=e?s.find(m=>m.nombre==="TRANSF.BS.")||s[0]||{nombre:"PUNTOD.VENTA",moneda:"BS"}:s.find(m=>m.nombre==="PUNTOD.VENTA")||s[0]||{nombre:"PUNTOD.VENTA",moneda:"BS"},d=this.cuentaSeleccionada.ventaId,c=this.pagosBorradorPorCuenta.get(d);let b;if(c&&c.length>0){if(b=c.map(m=>({...m})),b.length===1&&b[0]){const m=b[0];m.monto=m.moneda==="USD"?n:Number((n*t).toFixed(2)),m.tasaCambio=t}else if(b.length>1){const m=Number((n*t).toFixed(2));let S=0;for(let _=0;_<b.length-1;_++){const x=b[_],v=x.tasaCambio>0?x.tasaCambio:t;S+=x.moneda==="USD"?x.monto*v:x.monto}const M=Math.max(0,Math.round((m-S)*100)/100),B=b[b.length-1];B&&(B.monto=B.moneda==="USD"?t>0?Number((M/t).toFixed(2)):0:M,B.tasaCambio=t)}}else b=[{id:"p-1",metodo:r.nombre,moneda:r.moneda,monto:r.moneda==="USD"?n:Number((n*t).toFixed(2)),tasaCambio:t,referencia:""}];const g=this.resolucionVueltoPorCuenta.get(d);let h=(g==null?void 0:g.estado)||"PAGADO",y=(g==null?void 0:g.metodo)||(e?"TRANSF.BS.":"BS.EFEC."),k=(g==null?void 0:g.tasa)||t;s.some(m=>m.nombre===y)||(y=((f=s[0])==null?void 0:f.nombre)||(e?"TRANSF.BS.":"BS.EFEC."));let U=!1;const A=()=>{let m=0;for(const I of b)if(I.moneda==="USD"){const G=I.tasaCambio>0?I.tasaCambio:t;m+=I.monto*G}else m+=I.monto;m=Math.round(m*100)/100;const S=Math.round(n*t*100)/100,M=t>0?Math.round(m/t*100)/100:0,B=Math.max(0,Math.round((S-m)*100)/100),_=t>0?Math.max(0,Math.round(B/t*100)/100):0,x=Math.max(0,Math.round((m-S)*100)/100),v=t>0?Math.max(0,Math.round(x/t*100)/100):0,T=m>=S-.01&&b.every(I=>I.monto>0);return{cubiertoUsd:M,cubiertoBs:m,totalEsperadoBs:S,faltanteUsd:_,faltanteBs:B,vueltoUsd:v,vueltoBs:x,puedeConfirmar:T}},O=()=>{var G,P;const m=A(),S=s.filter(E=>E.moneda==="USD"),M=s.filter(E=>E.moneda==="BS"),B=b.map((E,K)=>{const W=S.map(z=>`<option value="${z.nombre}" ${z.nombre===E.metodo?"selected":""}>${z.nombre} ($ - Tasa Dinámica)</option>`).join(""),j=`
                        <optgroup label="MÉTODOS EN BOLÍVARES (Bs.)">
                            ${M.map(z=>`<option value="${z.nombre}" ${z.nombre===E.metodo?"selected":""}>${z.nombre} (Bs.)</option>`).join("")}
                        </optgroup>
                        <optgroup label="MÉTODOS EN DÓLARES ($ - TASA PROPIA)">
                            ${W}
                        </optgroup>
                    ...
                    <div class="border-2 border-brand-black rounded-lg p-2.5 bg-gray-50 mb-2">
                        ...
                        `}
                    </div>`}).join(""),_=s.find(E=>E.nombre===y),x=(_==null?void 0:_.moneda)==="USD",v=k>0?k:t,T=x&&v>0?Number((m.vueltoBs/v).toFixed(2)):m.vueltoUsd,I=m.faltanteBs>.009?`
                    <div class="bg-amber-50 border-2 border-amber-500 rounded p-2.5 text-center mb-3">
                        <p class="text-xs font-black uppercase text-amber-900">Monto Incompleto por Cubrir</p>
                        <p class="font-heading font-black text-lg text-amber-700">Faltan Bs. ${N(m.faltanteBs)} · <span class="text-sm text-amber-900">$ ${N(m.faltanteUsd)} (Equiv. Base)</span></p>
                    </div>`:m.vueltoBs>.009?`
                    <div class="border-2 border-emerald-700 bg-emerald-50/70 rounded-lg p-3 mb-3">
                        <div class="flex flex-wrap items-center justify-between gap-1 mb-2">
                            <div>
                                <span class="text-[10px] font-black uppercase text-emerald-900 tracking-wide block">Excedente / Vuelto del Cliente</span>
                                <span class="font-heading font-black text-lg text-emerald-800">Bs. ${N(m.vueltoBs)}</span>
                            </div>
                            <span class="text-xs font-bold text-emerald-900 font-mono">$ ${N(m.vueltoUsd)} equiv.</span>
                        </div>
                        ...
                        <div class="bg-white border border-emerald-600 rounded p-2.5 space-y-2">
                            ...
                            <div class="flex items-center justify-between pt-1 border-t border-gray-100 text-xs">
                                <span class="font-bold text-gray-600">Total a Entregar en Bolívares:</span>
                                <span class="font-heading font-black text-base text-brand-black">Bs. ${N(m.vueltoBs)}</span>
                            </div>
                            `}
                        </div>
                        `:`
                        <div class="bg-white border border-gray-300 rounded p-2 text-xs text-gray-700">
                            <p class="font-bold text-emerald-900 mb-0.5">Vuelto Retenido como Saldo a Favor</p>
                            <p class="text-[11px] text-gray-600 leading-tight">
                                El excedente de <b>Bs. ${N(m.vueltoBs)} ($ ${N(m.vueltoUsd)})</b> no genera egreso físico de caja y se asienta en el comprobante como saldo a favor retenido en el negocio.
                            </p>
                        </div>
                        `}
                ...
                <div class="bg-white border-2 border-brand-black rounded-lg shadow-brutal p-6 w-full max-w-xl max-h-[90vh] overflow-y-auto">
                    ...
                    <div class="bg-brand-gray/30 border-2 border-brand-black rounded-lg p-3 mb-3 flex flex-wrap justify-between items-center gap-2">
                        <div>
                            ...
                            <div class="flex items-baseline gap-2">
                                <span class="font-heading font-black text-2xl text-brand-black">$ ${N(n)} USD</span>
                                <span class="font-heading font-black text-xl text-brand-purple">Bs. ${N(m.totalEsperadoBs)}</span>
                            </div>
                        </div>
                        <div class="text-right text-xs">
                            <span class="text-gray-500 font-bold block">Consumo: $${N(a)} · Abonos: $${N(o)}</span>
                        </div>
                    </div>
                    ...
                    <div class="flex flex-wrap items-center justify-between gap-2 pt-2 border-t-2 border-brand-black">
                        ...
                        <button id="modal-liq-ok" ${m.puedeConfirmar?"":"disabled"}
            ...
            </div>`,R()},R=()=>{var B,_,x,v,T,I,G,P,E,K,W;const m=()=>{this.cuentaSeleccionada&&(this.pagosBorradorPorCuenta.set(this.cuentaSeleccionada.ventaId,b),this.resolucionVueltoPorCuenta.set(this.cuentaSeleccionada.ventaId,{estado:h,metodo:y,tasa:k}))},S=()=>{m(),this.modal.innerHTML=""};(B=this.modal.querySelector("#modal-liq-cerrar"))==null||B.addEventListener("click",S),(_=this.modal.querySelector("#modal-liq-cancel"))==null||_.addEventListener("click",S),(x=this.modal.querySelector("#modal-liq-agregar-mas"))==null||x.addEventListener("click",()=>{m(),this.modal.innerHTML=""}),(v=this.modal.querySelector("#btn-vuelto-pagado-cuenta"))==null||v.addEventListener("click",()=>{h="PAGADO",m(),O()}),(T=this.modal.querySelector("#btn-vuelto-retenido-cuenta"))==null||T.addEventListener("click",()=>{h="RETENIDO",m(),O()}),(I=this.modal.querySelector("#select-metodo-vuelto-cuenta"))==null||I.addEventListener("change",L=>{y=L.target.value,m(),O()});const M=this.modal.querySelector("#input-tasa-vuelto-cuenta");if(M&&(M.addEventListener("input",L=>{const j=Z(L.target.value);k=j>0?j:t,m()}),M.addEventListener("blur",()=>{O()})),(G=this.modal.querySelector("#btn-reset-tasa-vuelto-cuenta"))==null||G.addEventListener("click",()=>{k=t,m(),O()}),e){const L=this.modal.querySelector("#deuda-tasa-input"),j=this.modal.querySelector("#btn-tasa-bcv");L&&xe(L,1e5,9,()=>{const $=Z(L.value);t=Number.isFinite($)&&$>0?$:i,m(),O()}),j==null||j.addEventListener("click",()=>{t=i,m(),O()})}this.modal.querySelectorAll("select[data-pago-metodo]").forEach(L=>{L.addEventListener("change",j=>{const $=L.dataset.pagoMetodo,q=j.target.value,V=b.find(z=>z.id===$);if(V){const z=s.find(ne=>ne.nombre===q),oe=(z==null?void 0:z.moneda)||"BS";V.moneda=oe,V.tasaCambio=t,b.length===1?V.monto=V.moneda==="USD"?n:Number((n*t).toFixed(2)):oe!==V.moneda&&(oe==="USD"?V.monto=Number((t>0?V.monto/t:0).toFixed(2)):V.monto=Number((V.monto*t).toFixed(2))),V.metodo=q,m(),O()}})}),this.modal.querySelectorAll("input[data-pago-tasa]").forEach(L=>{L.addEventListener("input",j=>{const $=L.dataset.pagoTasa,q=Z(j.target.value),V=b.find(z=>z.id===$);if(V){V.tasaCambio=q>0?q:t,m();const z=A(),oe=this.modal.querySelector("#modal-liq-ok");oe&&(oe.disabled=!z.puedeConfirmar)}}),L.addEventListener("blur",()=>{O()})}),this.modal.querySelectorAll("button[data-reset-tasa-cuenta]").forEach(L=>{L.addEventListener("click",()=>{const j=L.dataset.resetTasaCuenta,$=b.find(q=>q.id===j);$&&($.tasaCambio=t,m(),O())})}),this.modal.querySelectorAll("input[data-pago-monto]").forEach(L=>{L.addEventListener("input",j=>{const $=L.dataset.pagoMonto,q=Z(j.target.value),V=b.find(z=>z.id===$);if(V){V.monto=q,m();const z=A(),oe=this.modal.querySelector("#modal-liq-ok");oe&&(oe.disabled=!z.puedeConfirmar)}}),L.addEventListener("blur",()=>{O()})}),this.modal.querySelectorAll("input[data-pago-ref]").forEach(L=>{L.addEventListener("input",j=>{const $=L.dataset.pagoRef,q=j.target.value,V=b.find(z=>z.id===$);V&&(V.referencia=q,m())})}),this.modal.querySelectorAll("button[data-eliminar-pago]").forEach(L=>{L.addEventListener("click",()=>{const j=L.dataset.eliminarPago;b=b.filter($=>$.id!==j),m(),O()})}),(P=this.modal.querySelector("#btn-agregar-pago-cuenta"))==null||P.addEventListener("click",()=>{let L=A();const j="p-"+Math.random().toString(36).slice(2,7),$=s.find(V=>!b.some(z=>z.metodo===V.nombre))||s[0]||{nombre:"BS.EFEC.",moneda:"BS"};if(L.faltanteBs<=.01&&b.length===1&&b[0]){const V=b[0],z=V.tasaCambio>0?V.tasaCambio:t,oe=V.moneda==="USD"?V.monto*z:V.monto,ne=Math.round(oe/2*100)/100;V.monto=V.moneda==="USD"?z>0?Number((ne/z).toFixed(2)):0:ne,L=A()}const q=L.faltanteBs>0?L.faltanteBs:0;b.push({id:j,metodo:$.nombre,moneda:$.moneda,monto:$.moneda==="USD"?t>0?Number((q/t).toFixed(2)):0:q,tasaCambio:t,referencia:""}),m(),O()}),(E=this.modal.querySelector("#btn-toggle-nuevo-metodo-cuenta"))==null||E.addEventListener("click",()=>{U=!U,O()}),(K=this.modal.querySelector("#btn-guardar-nuevo-metodo-cuenta"))==null||K.addEventListener("click",()=>{(async()=>{const L=this.modal.querySelector("#input-nuevo-metodo-nombre-cuenta"),j=(L==null?void 0:L.value.trim())||"",$=this.modal.querySelector('input[name="radio-nuevo-metodo-moneda-cuenta"][value="USD"]'),q=$!=null&&$.checked?"USD":"BS";if(j)try{s=await D.crearMetodoPago(j,q),U=!1,O()}catch{}})()}),(W=this.modal.querySelector("#modal-liq-ok"))==null||W.addEventListener("click",()=>{(async()=>{if(!this.cuentaSeleccionada)return;const L=this.modal.querySelector("#cobro-error-cuenta");try{const j=b.filter(ae=>ae.monto>0).map(ae=>{const be=ae.moneda==="USD"&&ae.tasaCambio>0?ae.tasaCambio:t,ke=ae.moneda==="USD"?ae.monto*be:ae.monto,Be=t>0?ke/t:0;return{metodo:ae.metodo,moneda:ae.moneda,montoUsd:(ae.moneda==="USD"?ae.monto:Be).toFixed(2),montoBs:ke.toFixed(2),tasaCambio:ae.moneda==="USD"?be.toFixed(2):void 0,referencia:ae.referencia.trim()||void 0}});if(j.length===0)throw new Error("Debe especificar al menos un pago válido.");const $=A(),q=j.reduce((ae,be)=>ae+Number(be.montoBs),0),V=s.find(ae=>ae.nombre===y),z=(V==null?void 0:V.moneda)==="USD",oe=z?k>0?k:t:void 0,ne=z&&oe?($.vueltoBs/oe).toFixed(2):$.vueltoUsd.toFixed(2),$e={estado:$.vueltoBs>.009?h:"SIN_VUELTO",metodo:$.vueltoBs>.009&&h==="PAGADO"?y:void 0,montoBs:$.vueltoBs>.009?$.vueltoBs.toFixed(2):void 0,montoUsd:$.vueltoBs>.009?ne:void 0,tasa:$.vueltoBs>.009&&h==="PAGADO"&&oe?oe.toFixed(2):void 0},ge=this.cuentaSeleccionada.ventaId;await D.cerrarCuenta(ge,q.toFixed(2),t.toFixed(4),j,$e),this.pagosBorradorPorCuenta.delete(ge),this.resolucionVueltoPorCuenta.delete(ge),this.cuentaSeleccionada=null,this.modal.innerHTML="",this.render()}catch(j){L&&(L.textContent=j instanceof Error?j.message.replace(/"/g,""):String(j),L.classList.remove("hidden"))}})()})};O()}}const de=(u,e,a,o)=>{u.maxLength=a,u.addEventListener("input",()=>{let n=u.value.replace(/[^0-9.,]/g,"");const l=n.split(/[.,]/);l.length>2?n=l[0]+"."+l.slice(1).join(""):l.length===2&&(n=l[0]+"."+l[1].slice(0,2)),n.length>a&&(n=n.slice(0,a));const i=parseFloat(n.replace(",","."));Number.isFinite(i)&&i>e&&(n=String(e)),u.value!==n&&(u.value=n),o&&o()})},we=(u,e,a,o,n)=>{u.maxLength=o,u.addEventListener("input",()=>{let l=u.value.replace(/[^0-9]/g,"");l.length>o&&(l=l.slice(0,o));const i=parseInt(l,10);Number.isFinite(i)&&i>a&&(l=String(a)),u.value!==l&&(u.value=l),n&&n()}),u.addEventListener("blur",()=>{const l=parseInt(u.value,10);Number.isFinite(l)&&l<e&&(u.value=String(e),n&&n())})},ce=(u,e,a=!1)=>{u.maxLength=e,u.addEventListener("input",()=>{let o=a?u.value.replace(/[<>{}\\]/g,""):u.value.replace(/[^a-zA-Z0-9áéíóúÁÉÍÓÚñÑüÜ .,/_#-]/g,"");o.length>e&&(o=o.slice(0,e)),u.value!==o&&(u.value=o)})};class Je{constructor(e,a){w(this,"contenedor");w(this,"modelo");w(this,"categorias",[]);w(this,"tasasImpuestos",[]);w(this,"productos",[]);w(this,"busquedaRepo","");w(this,"busquedaCatalogo","");w(this,"filtroCategoriaRepo","todas");w(this,"filtroCategoriaCatalogo","todas");w(this,"tabActiva","alta");w(this,"semaforoStock",{rojoMax:5,amarilloMax:15});w(this,"POR_PAGINA_CAT",40);w(this,"POR_PAGINA_REPO",24);w(this,"paginaCat",1);w(this,"paginaRepo",1);this.contenedor=e,this.modelo=a,this.modelo}getUnitLabel(e){return e.unidad==="kg"?"kg":e.unidad==="ml"?"ml":e.unidad==="un"?"un.":e.capacidades&2?"kg":"un."}getNombreCategoria(e){if(!e)return"General";const a=this.categorias.find(o=>o.id===e);return a?a.nombre:"General"}getConteoCategorias(){const e={todas:this.productos.length};return this.categorias.forEach(a=>{e[a.id]=0}),e["cat-general"]===void 0&&(e["cat-general"]=0),this.productos.forEach(a=>{const o=a.categoriaId||"cat-general";e[o]=(e[o]||0)+1}),e}async render(e){e&&(this.tabActiva=e);const[a,o,n,l]=await Promise.all([D.categorias(),D.tasasImpuestos(),D.productos(),D.obtenerSemaforoStock()]);this.categorias=a,this.tasasImpuestos=o,this.productos=n,this.semaforoStock=l;const i=this.getConteoCategorias(),s="Ej: Artículo Comercial 1L, Presentación 500g, Pack Estándar...";this.contenedor.innerHTML=`
        <div class="mb-4 sm:mb-6 flex flex-wrap items-center justify-between gap-3">
            <div>
        ...
        <div id="inv-seccion-stock" class="${this.tabActiva==="stock"?"block":"hidden"} space-y-4">
            <div class="bg-white border-2 border-brand-black rounded-lg shadow-brutal p-4 sm:p-6">
                ...
                <div class="flex items-center gap-1.5 overflow-x-auto pb-2 mb-3 max-w-full" id="repo-chips-categorias">
                    ...
                    ${this.categorias.map(t=>{const r=i[t.id]||0,d=this.filtroCategoriaRepo===t.id;return`
        ...
        <div id="inv-seccion-ajustes" class="${this.tabActiva==="ajustes"?"block":"hidden"} grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="bg-white border-2 border-brand-black rounded-lg shadow-brutal p-6 space-y-4">
                ...
                <div class="space-y-2 max-h-[350px] overflow-y-auto">
                    ${this.categorias.map(t=>{const r=i[t.id]||0;return`
                        <div class="flex justify-between items-center border-2 border-brand-black rounded-lg p-3 bg-gray-50">
                            <div>
                                ...
                                <p class="text-[11px] font-bold text-gray-500">${r} ${r===1?"producto asociado":"productos asociados"}</p>
                            </div>
                            <div class="flex items-center gap-2">
                                <span class="text-xs font-black px-2 py-0.5 rounded border border-brand-black ${r>0?"bg-amber-100 text-amber-900":"bg-gray-200 text-gray-600"}">
                                    ${r} prods.
                                </span>
        ...
        </div>`,this.conectarEventos(),this.renderCatalogoLista(),this.renderRepoLista()}conectarEventos(){var I,G,P,E,K,W;const e=document.getElementById("tab-alta"),a=document.getElementById("tab-stock"),o=document.getElementById("tab-ajustes"),n=document.getElementById("inv-seccion-alta"),l=document.getElementById("inv-seccion-stock"),i=document.getElementById("inv-seccion-ajustes"),s=(L,j,$)=>{this.tabActiva=L,[e,a,o].forEach(q=>{q&&(q.className="w-36 sm:w-48 h-9 flex items-center justify-center font-heading font-bold text-xs rounded text-brand-black hover:bg-gray-100 transition-colors shrink-0")}),[n,l,i].forEach(q=>{q==null||q.classList.remove("block"),q==null||q.classList.add("hidden")}),j&&$&&(j.className="w-36 sm:w-48 h-9 flex items-center justify-center font-heading font-black text-xs rounded bg-brand-black text-white transition-colors shrink-0",$.classList.remove("hidden"),$.classList.add("block"))};e==null||e.addEventListener("click",()=>s("alta",e,n)),a==null||a.addEventListener("click",()=>{s("stock",a,l),this.renderRepoLista()}),o==null||o.addEventListener("click",()=>s("ajustes",o,i));const t=document.getElementById("prod-precio-bruto"),r=document.getElementById("prod-margen"),d=document.getElementById("prod-impuesto"),c=document.getElementById("prod-precio-neto"),b=L=>{if(!L)return 0;const j=L.trim().replace(",","."),$=parseFloat(j);return Number.isFinite($)&&!isNaN($)?$:0},g=()=>{if(!t||!c||!r||!d)return;const L=b(t.value),j=b(r.value)/100,$=b(d.value)/100;if(L>0){const V=L*(1+j)*(1+$);c.value=V.toFixed(2)}},h=()=>{if(!t||!c||!r||!d)return;const L=b(c.value),j=b(t.value),$=b(r.value)/100,q=b(d.value)/100;if(L>0){const V=L/(1+q);if(j>0){const z=(V-j)/j*100;r.value=Math.max(0,Math.round(z*10)/10).toFixed(1)}else $>0&&(t.value=(V/(1+$)).toFixed(2))}},y=document.getElementById("prod-stock"),k=document.getElementById("prod-nombre"),U=document.getElementById("prod-unidad"),A=document.getElementById("cat-nombre"),O=document.getElementById("tax-nombre"),R=document.getElementById("tax-pct"),F=document.getElementById("cat-buscar-filtro"),C=document.getElementById("repo-buscar"),f=document.getElementById("alta-prod-msg");k&&ce(k,64),t&&de(t,99999.99,8,g),r&&de(r,999,5,g),c&&de(c,99999.99,8,h),y&&de(y,99999,7),A&&ce(A,32),O&&ce(O,32),R&&de(R,100,5),F&&ce(F,40),C&&ce(C,40),d==null||d.addEventListener("change",g);const m=()=>{f&&!f.classList.contains("hidden")&&f.style.color!=="#00823B"&&(f.classList.add("hidden"),f.textContent="")};k==null||k.addEventListener("input",m),c==null||c.addEventListener("input",m),y==null||y.addEventListener("input",m),U==null||U.addEventListener("change",m);const S=document.getElementById("prod-sin-stock"),M=document.getElementById("box-stock-inicial");S==null||S.addEventListener("change",()=>{m(),M&&(M.style.opacity=S.checked?"0.3":"1",M.querySelector("input").disabled=S.checked)});const B=document.getElementById("prod-es-caja"),_=document.getElementById("box-caja-config"),x=document.getElementById("prod-unidades-caja"),v=document.getElementById("prod-stock-cajas");B==null||B.addEventListener("change",()=>{m(),_&&(B.checked?_.classList.remove("hidden"):_.classList.add("hidden"))});const T=()=>{if(B!=null&&B.checked&&v&&x&&y){const L=parseInt(v.value,10),j=parseInt(x.value,10);Number.isFinite(L)&&L>=0&&Number.isFinite(j)&&j>1&&(y.value=String(L*j))}};x&&we(x,2,1e3,4,T),v&&we(v,0,9999,4,T),v==null||v.addEventListener("input",T),x==null||x.addEventListener("input",T),(I=document.getElementById("form-nuevo-producto"))==null||I.addEventListener("submit",L=>{L.preventDefault(),this.guardarProducto()}),(G=document.getElementById("cat-buscar-filtro"))==null||G.addEventListener("input",L=>{this.busquedaCatalogo=L.target.value.trim().toLowerCase(),this.paginaCat=1,this.renderCatalogoLista()}),(P=document.getElementById("cat-filtro-lateral"))==null||P.addEventListener("change",L=>{this.filtroCategoriaCatalogo=L.target.value,this.paginaCat=1,this.renderCatalogoLista()}),(E=document.getElementById("repo-buscar"))==null||E.addEventListener("input",L=>{this.busquedaRepo=L.target.value.trim().toLowerCase(),this.paginaRepo=1,this.renderRepoLista()}),this.contenedor.querySelectorAll("[data-cat-chip]").forEach(L=>{L.addEventListener("click",()=>{const j=L.dataset.catChip||"todas";this.filtroCategoriaRepo=j,this.paginaRepo=1,this.contenedor.querySelectorAll("[data-cat-chip]").forEach($=>{($.dataset.catChip||"todas")===j?$.className="px-3 py-1 rounded text-xs font-heading font-black border-2 border-brand-black shrink-0 transition-colors bg-brand-black text-white":$.className="px-3 py-1 rounded text-xs font-heading font-black border-2 border-brand-black shrink-0 transition-colors bg-gray-100 text-brand-black hover:bg-gray-200"}),this.renderRepoLista()})}),(K=document.getElementById("form-crear-cat"))==null||K.addEventListener("submit",L=>{L.preventDefault();const j=document.getElementById("cat-nombre");j!=null&&j.value.trim()&&D.crearCategoria(j.value.trim()).then(()=>void this.render("ajustes"))}),this.contenedor.querySelectorAll("[data-del-cat]").forEach(L=>{L.addEventListener("click",()=>{const j=L.dataset.delCat;j&&D.eliminarCategoria(j).then(()=>void this.render("ajustes"))})}),(W=document.getElementById("form-crear-tax"))==null||W.addEventListener("submit",L=>{L.preventDefault();const j=document.getElementById("tax-nombre"),$=document.getElementById("tax-pct");j!=null&&j.value.trim()&&$&&D.crearTasaImpuesto(j.value.trim(),Number($.value)).then(()=>void this.render("ajustes"))}),this.contenedor.querySelectorAll("[data-del-tax]").forEach(L=>{L.addEventListener("click",()=>{const j=L.dataset.delTax;j&&D.eliminarTasaImpuesto(j).then(()=>void this.render("ajustes"))})})}renderCatalogoLista(){var i,s,t,r;const e=document.getElementById("cat-lista-items");if(!e)return;const a=this.productos.filter(d=>{const c=!this.busquedaCatalogo||d.nombre.toLowerCase().includes(this.busquedaCatalogo)||d.sku.toLowerCase().includes(this.busquedaCatalogo),b=this.filtroCategoriaCatalogo==="todas"||(d.categoriaId||"cat-general")===this.filtroCategoriaCatalogo;return c&&b});if(a.length===0){e.innerHTML='<p class="py-4 text-center text-gray-400 font-bold text-xs">Sin coincidencias en esta categoría.</p>',(i=document.getElementById("paginacion-cat"))==null||i.remove();return}const o=Math.ceil(a.length/this.POR_PAGINA_CAT);this.paginaCat>o&&(this.paginaCat=o);const n=(this.paginaCat-1)*this.POR_PAGINA_CAT,l=a.slice(n,n+this.POR_PAGINA_CAT);if(e.innerHTML=l.map(d=>{const c=Number(d.stock),b=d.sinStock?"bg-purple-100 text-purple-900":c>15?"bg-green-100 text-green-900":c>=10?"bg-yellow-100 text-yellow-900":"bg-red-100 text-red-900",g=this.getUnitLabel(d),h=this.getNombreCategoria(d.categoriaId);return`
            <div class="border border-brand-black rounded p-2 bg-gray-50 flex justify-between items-center text-xs">
            ...
            </div>`}).join(""),(s=document.getElementById("paginacion-cat"))==null||s.remove(),o>1){const d=document.createElement("div");d.id="paginacion-cat",d.className="flex items-center justify-between mt-2 gap-2 flex-wrap",d.innerHTML=`
                ...
                </div>`,e.insertAdjacentElement("afterend",d),(t=document.getElementById("cat-pag-ant"))==null||t.addEventListener("click",()=>{this.paginaCat>1&&(this.paginaCat--,this.renderCatalogoLista())}),(r=document.getElementById("cat-pag-sig"))==null||r.addEventListener("click",()=>{this.paginaCat<o&&(this.paginaCat++,this.renderCatalogoLista())})}}renderRepoLista(){var t,r,d;const e=document.getElementById("repo-lista");if(!e)return;const a=this.productos.filter(c=>{const b=!this.busquedaRepo||c.nombre.toLowerCase().includes(this.busquedaRepo)||c.sku.toLowerCase().includes(this.busquedaRepo),g=this.filtroCategoriaRepo==="todas"||(c.categoriaId||"cat-general")===this.filtroCategoriaRepo;return b&&g}),o=document.getElementById("repo-contador-estado");if(o){const c=this.filtroCategoriaRepo==="todas"?"en depósito":`en ${this.getNombreCategoria(this.filtroCategoriaRepo)}`;o.textContent=`${a.length} de ${this.productos.length} artículos ${c}`}if(a.length===0){e.innerHTML='<p class="col-span-full py-8 text-center text-gray-500 font-bold">No se encontraron productos en la categoría seleccionada.</p>';return}const n=this.semaforoStock,l=Math.ceil(a.length/this.POR_PAGINA_REPO);this.paginaRepo>l&&(this.paginaRepo=l);const i=(this.paginaRepo-1)*this.POR_PAGINA_REPO,s=a.slice(i,i+this.POR_PAGINA_REPO);if(e.innerHTML=s.map(c=>{const b=Number(c.stock),g=c.sinStock?"text-purple-700":b<=n.rojoMax?"text-red-600":b<=n.amarilloMax?"text-amber-600":"text-emerald-700",h=this.getUnitLabel(c),y=this.getNombreCategoria(c.categoriaId),k=c.sinStock?"Venta libre":c.esCaja&&c.unidadesPorCaja&&c.unidadesPorCaja>1?`${Math.floor(b/c.unidadesPorCaja)} cajas y ${b%c.unidadesPorCaja} un. (${b} un.)`:`${b} ${h}`;return`
            <div class="border-2 border-brand-black rounded-lg p-3 bg-white shadow-sm flex flex-col justify-between h-[175px] min-h-[175px] max-h-[175px] box-border overflow-hidden">
                ...
                </div>`,e.insertAdjacentElement("afterend",c),(r=document.getElementById("repo-pag-ant"))==null||r.addEventListener("click",()=>{this.paginaRepo>1&&(this.paginaRepo--,this.renderRepoLista())}),(d=document.getElementById("repo-pag-sig"))==null||d.addEventListener("click",()=>{this.paginaRepo<l&&(this.paginaRepo++,this.renderRepoLista())})}e.querySelectorAll("input[data-repo-cant]").forEach(c=>{de(c,9999,5)}),e.querySelectorAll("[data-repo-del]").forEach(c=>{c.addEventListener("click",()=>{const b=c.dataset.repoDel,g=this.productos.find(y=>y.sku===b);if(!b||!g)return;window.confirm(`¿Seguro que deseas eliminar permanentemente el producto "${g.nombre}" del catálogo?`)&&D.eliminarProducto(b).then(()=>{D.productos().then(y=>{this.productos=y,this.renderRepoLista(),this.renderCatalogoLista();const k=document.getElementById("repo-contador-estado");k&&(k.textContent=`${this.productos.length} artículos en depósito`)})})})}),e.querySelectorAll("[data-repo-in]").forEach(c=>{c.addEventListener("click",()=>{const b=c.dataset.repoIn,g=e.querySelector(`input[data-repo-cant="${b}"]`),h=g==null?void 0:g.value;b&&h&&Number(h)>0&&Number(h)<=9999&&D.compraStock(b,h).then(()=>{D.productos().then(y=>{this.productos=y,this.renderRepoLista(),this.renderCatalogoLista()})})})}),e.querySelectorAll("[data-repo-red]").forEach(c=>{c.addEventListener("click",()=>{const b=c.dataset.repoRed,g=e.querySelector(`input[data-repo-cant="${b}"]`),h=g==null?void 0:g.value;b&&h&&Number(h)>0&&Number(h)<=9999&&D.reducirStock(b,h).then(()=>{D.productos().then(y=>{this.productos=y,this.renderRepoLista(),this.renderCatalogoLista()})})})}),e.querySelectorAll("[data-repo-out]").forEach(c=>{c.addEventListener("click",()=>{const b=c.dataset.repoOut,g=e.querySelector(`input[data-repo-cant="${b}"]`),h=g==null?void 0:g.value;b&&h&&Number(h)>0&&Number(h)<=9999&&D.merma(b,h,null).then(()=>{D.productos().then(y=>{this.productos=y,this.renderRepoLista(),this.renderCatalogoLista()})})})})}async guardarProducto(){var C;const e=document.getElementById("prod-nombre"),a=document.getElementById("prod-categoria"),o=document.getElementById("prod-impuesto"),n=document.getElementById("prod-precio-bruto"),l=document.getElementById("prod-margen"),i=document.getElementById("prod-precio-neto"),s=document.getElementById("prod-stock"),t=((C=document.getElementById("prod-unidad"))==null?void 0:C.value)||"un",r=document.getElementById("prod-sin-stock"),d=document.getElementById("alta-prod-msg"),c=f=>{if(!f)return 0;const m=f.trim().replace(",","."),S=parseFloat(m);return Number.isFinite(S)&&!isNaN(S)?S:0};if(d&&(d.classList.add("hidden"),d.textContent=""),!(e!=null&&e.value.trim())){d&&(d.textContent="El nombre del producto o servicio es obligatorio.",d.style.color="#C60C15",d.classList.remove("hidden"),e==null||e.focus());return}const b=c(i==null?void 0:i.value);if(b<=0||b>99999.99){d&&(d.textContent="El precio neto final debe ser un monto positivo entre $0.01 y $99,999.99 USD.",d.style.color="#C60C15",d.classList.remove("hidden"),i==null||i.focus());return}const g=c(s==null?void 0:s.value),h=(r==null?void 0:r.checked)||!1;if(!h&&(g<0||g>99999)){d&&(d.textContent="El stock inicial debe estar comprendido entre 0 y 99,999 unidades.",d.style.color="#C60C15",d.classList.remove("hidden"),s==null||s.focus());return}if(!h&&t==="un"&&(!Number.isInteger(g)||g%1!==0)){d&&(d.textContent='Los productos vendidos por Unidad (un.) deben tener un stock entero exacto (ej: 10). Para registrar fracciones o decimales, seleccione Tipo de Medida "Por Peso (kg)" o "Por Volumen (ml)".',d.style.color="#C60C15",d.classList.remove("hidden"),s==null||s.focus());return}const y=c((o==null?void 0:o.value)||"0"),k=c((l==null?void 0:l.value)||"30");let U=c(n==null?void 0:n.value);U<=0&&(U=b/(1+y/100)/(1+k/100));const A=document.getElementById("prod-es-caja"),O=document.getElementById("prod-unidades-caja"),R=(A==null?void 0:A.checked)||!1,F=R&&O?Math.max(2,parseInt(O.value,10)||12):void 0;try{await D.crearProducto({nombre:e.value.trim(),precioBrutoUsd:U.toFixed(2),margenPct:String(k),precioUsd:b.toFixed(2),impuestoPct:String(y),stockInicial:r!=null&&r.checked?"0":String(g),categoriaId:(a==null?void 0:a.value)||"",sinStock:(r==null?void 0:r.checked)||!1,pesable:t==="kg"||t==="ml",unidad:t,alcoholica:!1,esCaja:R,unidadesPorCaja:F}),d&&(d.textContent="Producto o servicio registrado exitosamente.",d.style.color="#00823B",d.classList.remove("hidden")),setTimeout(()=>void this.render("alta"),800)}catch(f){d&&(d.textContent=f instanceof Error?f.message:String(f),d.style.color="#C60C15",d.classList.remove("hidden"))}}}const le=u=>{if(typeof u=="number")return Number.isFinite(u)?u:0;if(u==null)return 0;let e=String(u).trim();if(!e)return 0;e.includes(",")&&!e.includes(".")?e=e.replace(",","."):e.includes(".")&&e.includes(",")&&(e=e.replace(/\./g,"").replace(",","."));const a=parseFloat(e);return Number.isFinite(a)&&!isNaN(a)?a:0},X=u=>le(u).toLocaleString("es-VE",{minimumFractionDigits:2,maximumFractionDigits:2}),Se=u=>{const e=u.finUnix||Math.floor(Date.now()/1e3),a=Math.max(0,e-u.inicioUnix),o=Math.floor(a/3600),n=Math.floor(a%3600/60);return`${o}h ${n}m`},Ke=(u,e)=>{const a=u.operadoresRelevo.length>0?u.operadoresRelevo.join(" / "):u.operadorActual,o=e.filter(c=>c.operador&&u.operadoresRelevo.some(b=>c.operador===b)),n=[["INFORME DE JORNADA",u.id],["Estado",u.estado==="abierta"?"En curso":"Cerrada"],["Apertura",u.inicioStr],["Cierre",u.finStr||"-"],["Duracion",Se(u)],["Operadores",a],["Total USD",`$${X(u.ventasTotalUsd)}`],["Total Bs",`Bs. ${X(u.ventasTotalBs)}`],["Tickets emitidos",String(u.ticketsEmitidos)],["Vuelto pagado Bs",`Bs. ${X(u.vueltoPagadoBs)}`],["Vuelto retenido Bs",`Bs. ${X(u.vueltoRetenidoBs)}`],["Deudas liquidadas USD",`$${X(u.deudasLiquidadasUsd)}`],["Entradas stock",String(u.entradasStockReg)],["Mermas",String(u.mermasStockReg)],["Cambios de precio",String(u.cambiosPrecioReg)],["Tasa apertura",`Bs. ${u.tasaInicio}`],["Tasa cierre",u.tasaFin?`Bs. ${u.tasaFin}`:"-"],...u.checksumSha256?[["SHA-256",u.checksumSha256]]:[],[],["TICKETS DE LA JORNADA"],["ID Ticket","Fecha/Hora","Canal","Operador","Metodo(s) de Pago","Total USD","Total Bs"]],l=o.map(c=>{const b=c.pagos&&c.pagos.length>0?c.pagos.map(g=>`${g.metodo}:$${X(g.montoUsd)}`).join(" | "):"DIRECTO";return[c.ventaId,c.fechaHora||"",c.canal||"VENTA DIRECTA",c.operador||"Caja",b,`$${X(c.totalUsd)}`,`Bs. ${X(c.totalBs)}`]}),s=[...n,...l].map(c=>c.map(b=>`"${String(b).replace(/"/g,'""')}"`).join(",")).join(`\r
`),t=new Blob(["\uFEFF"+s],{type:"text/csv;charset=utf-8;"}),r=URL.createObjectURL(t),d=document.createElement("a");d.href=r,d.download=`jornada_${u.id}.csv`,document.body.appendChild(d),d.click(),document.body.removeChild(d),URL.revokeObjectURL(r)};class Xe{constructor(e,a,o){w(this,"contenedor");w(this,"modalRoot");w(this,"modelo");w(this,"jornadas",[]);w(this,"jornadaActual",null);w(this,"todosLosTickets",[]);w(this,"busqueda","");w(this,"expandidos",new Set);w(this,"paginaActual",1);w(this,"POR_PAGINA",20);this.contenedor=e,this.modalRoot=a,this.modelo=o}async render(){const[e,a,o]=await Promise.all([D.obtenerJornadaActual(),D.listarHistoricoJornadas(),D.ventas()]);this.jornadaActual=e,this.jornadas=a,this.todosLosTickets=o;const n=(e?1:0)+a.length,l=[...e?[e]:[],...a],i=l.reduce((t,r)=>t+le(r.ventasTotalUsd),0),s=l.reduce((t,r)=>t+r.ticketsEmitidos,0);this.contenedor.innerHTML=`
        <div class="mb-4 sm:mb-6 flex flex-wrap items-center justify-between gap-3">
            <div>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3 mb-4 sm:mb-5">
            <div class="border-2 border-brand-black rounded-lg shadow-brutal-sm p-3 bg-amber-50">
                ...
                <div class="font-heading font-black text-lg mt-0.5 text-brand-black">$ ${X(i)}</div>
            </div>
            ...
            <div class="border-2 border-brand-black rounded-lg shadow-brutal-sm p-3 bg-purple-50">
                ...
                <div class="font-heading font-black text-lg mt-0.5 text-brand-purple">Bs. ${X(this.modelo.tasaActual||807.39)}</div>
            </div>
        ...
        <div id="lista-jornadas" class="flex flex-col gap-2"></div>`,this.conectarEventosBuscador(),this.renderLista()}conectarEventosBuscador(){var e;(e=document.getElementById("jornadas-buscar"))==null||e.addEventListener("input",a=>{this.busqueda=a.target.value,this.paginaActual=1,this.renderLista()})}renderLista(){var d,c;const e=document.getElementById("lista-jornadas");if(!e)return;const a=[...this.jornadaActual?[this.jornadaActual]:[],...this.jornadas],o=this.busqueda.trim().toLowerCase(),n=o?a.filter(b=>b.id.toLowerCase().includes(o)||b.operadorActual.toLowerCase().includes(o)||b.operadoresRelevo.some(g=>g.toLowerCase().includes(o))||b.inicioStr.toLowerCase().includes(o)||(b.finStr||"").toLowerCase().includes(o)):a,l=document.getElementById("paginacion-jornadas");if(l&&l.remove(),n.length===0){e.innerHTML=`
            ...
            </div>`;return}const i=Math.max(1,Math.ceil(n.length/this.POR_PAGINA));this.paginaActual>i&&(this.paginaActual=i),this.paginaActual<1&&(this.paginaActual=1);const s=this.paginaActual,t=n.slice((s-1)*this.POR_PAGINA,s*this.POR_PAGINA);e.innerHTML=t.map(b=>this.renderFila(b)).join(""),t.forEach(b=>{if(this.expandidos.has(b.id)){const g=document.getElementById(`detalle-${b.id}`);g==null||g.classList.remove("hidden");const h=document.getElementById(`icon-${b.id}`);h&&(h.textContent="▲")}}),e.querySelectorAll("[data-toggle-jornada]").forEach(b=>{b.addEventListener("click",()=>{const g=b.dataset.toggleJornada,h=document.getElementById(`detalle-${g}`),y=document.getElementById(`icon-${g}`);if(h){const k=h.classList.contains("hidden");h.classList.toggle("hidden",!k),y&&(y.textContent=k?"▲":"▼"),k?this.expandidos.add(g):this.expandidos.delete(g)}})}),e.querySelectorAll("[data-exportar-jornada]").forEach(b=>{b.addEventListener("click",g=>{g.stopPropagation();const h=b.dataset.exportarJornada,y=n.find(k=>k.id===h);y&&Ke(y,this.todosLosTickets)})}),e.querySelectorAll("[data-ver-ticket]").forEach(b=>{b.addEventListener("click",()=>{const g=b.dataset.verTicket,h=this.todosLosTickets.find(y=>y.ventaId===g);h&&this.mostrarModalTicket(h)})});const r=document.createElement("div");r.id="paginacion-jornadas",r.innerHTML=`
            <div class="flex items-center justify-between mt-3 gap-3 flex-wrap">
                <span class="text-xs font-bold text-gray-600">
                    Pagina ${s} de ${i} &middot; ${n.length} jornada${n.length!==1?"s":""}
                </span>
                <div class="flex gap-2">
            ...
            </div>`,e.insertAdjacentElement("afterend",r),(d=document.getElementById("btn-pag-ant"))==null||d.addEventListener("click",()=>{this.paginaActual>1&&(this.paginaActual--,this.renderLista())}),(c=document.getElementById("btn-pag-sig"))==null||c.addEventListener("click",()=>{this.paginaActual<i&&(this.paginaActual++,this.renderLista())})}renderFila(e){const a=e.estado==="abierta",o=e.operadoresRelevo.length>0?e.operadoresRelevo.join(", "):e.operadorActual,n=this.todosLosTickets.filter(t=>t.operador&&e.operadoresRelevo.some(r=>t.operador===r)),l={};n.forEach(t=>{(t.pagos||[]).forEach(r=>{l[r.metodo]||(l[r.metodo]={usd:0,bs:0}),l[r.metodo].usd+=le(r.montoUsd),l[r.metodo].bs+=le(r.montoBs)})});const i=Object.entries(l).map(([t,r])=>`
            <div class="flex justify-between items-center text-xs border-b border-gray-100 last:border-none py-1">
                ...
                <div>
                    <span class="font-black">$ ${X(r.usd)}</span>
                    <span class="text-brand-purple font-mono font-bold ml-2">Bs. ${X(r.bs)}</span>
                </div>
            </div>`).join(""),s=n.length>0?n.map(t=>{const d=t.canal==="CONSUMO EN CUENTA"||t.ventaId.startsWith("CTA-")?'<span class="bg-amber-100 text-amber-900 border border-brand-black px-1.5 py-0.5 rounded font-black text-[9px]">CUENTA</span>':'<span class="bg-blue-100 text-blue-900 border border-brand-black px-1.5 py-0.5 rounded font-black text-[9px]">VENTA</span>';let c="";return t.pagos&&t.pagos.length>1?c='<span class="bg-amber-50 text-amber-950 border border-amber-500 px-1.5 py-0.5 rounded font-black text-[9px]">MIXTO</span>':t.pagos&&t.pagos.length===1?c=`<span class="bg-gray-100 text-brand-black border border-brand-black px-1.5 py-0.5 rounded font-black text-[9px]">${t.pagos[0].metodo}</span>`:c='<span class="bg-gray-100 text-gray-500 border border-gray-300 px-1.5 py-0.5 rounded font-mono text-[9px]">DIRECTO</span>',`
                <tr class="hover:bg-gray-50 transition-colors">
                    ...
                    <td class="py-1.5 px-2 text-right font-black text-brand-black text-[10px]">$ ${X(t.totalUsd)}</td>
                    <td class="py-1.5 px-2 text-right font-black text-brand-purple text-[10px]">Bs. ${X(t.totalBs)}</td>
        ...
        <div class="bg-white border-2 border-brand-black rounded-lg shadow-brutal-sm overflow-hidden">
            ...
            <div class="flex items-center gap-2 px-3 py-2.5 hover:bg-gray-50 transition-colors cursor-pointer select-none"
                ...
                <div class="flex items-center gap-3 shrink-0 ml-auto">
                    ...
                    <span class="font-heading font-black text-sm text-brand-black">$ ${X(e.ventasTotalUsd)}</span>
                    <span class="font-heading font-black text-sm text-brand-purple hidden sm:block">Bs. ${X(e.ventasTotalBs)}</span>
                </div>
            ...
            <div id="detalle-${e.id}" class="hidden border-t-2 border-brand-black">
                ...
                <div class="grid grid-cols-3 sm:grid-cols-6 divide-x divide-y sm:divide-y-0 divide-gray-200 border-b border-gray-200 text-center">
                    ...
                    <div class="p-2">
                        ...
                        <p class="font-bold text-xs text-brand-black mt-0.5">Bs. ${X(e.vueltoPagadoBs)}</p>
                    </div>
                    <div class="p-2">
                        <p class="text-[9px] font-black uppercase text-gray-400">Vuelto retenido</p>
                        <p class="font-bold text-xs text-brand-black mt-0.5">Bs. ${X(e.vueltoRetenidoBs)}</p>
                    </div>
                    <div class="p-2">
                        <p class="text-[9px] font-black uppercase text-gray-400">Deudas liquid.</p>
                        <p class="font-bold text-xs text-brand-black mt-0.5">$ ${X(e.deudasLiquidadasUsd)}</p>
                    </div>
                    <div class="p-2">
        ...
        </div>`}mostrarModalTicket(e){var l,i;const a=e.tasaDelDia?`Bs. ${X(e.tasaDelDia)}`:`Bs. ${X(this.modelo.tasaActual||807.39)}`,o=e.lineas&&e.lineas.length>0?e.lineas.map(s=>`
                <tr class="border-b border-gray-100 last:border-none">
                    <td class="py-2 pr-2">
                    ...
                    <td class="py-2 text-right font-mono">$ ${X(s.precioUsd)}</td>
                    <td class="py-2 text-right font-bold text-brand-black">$ ${X(s.subtotalUsd)}</td>
                    <td class="py-2 text-right font-black text-brand-purple">Bs. ${X(s.subtotalBs)}</td>
                </tr>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-3 bg-gray-50 border border-brand-black rounded p-3 text-xs">
                        ...
                        <div>
                            ...
                            <span class="font-bold text-gray-900">Bs. ${X(e.montoRecibidoBs||e.totalBs)}</span>
                        </div>
                        <div>
                            <span class="text-[10px] text-gray-500 font-bold uppercase block">Vuelto (${e.estadoVuelto==="RETENIDO"?"Retenido":"Entregado"})</span>
                            <span class="font-bold ${e.estadoVuelto==="RETENIDO"?"text-blue-700":"text-green-700"}">Bs. ${X(e.vueltoBs||"0.00")}</span>
                        </div>
                    </div>
                    ...
                    <div class="mb-3 ${e.estadoVuelto==="RETENIDO"?"bg-blue-50 border-blue-500 text-blue-950":"bg-emerald-50 border-emerald-500 text-emerald-950"} border-2 rounded p-2.5 text-xs font-bold flex flex-wrap items-center justify-between gap-2">
                        <div>
                            ...
                            <span>
                                ${e.estadoVuelto==="RETENIDO"?"Asentado como saldo a favor retenido.":`Entregado via ${e.metodoVuelto||"Efectivo"}${e.montoVueltoUsd&&Number(e.montoVueltoUsd)>0&&e.tasaVuelto?` ($ ${X(e.montoVueltoUsd)} a Bs. ${X(e.tasaVuelto)})`:""}.`}
                            </span>
                        </div>
                        <span class="font-heading font-black text-lg shrink-0">Bs. ${X(e.vueltoBs)}</span>
                    </div>`:""}
                            ...
                            ${e.pagos&&e.pagos.length>0?e.pagos.map(s=>`
                                    <div class="bg-white border border-brand-black rounded p-2 text-xs flex justify-between items-center">
                                        <div>
                                            ...
                                            ${s.tasaCambio?`<p class="text-[10px] font-mono font-bold text-brand-purple">Tasa: Bs. ${X(s.tasaCambio)}</p>`:""}
                                            ${s.referencia?`<p class="text-[10px] font-mono text-gray-500">Ref: ${s.referencia}</p>`:""}
                                        </div>
                                        <div class="text-right">
                                            <span class="font-black text-brand-black block">$ ${X(s.montoUsd)}</span>
                                            <span class="text-[10px] font-mono font-bold text-brand-purple block">Bs. ${X(s.montoBs)}</span>
                                        </div>
                                    </div>
                    ...
                    <div class="mb-4 bg-emerald-50 border-2 border-emerald-600 rounded p-3 text-xs text-emerald-950 font-bold flex items-center justify-between">
                        ...
                        <span class="font-heading font-black text-lg text-emerald-700 ml-4 shrink-0">+$${X(e.saldoAFavorConsolidadoUsd)} USD</span>
                    </div>`:""}
                ...
                <div>
                    <div class="border-t-2 border-brand-black pt-3 flex flex-wrap justify-between items-center gap-3">
                        <div>
                            ...
                            <div class="flex items-baseline gap-2">
                                <span class="font-heading font-black text-2xl text-brand-black">$ ${X(e.totalUsd)}</span>
                                <span class="font-heading font-black text-xl text-brand-purple font-mono">Bs. ${X(e.totalBs)}</span>
                            </div>
                        </div>
        ...
        </div>`;const a=()=>{this.modalRoot.innerHTML=""};(n=document.getElementById("pin-cancelar"))==null||n.addEventListener("click",a);const o=()=>void(async()=>{var r;const s=document.getElementById("pin-input").value;await D.validarPin(s).catch(()=>!1)?(a(),await this.abrirPanel()):(r=document.getElementById("pin-error"))==null||r.classList.remove("hidden")})();(l=document.getElementById("pin-ok"))==null||l.addEventListener("click",o),(i=document.getElementById("pin-input"))==null||i.addEventListener("keydown",s=>{s.key==="Enter"&&o()})}async abrirPanel(){this.marcarActivo("panel");const e=new Ge(this.root,this.modalRoot,this.panelVm,this.modelo);try{await e.render()}catch(a){this.toastError(a instanceof Error?a.message:String(a))}}async abrirModalQrMovil(){var o,n,l;const e=await D.dispositivos();this.modalRoot.innerHTML=`
        <div class="fixed inset-0 bg-black/40 flex items-center justify-center z-[100] p-4">
            <div class="bg-white border-2 border-brand-black rounded-lg shadow-brutal p-6 w-full max-w-lg">
                <div class="flex justify-between items-center border-b-2 border-brand-black pb-3 mb-4">
                    <div>
        ...
        </div>`;const a=()=>{this.modalRoot.innerHTML=""};(o=document.getElementById("qr-cerrar"))==null||o.addEventListener("click",a),(n=document.getElementById("qr-cerrar-btn"))==null||n.addEventListener("click",a),(l=document.getElementById("form-vincular-dev"))==null||l.addEventListener("submit",async i=>{i.preventDefault();const s=document.getElementById("dev-nombre");s!=null&&s.value.trim()&&(await D.registrarDispositivo(s.value.trim()),this.abrirModalQrMovil())}),this.modalRoot.querySelectorAll("[data-revocar-dev]").forEach(i=>{i.addEventListener("click",async()=>{const s=i.dataset.revocarDev;s&&(await D.revocarDispositivo(s),this.abrirModalQrMovil())})})}toastError(e){var a;this.modalRoot.innerHTML=`
