...
Esta tasa afectará todos los nuevos cobros y tickets a partir de este momento.`)){const u=await z("fijar_tasa_manual",{tasa:n.toFixed(2)});u&&this.pintar(u),o()}})}async actualizar(){const e=await this.obtenerTasa("obtener_tasa_bcv");e&&this.pintar(e)}async forzar(){const e=document.getElementById("bcv-refresh");e&&e.classList.add("animate-spin");const r=await this.obtenerTasa("forzar_actualizacion_tasa");e&&e.classList.remove("animate-spin"),r&&this.pintar(r)}async obtenerTasa(e){try{return await z(e)}catch{return null}}pintar(e){const r=document.getElementById("bcv-valor"),a=document.getElementById("bcv-fluctuacion");if(!r||!a)return;const o=Number(e.valor);r.textContent=Number.isFinite(o)&&o>0?o.toLocaleString("es-VE",{minimumFractionDigits:2,maximumFractionDigits:2}):"--",Number.isFinite(o)&&o>0&&window.dispatchEvent(new CustomEvent("tasa_actualizada",{detail:o}));const d=Number(e.fluctuacionPct);if(e.direccion&&Number.isFinite(d)){const i=e.direccion==="subio"?et:e.direccion==="bajo"?tt:at,p=d>0?"+":"";a.textContent=`${p}${d.toFixed(2)}%`,a.style.color=i}else a.textContent=""}}class nt{constructor(){T(this,"config",null);T(this,"rol","caja");T(this,"tasa",0);T(this,"oyentes",[])}suscribir(e){this.oyentes.push(e)}notificar(){this.oyentes.forEach(e=>e())}async cargarConfig(){return this.config=await D.config(),this.config}getConfig(){return this.config}async inicializar(e,r,a,o,d){await D.inicializar(e,r,a,o,d),await this.cargarConfig(),this.notificar()}get rubroAbasto(){return!0}tieneRubro(e){return!0}tieneCapacidad(e){var r;return(r=this.config)!=null&&r.capacidades?(this.config.capacidades&e)!==0:!0}setRol(e){this.rol=e,this.notificar()}get rolActual(){return this.rol}setTasa(e){this.tasa=e,this.notificar()}get tasaActual(){return this.tasa}bs(e){return(Number(e)*this.tasa).toLocaleString("es-VE",{minimumFractionDigits:2,maximumFractionDigits:2})}hasPinSet(){var e;return!!((e=this.config)!=null&&e.tienePin)}async verificarPin(e){return D.validarPin(e)}}class st{constructor(e,r,a){T(this,"contenedor");T(this,"modelo");T(this,"alTerminar");T(this,"modo","nuevo");T(this,"bloquearPanel",!0);this.contenedor=e,this.modelo=r,this.alTerminar=a}render(){this.contenedor.innerHTML=`
        ...
        `;const o=i=>{a.remove(),r(i)};a.querySelectorAll("[data-dialog-cancel]").forEach(i=>i.addEventListener("click",()=>o(!1))),(d=a.querySelector("[data-dialog-confirm]"))==null||d.addEventListener("click",()=>o(!0)),document.body.appendChild(a)})}function be(b,e="",r="SOLICITUD DE DATO"){return new Promise(a=>{var p;const o=document.createElement("div");o.className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-[120]",o.innerHTML=`
            <div class="bg-white border-2 border-brand-black rounded-lg shadow-brutal max-w-md w-full p-5 sm:p-6">
                <div class="flex justify-between items-center border-b-2 border-brand-black pb-2 mb-3">
        ...
        `;const d=o.querySelector("#dialog-input-val"),i=l=>{o.remove(),a(l)};o.querySelectorAll("[data-dialog-cancel]").forEach(l=>l.addEventListener("click",()=>i(null))),(p=o.querySelector("[data-dialog-accept]"))==null||p.addEventListener("click",()=>{i((d==null?void 0:d.value)??"")}),d==null||d.addEventListener("keydown",l=>{l.key==="Enter"&&i(d.value),l.key==="Escape"&&i(null)}),document.body.appendChild(o),d==null||d.focus(),d==null||d.select()})}const Me="America/Caracas";function he(b){if(!b||b<=0)return"-";const e=new Date(b*1e3);return new Intl.DateTimeFormat("es-VE",{timeZone:Me,year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit",second:"2-digit",hour12:!1}).format(e)}function Ue(b,e=!1){return new Intl.DateTimeFormat("es-VE",{timeZone:Me,year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit",...e?{second:"2-digit"}:{},hour12:!1}).format(b)}function it(b){if(!b||b<=0)return"Hoy";const e=Math.floor(Date.now()/1e3),r=Math.max(0,e-b);if(r<3600)return"Hoy (reciente)";const a=Math.floor(r/3600);if(a<24)return`Hoy (hace ${a}h)`;const o=Math.floor(a/24);return o===1?"Hace 1 día":`Hace ${o} días`}const ie=b=>X(b).toLocaleString("es-VE",{minimumFractionDigits:2,maximumFractionDigits:2});async function qe(b){var n,c,u;const[e,r]=await Promise.all([D.obtenerJornadaActual(),D.ventas()]);if(!e){b.innerHTML=`
        ...
        </div>`,(n=b.querySelector("#btn-cerrar-historial-turno"))==null||n.addEventListener("click",()=>{b.innerHTML=""});return}const a=e.inicioUnix,o=r.filter(m=>(m.fechaUnix||0)>=a);let d=0,i=0,p={},l={};o.forEach(m=>{const g=X(m.totalUsd),y=X(m.totalBs);d+=g,i+=y;const h=m.operador||e.operadorActual||"Cajero";l[h]||(l[h]={usd:0,bs:0,tickets:0}),l[h].usd+=g,l[h].bs+=y,l[h].tickets+=1,Array.isArray(m.pagos)&&m.pagos.length>0&&m.pagos.forEach(w=>{const f=w.metodo||"EFECTIVO";p[f]||(p[f]={usd:0,bs:0}),p[f].usd+=X(w.montoUsd),p[f].bs+=X(w.montoBs)})});const t=e.operadoresActivos&&e.operadoresActivos.length>0?e.operadoresActivos.join(", "):e.operadorActual||"Sin asignar";b.innerHTML=`
    <div class="fixed inset-0 bg-black/40 flex items-center justify-center z-[100] p-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
                <div class="border-2 border-brand-black rounded p-3 bg-white">
                    ...
                    <div class="space-y-1.5 max-h-36 overflow-y-auto pr-1">
                        ${Object.keys(l).length===0?'<p class="text-xs text-gray-400 italic">Sin transacciones registradas por operadores.</p>':Object.entries(l).map(([m,g])=>`
        ...
        <div class="fixed inset-0 bg-black/40 flex items-center justify-center z-[100] p-4">
            ...
            <div class="bg-emerald-50 border-2 border-emerald-600 rounded p-2.5 text-center mb-3">
                ...
                </div>`,"max-w-xl"),m()},m=()=>{var E,L,N,j,F,M,K,P,Z;const f=()=>{this.pagosBorrador=i,this.resolucionVueltoBorrador={estado:p,metodo:l,tasa:t},this.cerrarModal()};this.modal.querySelectorAll("[data-cerrar]").forEach(k=>k.addEventListener("click",f)),(E=this.modal.querySelector("[data-cancelar]"))==null||E.addEventListener("click",f),(L=this.modal.querySelector("#btn-volver-caja"))==null||L.addEventListener("click",()=>{this.pagosBorrador=i,this.resolucionVueltoBorrador={estado:p,metodo:l,tasa:t},this.cerrarModal();const k=document.getElementById("caja-buscar");k==null||k.focus()}),(N=this.modal.querySelector("#btn-vuelto-pagado"))==null||N.addEventListener("click",()=>{p="PAGADO",this.resolucionVueltoBorrador={estado:p,metodo:l,tasa:t},u()}),(j=this.modal.querySelector("#btn-vuelto-retenido"))==null||j.addEventListener("click",()=>{p="RETENIDO",this.resolucionVueltoBorrador={estado:p,metodo:l,tasa:t},u()}),(F=this.modal.querySelector("#select-metodo-vuelto"))==null||F.addEventListener("change",k=>{l=k.target.value,this.resolucionVueltoBorrador={estado:p,metodo:l,tasa:t},u()});const S=this.modal.querySelector("#input-tasa-vuelto");S&&(S.addEventListener("input",k=>{const $=X(k.target.value);t=$>0?$:a,this.resolucionVueltoBorrador={estado:p,metodo:l,tasa:t}}),S.addEventListener("blur",()=>{u()})),(M=this.modal.querySelector("#btn-reset-tasa-vuelto"))==null||M.addEventListener("click",()=>{t=a,this.resolucionVueltoBorrador={estado:p,metodo:l,tasa:t},u()}),this.modal.querySelectorAll("select[data-pago-metodo]").forEach(k=>{k.addEventListener("change",$=>{const U=k.dataset.pagoMetodo,H=$.target.value,G=i.find(W=>W.id===U);if(G){const W=e.find(Y=>Y.nombre===H),J=(W==null?void 0:W.moneda)||"BS";G.moneda=J,G.tasaCambio=a,i.length===1?G.monto=G.moneda==="USD"?r:o:J!==G.moneda&&(J==="USD"?G.monto=Number((a>0?G.monto/a:0).toFixed(2)):G.monto=Number((G.monto*a).toFixed(2))),G.metodo=H,this.pagosBorrador=i,u()}})});const B=()=>{var H,G,W,J;const k=n(),$=this.modal.querySelector("#contenedor-estado-balance");if($){$.innerHTML=c(k),(H=this.modal.querySelector("#btn-vuelto-pagado"))==null||H.addEventListener("click",()=>{p="PAGADO",this.resolucionVueltoBorrador={estado:p,metodo:l,tasa:t},u()}),(G=this.modal.querySelector("#btn-vuelto-retenido"))==null||G.addEventListener("click",()=>{p="RETENIDO",this.resolucionVueltoBorrador={estado:p,metodo:l,tasa:t},u()}),(W=this.modal.querySelector("#select-metodo-vuelto"))==null||W.addEventListener("change",O=>{l=O.target.value,this.resolucionVueltoBorrador={estado:p,metodo:l,tasa:t},u()});const Y=this.modal.querySelector("#input-tasa-vuelto");Y&&(Y.addEventListener("input",O=>{const _=X(O.target.value);t=_>0?_:a,this.resolucionVueltoBorrador={estado:p,metodo:l,tasa:t}}),Y.addEventListener("blur",()=>u())),(J=this.modal.querySelector("#btn-reset-tasa-vuelto"))==null||J.addEventListener("click",()=>{t=a,this.resolucionVueltoBorrador={estado:p,metodo:l,tasa:t},u()})}const U=this.modal.querySelector("#btn-confirmar-cobro");U&&(U.disabled=!k.puedeConfirmar)},A=k=>{const $=this.modal.querySelector(`div[data-aporte-info="${k.id}"]`);if(!$)return;const U=k.tasaCambio>0?k.tasaCambio:a,H=k.moneda==="USD"?k.monto*U:k.monto,G=a>0?H/a:0;k.moneda==="USD"?$.innerHTML=`
            ...
            <button data-cerrar class="w-full bg-brand-black text-white font-heading font-black text-lg py-3 rounded border-2 border-brand-black shadow-brutal-sm hover:bg-gray-800">
                LISTO
            </button>`,"max-w-md")}renderDinamico(){const e=this.contenedor.querySelector("#caja-grid");if(!e)return;const r=this.semaforoStock;e.innerHTML=this.vm.visibles.map(i=>{const p=(i.capacidades&fe)!==0,l=Number(i.stock),t=l<=0,s=i.sinStock?"text-purple-700":l<=r.rojoMax?"text-red-600":l<=r.amarilloMax?"text-amber-600":"text-emerald-700",n=i.unidad?i.unidad.toLowerCase()==="un"?"un.":i.unidad.toLowerCase():p?"kg":"un.",c=n==="kg"?"kg":n==="ml"?"ml":"un.";let u="";if(i.sinStock)u="LIBRE";else if(t)u="AGOTADO";else if(i.esCaja&&i.unidadesPorCaja&&i.unidadesPorCaja>1){const y=Math.floor(l/i.unidadesPorCaja),h=l%i.unidadesPorCaja;u=`${y} cajas y ${h} un. (${l} un.)`}else u=`${l} ${n}`;const m=i.precioPaqueteUsd&&i.nombrePaquete||i.esCaja&&i.unidadesPorCaja&&i.unidadesPorCaja>1,g=i.presentaciones&&i.presentaciones.length>0?i.presentaciones.map(y=>`
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
            ...
            <div class="bg-white border-2 border-brand-black rounded-lg shadow-brutal p-4 sm:p-6 flex flex-col justify-between">
                ...
                <div class="mt-4 pt-3 border-t border-brand-black">
                    ...
                    `).join("")),this.mostrarToast("Copia de respaldo generada exitosamente.","success")}catch(C){this.mostrarToast("Error generando respaldo: "+(C instanceof Error?C.message:String(C)),"error")}finally{c.disabled=!1,c.textContent=A||"+ COPIA"}});const u=async()=>{var H,G,W;const A=await D.obtenerJornadaActual(),C=await D.listarOperadores(),v=await D.obtenerSemaforoStock(),E=document.getElementById("jornada-estado-texto"),L=document.getElementById("jornada-fechas-texto"),N=document.getElementById("jornada-operador-activo"),j=document.getElementById("jornada-balance-texto"),F=document.getElementById("jornada-tickets-texto"),M=document.getElementById("box-acciones-jornada"),K=document.getElementById("lista-operadores-chips"),P=(A==null?void 0:A.estado)==="abierta";E&&(E.textContent=P?"Jornada en Curso":"Jornada Cerrada"),L&&(L.textContent=P?`Inicio: ${(A==null?void 0:A.inicioStr)||"Reciente"}`:`Último cierre: ${(A==null?void 0:A.finStr)||"Sin jornadas"}`);const Z=A!=null&&A.operadoresActivos&&A.operadoresActivos.length>0?A.operadoresActivos.join(", "):((H=A==null?void 0:A.operadorActual)==null?void 0:H.trim())||"Sin operador asignado";N&&(N.textContent=Z),j&&(j.textContent=`$ ${ae((A==null?void 0:A.ventasTotalUsd)||0)} USD (Bs. ${ae((A==null?void 0:A.ventasTotalBs)||0)})`),F&&(F.textContent=`${(A==null?void 0:A.ticketsEmitidos)||0} tickets emitidos · ${(A==null?void 0:A.entradasStockReg)||0} reposiciones`),M&&(P?M.innerHTML=`
        ...
        <div class="border-2 border-brand-black rounded-lg shadow-brutal-sm p-4 ${o} flex flex-col justify-between min-w-0 overflow-hidden" title="${d}">
            ...
            ${d?`<div class="text-[10px] text-gray-500 font-bold truncate mt-0.5 border-t border-gray-200/60 pt-1">${d}</div>`:""}
                ...
                `,l+=w});const u=p.map((g,y)=>{const h=a[y%a.length],w=o?`${g.valor} un.`:`$ ${ae(g.valor)}`;return`
                <tr class="border-b border-gray-100 last:border-none">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6 items-start">
            ...
            <div class="lg:col-span-2">
                ${this.cuentaSeleccionada?(()=>{var y;const s=this.cuentaSeleccionada.tipo==="deuda",n=oe(this.cuentaSeleccionada.totalParcialUsd),c=oe(this.cuentaSeleccionada.abonosUsd),u=Math.max(0,n-c),m=Math.max(0,c-n),g=Pe(this.cuentaSeleccionada.fechaCreacionUnix);return`
            ...
            </div>`,m()},u=()=>{const g=l.moneda==="USD",y=g&&s>0?s:d,h=g?t*y:t,w=d>0?h/d:0,f=a+w,S=f-r,B=S*d;let A="";if(t<=1e-4)A=`
                    <div class="rounded p-2.5 border-2 border-brand-black bg-gray-50 text-xs text-gray-700 font-bold">
                        <div class="flex justify-between items-center">
                            <span>Aporte de este abono:</span>
                            <span class="font-heading font-black text-sm text-gray-800">$0.00 USD (Bs. 0.00)</span>
        ...
        <div class="fixed inset-0 bg-black/40 flex items-center justify-center z-[100] p-4">
            <div class="bg-white border-2 border-brand-black rounded-lg shadow-brutal p-6 w-full max-w-md">
                ...
                <div class="grid grid-cols-2 gap-3">
        ...
        </div>`;const o=this.modal.querySelector("#tab-edit-usd"),d=this.modal.querySelector("#tab-edit-bs"),i=this.modal.querySelector("#sec-edit-usd"),p=this.modal.querySelector("#sec-edit-bs"),l=this.modal.querySelector("#edit-abono-usd-val"),t=this.modal.querySelector("#edit-abono-bs-val"),s=this.modal.querySelector("#edit-usd-equiv"),n=this.modal.querySelector("#edit-bs-equiv"),c=this.modal.querySelector("#edit-preview-box");let u="usd";const m=()=>{let S=0;u==="usd"?(S=oe(l==null?void 0:l.value),s&&(s.textContent=`Bs. ${(S*a).toFixed(2)}`)):(S=oe(t==null?void 0:t.value)/a,n&&(n.textContent=`$${S.toFixed(2)} USD`)),Number.isFinite(S)||(S=0);const B=S-e;if(c)if(B>.001)c.className="rounded p-2.5 border-2 border-emerald-600 bg-emerald-50 text-xs text-emerald-950 font-bold",c.innerHTML=`
                        <div class="flex justify-between items-center mb-1">
                    ...
                    `};l&&Se(l,5e4,8,m),t&&Se(t,5e7,11,m),m(),(y=this.modal.querySelector("#btn-preset-cero"))==null||y.addEventListener("click",async()=>{const S=oe(u==="usd"?l==null?void 0:l.value:t==null?void 0:t.value);(r>0||S>0)&&!await ue("¿Desea restablecer el saldo abonado a $0.00? Esta acción dejará los pagos registrados en cero.","RESTABLECER ABONO")||(l&&(l.value="0.00"),t&&(t.value="0.00"),m())}),(h=this.modal.querySelector("#btn-preset-exacto"))==null||h.addEventListener("click",()=>{l&&(l.value=e.toFixed(2)),t&&(t.value=(e*a).toFixed(2)),m()}),o==null||o.addEventListener("click",()=>{u="usd",o.className="flex-1 py-1.5 font-heading font-black text-xs rounded bg-brand-black text-white",d.className="flex-1 py-1.5 font-heading font-bold text-xs rounded text-brand-black hover:bg-white",i==null||i.classList.remove("hidden"),p==null||p.classList.add("hidden"),m()}),d==null||d.addEventListener("click",()=>{u="bs",d.className="flex-1 py-1.5 font-heading font-black text-xs rounded bg-brand-black text-white",o.className="flex-1 py-1.5 font-heading font-bold text-xs rounded text-brand-black hover:bg-white",p==null||p.classList.remove("hidden"),i==null||i.classList.add("hidden"),m()});const g=()=>{this.modal.innerHTML=""};(w=this.modal.querySelector("#modal-edit-cancel"))==null||w.addEventListener("click",g),(f=this.modal.querySelector("#modal-edit-ok"))==null||f.addEventListener("click",async()=>{if(!this.cuentaSeleccionada)return;let S=0;if(u==="usd"?S=oe(l==null?void 0:l.value):S=oe(t==null?void 0:t.value)/a,!Number.isFinite(S)||S<0||S>5e4){this.mostrarToast("Monto inválido. Ingrese un valor entre $0.00 y $50,000.00 USD.","error");return}if(Math.abs(S-r)>.009){const C=S===0?`¿Confirmas reiniciar el saldo abonado a $0.00? (Monto previo registrado: $${R(r)})`:`¿Confirmas modificar el saldo abonado de esta cuenta a $${R(S)} USD (Bs. ${R(S*a)})? (Monto previo registrado: $${R(r)})`;if(!await ue(C,"MODIFICAR ABONO"))return}const A=await D.editarAbonoCuenta(this.cuentaSeleccionada.ventaId,S);A&&(this.cuentaSeleccionada=A),g(),this.render()})}async modalCierre(){var A,C,v;if(!this.cuentaSeleccionada)return;const e=this.cuentaSeleccionada.tipo==="deuda",r=oe(this.cuentaSeleccionada.totalParcialUsd),a=oe(this.cuentaSeleccionada.abonosUsd),o=Math.max(0,Math.round((r-a)*100)/100),d=Math.max(0,Math.round((a-r)*100)/100),i=this.modelo.tasaActual||807.39;if(o<=.009){this.modal.innerHTML=`
            <div class="fixed inset-0 bg-black/40 flex items-center justify-center z-[100] p-4">
                <div class="bg-white border-2 border-brand-black rounded-lg shadow-brutal p-6 w-full max-w-md">
            ...
            <div class="bg-emerald-50 border-2 border-emerald-600 rounded p-2.5 text-center mb-3">
                <p class="text-xs font-black uppercase text-emerald-800">Total Exactamente Cubierto</p>
                <p class="text-xs text-emerald-700 font-bold">Importe liquidado sin diferencia pendiente.</p>
            </div>`},S=()=>{var M,K;const E=w(),L=p.filter(P=>P.moneda==="USD"),N=p.filter(P=>P.moneda==="BS"),j=c.map((P,Z)=>{const k=L.map(J=>`<option value="${J.nombre}" ${J.nombre===P.metodo?"selected":""}>${J.nombre} ($ - Tasa Dinámica)</option>`).join(""),U=`
                        <optgroup label="MÉTODOS EN BOLÍVARES (Bs.)">
        ...
        <div id="inv-seccion-stock" class="${this.tabActiva==="stock"?"block":"hidden"} space-y-4">
            <div class="bg-white border-2 border-brand-black rounded-lg shadow-brutal p-4 sm:p-6">
                ...
                <div class="flex items-center gap-1.5 overflow-x-auto pb-2 mb-3 max-w-full" id="repo-chips-categorias">
                    ...
                    ${this.categorias.map(n=>{const c=i[n.id]||0,u=this.filtroCategoriaRepo===n.id;return`
...
`),l=new Blob(["\uFEFF"+p],{type:"text/csv;charset=utf-8;"}),t=URL.createObjectURL(l),s=document.createElement("a");s.href=t,s.download=`jornada_${b.id}.csv`,document.body.appendChild(s),s.click(),document.body.removeChild(s),URL.revokeObjectURL(t)};class xt{constructor(e,r,a){T(this,"contenedor");T(this,"modalRoot");T(this,"modelo");T(this,"jornadas",[]);T(this,"jornadaActual",null);T(this,"todosLosTickets",[]);T(this,"busqueda","");T(this,"expandidos",new Set);T(this,"paginaActual",1);T(this,"POR_PAGINA",20);this.contenedor=e,this.modalRoot=r,this.modelo=a}parseFechaUnix(e){if(!e)return 0;const r=e.match(/(\d{4})-(\d{2})-(\d{2})[T\s](\d{2}):(\d{2}):(\d{2})/);if(r&&r[1]&&r[2]&&r[3]&&r[4]&&r[5]&&r[6]){const a=Date.UTC(parseInt(r[1],10),parseInt(r[2],10)-1,parseInt(r[3],10),parseInt(r[4],10),parseInt(r[5],10),parseInt(r[6],10));if(!isNaN(a))return Math.floor(a/1e3)}return 0}async render(){const[e,r,a]=await Promise.all([D.obtenerJornadaActual(),D.listarHistoricoJornadas(),D.ventas()]);this.jornadaActual=e,this.jornadas=r,this.todosLosTickets=a;const o=e?r.filter(t=>t.id!==e.id):r,d=[...e?[e]:[],...o],i=d.length,p=d.reduce((t,s)=>t+ye(s.ventasTotalUsd),0),l=d.reduce((t,s)=>t+s.ticketsEmitidos,0);this.contenedor.innerHTML=`
        ...
        <!-- Lista -->
        <div id="lista-jornadas" class="flex flex-col gap-2"></div>`,this.conectarEventosBuscador(),this.renderLista()}conectarEventosBuscador(){var e;(e=document.getElementById("jornadas-buscar"))==null||e.addEventListener("input",r=>{this.busqueda=r.target.value,this.paginaActual=1,this.renderLista()})}renderLista(){var n,c;const e=document.getElementById("lista-jornadas");if(!e)return;const r=this.jornadaActual?this.jornadas.filter(u=>{var m;return u.id!==((m=this.jornadaActual)==null?void 0:m.id)}):this.jornadas,a=[...this.jornadaActual?[this.jornadaActual]:[],...r],o=this.busqueda.trim().toLowerCase(),d=o?a.filter(u=>u.id.toLowerCase().includes(o)||u.operadorActual.toLowerCase().includes(o)||u.operadoresRelevo.some(m=>m.toLowerCase().includes(o))||u.inicioStr.toLowerCase().includes(o)||(u.finStr||"").toLowerCase().includes(o)):a,i=document.getElementById("paginacion-jornadas");if(i&&i.remove(),d.length===0){e.innerHTML=`
            ...
            </div>`;return}const p=Math.max(1,Math.ceil(d.length/this.POR_PAGINA));this.paginaActual>p&&(this.paginaActual=p),this.paginaActual<1&&(this.paginaActual=1);const l=this.paginaActual,t=d.slice((l-1)*this.POR_PAGINA,l*this.POR_PAGINA);e.innerHTML=t.map(u=>this.renderFila(u)).join(""),t.forEach(u=>{if(this.expandidos.has(u.id)){const m=document.getElementById(`detalle-${u.id}`);m==null||m.classList.remove("hidden");const g=document.getElementById(`icon-${u.id}`);g&&(g.textContent="▲")}}),e.querySelectorAll("[data-toggle-jornada]").forEach(u=>{u.addEventListener("click",()=>{const m=u.dataset.toggleJornada,g=document.getElementById(`detalle-${m}`),y=document.getElementById(`icon-${m}`);if(g){const h=g.classList.contains("hidden");g.classList.toggle("hidden",!h),y&&(y.textContent=h?"▲":"▼"),h?this.expandidos.add(m):this.expandidos.delete(m)}})}),e.querySelectorAll("[data-exportar-jornada]").forEach(u=>{u.addEventListener("click",m=>{m.stopPropagation();const g=u.dataset.exportarJornada,y=d.find(h=>h.id===g);y&&mt(y,this.todosLosTickets)})}),e.querySelectorAll("[data-ver-ticket]").forEach(u=>{u.addEventListener("click",()=>{const m=u.dataset.verTicket,g=this.todosLosTickets.find(y=>y.ventaId===m);g&&this.mostrarModalTicket(g)})});const s=document.createElement("div");s.id="paginacion-jornadas",s.innerHTML=`
            ...
            <div id="detalle-${e.id}" class="hidden border-t-2 border-brand-black">
                ...
                <div class="grid grid-cols-3 sm:grid-cols-6 divide-x divide-y sm:divide-y-0 divide-gray-200 border-b border-gray-200 text-center">
                    <div class="p-2">
                        ...
                        <p class="font-bold text-xs text-brand-black mt-0.5">${Fe(e)}</p>
                    </div>
                    <div class="p-2">
        ...
        </div>`;const o=()=>{this.modalRoot.innerHTML=""};(d=document.getElementById("modal-ticket-cerrar"))==null||d.addEventListener("click",o),(i=document.getElementById("modal-ticket-ok"))==null||i.addEventListener("click",o)}}class gt{constructor(e){T(this,"contenedor");this.contenedor=e}render(){this.contenedor.innerHTML=`
        ...
        </div>`}}const ft=3e5;class ht{constructor(){T(this,"modelo",new nt);T(this,"cajaVm",null);T(this,"panelVm",new lt);T(this,"widget",new rt);T(this,"root");T(this,"modalRoot")}async arrancar(){const e=document.getElementById("app-root");if(!e)return;this.root=e,this.modalRoot=this.crearModalRoot(),this.widget.iniciar(),window.addEventListener("tasa_actualizada",a=>{const o=a;o.detail&&this.modelo.setTasa(o.detail)}),await this.refrescarTasa();let r=null;try{r=await this.modelo.cargarConfig()}catch{r=await this.modelo.cargarConfig()}if(window.setInterval(()=>void this.refrescarTasa(),ft),!r){new st(this.root,this.modelo,()=>void this.arrancar()).render();return}this.pintarBotonesRol(r),this.ejecutarBackupAutomatico(),await this.arrancarCaja()}crearModalRoot(){let e=document.getElementById("modal-root");return e||(e=document.createElement("div"),e.id="modal-root",document.body.appendChild(e)),e}async refrescarTasa(){try{const e=await D.tasa();e&&this.modelo.setTasa(Number(e.valor))}catch{}}async ejecutarBackupAutomatico(){try{const e=await D.getBackupDir();if(e){const r=await D.autoBackup(e,5);r&&console.log(`[Backup] Auto-backup generado: ${r.totalRegistros} registros`)}}catch(e){console.warn("[Backup] Auto-backup falló:",e)}}pintarBotonesRol(e){const r=[document.getElementById("nav-actions-desktop"),document.getElementById("nav-actions-mobile")].filter(Boolean);r.length===0||document.querySelector('[data-nav-btn="caja"]')||r.forEach(a=>{a.innerHTML="";const o=(n,c,u)=>{const m=document.createElement("button");return m.id=`${n}-${a.id.includes("mobile")?"mob":"dsk"}`,m.dataset.navBtn=u,m.className="font-heading font-black text-xs sm:text-sm bg-white border-2 border-brand-black px-2.5 sm:px-3.5 py-1.5 sm:py-2 rounded shadow-brutal-sm hover:-translate-y-0.5 transition-transform shrink-0",m.textContent=c,m},d=o("btn-ir-caja","CAJA","caja"),i=o("btn-ir-cuentas","CUENTAS","cuentas"),p=o("btn-ir-ventas","VENTAS","ventas"),l=o("btn-ir-inventario","INVENTARIO","inventario"),t=o("btn-ir-panel","PANEL","panel"),s=o("btn-ir-guia","GUIA","guia");a.appendChild(d),a.appendChild(i),a.appendChild(p),a.appendChild(l),a.appendChild(t),a.appendChild(s),d.addEventListener("click",()=>void this.arrancarCaja()),i.addEventListener("click",()=>void this.arrancarCuentas()),p.addEventListener("click",()=>void this.arrancarVentas()),l.addEventListener("click",()=>void this.arrancarInventario()),t.addEventListener("click",()=>this.solicitarAccesoPanel()),s.addEventListener("click",()=>this.arrancarGuia())})}arrancarGuia(){this.marcarActivo("guia"),new gt(this.root).render()}async arrancarCaja(){this.marcarActivo("caja"),this.cajaVm=new dt(this.modelo),new ct(this.root,this.modalRoot,this.cajaVm,this.modelo).render();try{await this.cajaVm.cargar()}catch(r){this.toastError(r instanceof Error?r.message:String(r))}}async arrancarCuentas(){this.marcarActivo("cuentas");const e=new ut(this.root,this.modalRoot,this.modelo);try{await e.render()}catch(r){this.toastError(r instanceof Error?r.message:String(r))}}arrancarVentas(){var o,d,i;const e=this.modelo.getConfig();if(!(e!=null&&e.tienePin)){this.abrirVentas();return}this.modalRoot.innerHTML=`
        <div class="fixed inset-0 bg-black/40 flex items-center justify-center z-[100] p-4">
        ...
        </div>`,(r=document.getElementById("toast-close"))==null||r.addEventListener("click",()=>{this.modalRoot.innerHTML=""})}}document.addEventListener("DOMContentLoaded",()=>{document.addEventListener("contextmenu",b=>b.preventDefault()),new ht().arrancar().catch(b=>console.error(b))});
