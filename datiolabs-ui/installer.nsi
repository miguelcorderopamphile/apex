...
!define MUI_UNWELCOMEPAGE_TITLE "Desinstalar DatioLabs Retail"
!define MUI_UNWELCOMEPAGE_TEXT "Este asistente desinstalará DatioLabs Retail de su computadora.$\n$\nDurante el proceso podrá elegir si desea conservar o purgar totalmente su base de datos local."
...
Section "MainSection" SEC01
  ...
  CreateShortcut "$SMPROGRAMS\DatioLabs\DatioLabs Retail.lnk" "$INSTDIR\DatioLabs.exe" "" "$INSTDIR\DatioLabs.exe" 0
  CreateShortcut "$SMPROGRAMS\DatioLabs\Desinstalar.lnk" "$INSTDIR\uninstall.exe" "" "$INSTDIR\uninstall.exe" 0
  
  ; Acceso directo en Escritorio
  CreateShortcut "$DESKTOP\DatioLabs Retail.lnk" "$INSTDIR\DatioLabs.exe" "" "$INSTDIR\DatioLabs.exe" 0
SectionEnd
...
Function un.onUninstSuccess
  HideWindow
  MessageBox MB_ICONINFORMATION|MB_OK "DatioLabs Retail ha sido desinstalado correctamente de su equipo."
FunctionEnd
