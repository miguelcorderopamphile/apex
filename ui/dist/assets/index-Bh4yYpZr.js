...
Esta tasa afectará todos los nuevos cobros y tickets a partir de este momento.`)){const p=await G("fijar_tasa_manual",{tasa:s.toFixed(2)});p&&this.pintar(p),r()}})}async actualizar(){const e=await this.obtenerTasa("obtener_tasa_bcv");e&&this.pintar(e)}async forzar(){const e=document.getElementById("bcv-refresh");e&&e.classList.add("animate-spin");const o=await this.obtenerTasa("forzar_actualizacion_tasa");e&&e.classList.remove("animate-spin"),o&&this.pintar(o)}async obtenerTasa(e){try{return await G(e)}catch{return null}}pintar(e){const o=document.getElementById("bcv-valor"),a=document.getElementById("bcv-fluctuacion");if(!o||!a)return;const r=Number(e.valor);o.textContent=Number.isFinite(r)&&r>0?r.toLocaleString("es-VE",{minimumFractionDigits:2,maximumFractionDigits:2}):"--",Number.isFinite(r)&&r>0&&window.dispatchEvent(new CustomEvent("tasa_actualizada",{detail:r}));const i=Number(e.fluctuacionPct);if(e.direccion&&Number.isFinite(i)){const c=e.direccion==="subio"?Xe:e.direccion==="bajo"?Ye:Qe,b=i>0?"+":"";a.textContent=`${b}${i.toFixed(2)}%`,a.style.color=c}else a.textContent=""}}class tt{constructor(){U(this,"config",null);U(this,"rol","caja");U(this,"tasa",0);U(this,"oyentes",[])}suscribir(e){this.oyentes.push(e)}notificar(){this.oyentes.forEach(e=>e())}async cargarConfig(){return this.config=await D.config(),this.config}getConfig(){return this.config}async inicializar(e,o,a,r,i){await D.inicializar(e,o,a,r,i),await this.cargarConfig(),this.notificar()}get rubroAbasto(){var e,o;return(((e=this.config)==null?void 0:e.rubros)??0&Ce)!==0||(((o=this.config)==null?void 0:o.rubros)??0)===Ce}tieneRubro(e){var o;return((((o=this.config)==null?void 0:o.rubros)??0)&e)!==0}tieneCapacidad(e){var o;return((((o=this.config)==null?void 0:o.capacidades)??0)&e)!==0}setRol(e){this.rol=e,this.notificar()}get rolActual(){return this.rol}setTasa(e){this.tasa=e,this.notificar()}get tasaActual(){return this.tasa}bs(e){return(Number(e)*this.tasa).toLocaleString("es-VE",{minimumFractionDigits:2,maximumFractionDigits:2})}hasPinSet(){var e;return!!((e=this.config)!=null&&e.tienePin)}async verificarPin(e){return D.validarPin(e)}}function at(l,e){const o=l.replace(/[^a-zA-Z0-9]/g,"");if(o.length!==16)return!1;const a=o.split("").map(b=>parseInt(b,10)).filter(b=>!isNaN(b)&&b<10);if(a.length!==16)return!1;let r=0;for(let b=0;b<12;b++)r+=a[b]*(b+1);const i=r%1e4,c=a[12]*1e3+a[13]*100+a[14]*10+a[15];return i===c}class ot{constructor(e,o,a){U(this,"contenedor");U(this,"modelo");U(this,"alTerminar");U(this,"modo","nuevo");U(this,"bloquearPanel",!0);this.contenedor=e,this.modelo=o,this.alTerminar=a}render(){this.contenedor.innerHTML=`
        ...
        </div>`,(s=l.querySelector("#btn-cerrar-historial-turno"))==null||s.addEventListener("click",()=>{l.innerHTML=""});return}const a=e.inicioUnix,r=o.filter(x=>(x.fechaUnix||0)>=a);let i=0,c=0,b={},t={};r.forEach(x=>{const g=X(x.totalUsd),S=X(x.totalBs);i+=g,c+=S;const h=x.operador||e.operadorActual||"Cajero";t[h]||(t[h]={usd:0,bs:0,tickets:0}),t[h].usd+=g,t[h].bs+=S,t[h].tickets+=1,Array.isArray(x.pagos)&&x.pagos.length>0&&x.pagos.forEach(A=>{const f=A.metodo||"EFECTIVO";b[f]||(b[f]={usd:0,bs:0}),b[f].usd+=X(A.montoUsd),b[f].bs+=X(A.montoBs)})});const n=e.operadoresActivos&&e.operadoresActivos.length>0?e.operadoresActivos.join(", "):e.operadorActual||"Sin asignar";l.innerHTML=`
    <div class="fixed inset-0 bg-black/40 flex items-center justify-center z-[100] p-4">
        <div class="bg-white border-2 border-brand-black rounded-lg shadow-brutal p-5 sm:p-6 w-full max-w-3xl max-h-[90vh] overflow-y-auto flex flex-col">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
                ...
                <div class="border-2 border-brand-black rounded p-3 bg-white">
                    ...
                    <div class="space-y-1.5 max-h-36 overflow-y-auto pr-1">
                        ${Object.keys(b).length===0?'<p class="text-xs text-gray-400 italic">Sin pagos asentados.</p>':Object.entries(b).map(([x,g])=>`
                                <div class="flex justify-between items-center text-xs border-b border-gray-100 pb-1">
                ...
                <div class="space-y-1.5 overflow-y-auto max-h-56 pr-1">
                    ${r.length===0?'<p class="py-6 text-center text-xs text-gray-400 font-bold">No se han emitido tickets durante el turno actual.</p>':r.map(x=>`
                            <div class="flex items-center justify-between border border-brand-black bg-white rounded p-2 text-xs font-bold hover:bg-amber-50/50">
                                <div>
                                    <p class="font-mono text-brand-black">${x.ventaId.slice(0,10)} · <span class="text-[10px] text-gray-500">${$e(x.fechaUnix)}</span></p>
        ...
        <div class="fixed inset-0 bg-black/40 flex items-center justify-center z-[100] p-4">
            ...
            </div>`},p=()=>{const f=s(),E=e.filter(y=>y.moneda==="USD"),$=e.filter(y=>y.moneda==="BS"),k=c.map((y,v)=>{const C=E.map(O=>`<option value="${O.nombre}" ${O.nombre===y.metodo?"selected":""}>${O.nombre} ($ - Tasa Dinámica)</option>`).join(""),z=`
            ...
            <button data-cerrar class="w-full bg-brand-black text-white font-heading font-black text-lg py-3 rounded border-2 border-brand-black shadow-brutal-sm hover:bg-gray-800">
                LISTO
            </button>`,"max-w-md")}renderDinamico(){const e=this.contenedor.querySelector("#caja-grid");if(!e)return;const o=this.semaforoStock;e.innerHTML=this.vm.visibles.map(c=>{const b=(c.capacidades&fe)!==0,t=Number(c.stock),n=t<=0,d=c.sinStock?"text-purple-700":t<=o.rojoMax?"text-red-600":t<=o.amarilloMax?"text-amber-600":"text-emerald-700",s=c.unidad?c.unidad.toLowerCase()==="un"?"un.":c.unidad.toLowerCase():b?"kg":"un.",u=s==="kg"?"kg":s==="ml"?"ml":"un.";let p="";if(c.sinStock)p="LIBRE";else if(n)p="AGOTADO";else if(c.esCaja&&c.unidadesPorCaja&&c.unidadesPorCaja>1){const g=Math.floor(t/c.unidadesPorCaja),S=t%c.unidadesPorCaja;p=`${g} cajas y ${S} un. (${t} un.)`}else p=`${t} ${s}`;const x=c.precioPaqueteUsd&&c.nombrePaquete||c.esCaja&&c.unidadesPorCaja&&c.unidadesPorCaja>1;return`
                ...
                <table class="w-full text-left text-xs">
                    ...
                    <tfoot class="border-t-2 border-brand-black bg-gray-100 font-black font-heading text-xs">
                        <tr>
                            ...
                            <td class="py-2.5 px-3 text-right text-brand-black">$ ${ee(x)}</td>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
            ...
            <div class="bg-white border-2 border-brand-black rounded-lg shadow-brutal p-4 sm:p-6 flex flex-col justify-between">
                ...
                <div class="mt-4 pt-3 border-t border-brand-black">
                    ...
                    `).join("")),this.mostrarToast("Copia de respaldo generada exitosamente.","success")}catch(w){this.mostrarToast("Error generando respaldo: "+(w instanceof Error?w.message:String(w)),"error")}finally{u.disabled=!1,u.textContent=k||"+ COPIA"}});const p=async()=>{var K,J,Y;const k=await D.obtenerJornadaActual(),w=await D.listarOperadores(),y=await D.obtenerSemaforoStock(),v=document.getElementById("jornada-estado-texto"),C=document.getElementById("jornada-fechas-texto"),j=document.getElementById("jornada-operador-activo"),z=document.getElementById("jornada-balance-texto"),V=document.getElementById("jornada-tickets-texto"),H=document.getElementById("box-acciones-jornada"),_=document.getElementById("lista-operadores-chips"),O=(k==null?void 0:k.estado)==="abierta";v&&(v.textContent=O?"Jornada en Curso":"Jornada Cerrada"),C&&(C.textContent=O?`Inicio: ${(k==null?void 0:k.inicioStr)||"Reciente"}`:`Último cierre: ${(k==null?void 0:k.finStr)||"Sin jornadas"}`);const I=k!=null&&k.operadoresActivos&&k.operadoresActivos.length>0?k.operadoresActivos.join(", "):((K=k==null?void 0:k.operadorActual)==null?void 0:K.trim())||"Sin operador asignado";j&&(j.textContent=I),z&&(z.textContent=`$ ${ee((k==null?void 0:k.ventasTotalUsd)||0)} USD (Bs. ${ee((k==null?void 0:k.ventasTotalBs)||0)})`),V&&(V.textContent=`${(k==null?void 0:k.ticketsEmitidos)||0} tickets emitidos · ${(k==null?void 0:k.entradasStockReg)||0} reposiciones`),H&&(O?H.innerHTML=`
        ...
        <div class="border-2 border-brand-black rounded-lg shadow-brutal-sm p-4 ${r} flex flex-col justify-between min-w-0 overflow-hidden" title="${i}">
            ...
            ${i?`<div class="text-[10px] text-gray-500 font-bold truncate mt-0.5 border-t border-gray-200/60 pt-1">${i}</div>`:""}
                ...
                `,t+=A});const p=b.map((g,S)=>{const h=a[S%a.length],A=r?`${g.valor} un.`:`$ ${ee(g.valor)}`;return`
                <tr class="border-b border-gray-100 last:border-none">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6 items-start">
            ...
            <div class="space-y-3">
                ...
                <div id="cuentas-lista" class="space-y-2 max-h-[65vh] overflow-y-auto pr-1">
                    ${r.length===0?`<div class="bg-white border-2 border-brand-black rounded-lg p-6 text-center text-brand-text font-bold">
                        ...
                        </div>`:(()=>{const d=Math.ceil(r.length/this.POR_PAGINA_CUENTAS);this.paginaCuentas>d&&(this.paginaCuentas=d),this.paginaCuentas<1&&(this.paginaCuentas=1);const s=(this.paginaCuentas-1)*this.POR_PAGINA_CUENTAS;return r.slice(s,s+this.POR_PAGINA_CUENTAS).map(p=>{var k;const x=p.tipo==="deuda",g=ae(p.totalParcialUsd),S=ae(p.abonosUsd),h=Math.max(0,g-S),A=Math.max(0,S-g),f=((k=this.cuentaSeleccionada)==null?void 0:k.ventaId)===p.ventaId,E=Te(p.fechaCreacionUnix);let $="cursor-pointer border-2 border-brand-black rounded-lg p-3 sm:p-4 transition-all ";return f?$+=x?"bg-amber-50 text-brand-black border-l-[6px] border-l-amber-600 shadow-brutal ring-1 ring-amber-500/20":"bg-blue-50 text-brand-black border-l-[6px] border-l-blue-600 shadow-brutal ring-1 ring-blue-500/20":$+=x?"bg-stone-50 border-l-4 border-l-amber-300 text-brand-black shadow-brutal-sm hover:bg-amber-50/60 hover:-translate-y-0.5":"bg-stone-50 border-l-4 border-l-blue-200 text-brand-black shadow-brutal-sm hover:bg-blue-50/50 hover:-translate-y-0.5",`
                        <div data-cuenta-id="${p.ventaId}" class="${$}">
                            <div class="flex justify-between items-start gap-2">
                                ...
                                <div class="text-right shrink-0">
                                    ...
                                    ${A>0?`<p class="text-[11px] font-black text-emerald-700">A favor: +$${R(A)}</p>`:h>0?`<p class="text-xs font-bold ${x?"text-amber-900":"text-blue-900"}">Pend: $${R(h)}</p>`:'<p class="text-[11px] font-bold text-gray-400">Cubierta ($0,00)</p>'}
                                </div>
                            </div>
            ...
            <div class="lg:col-span-2">
                ${this.cuentaSeleccionada?(()=>{var S;const d=this.cuentaSeleccionada.tipo==="deuda",s=ae(this.cuentaSeleccionada.totalParcialUsd),u=ae(this.cuentaSeleccionada.abonosUsd),p=Math.max(0,s-u),x=Math.max(0,u-s),g=Te(this.cuentaSeleccionada.fechaCreacionUnix);return`
                <div class="bg-white border-2 border-brand-black rounded-lg shadow-brutal p-4 sm:p-6 flex flex-col space-y-4">
                    ...
                    <div class="border-t-2 border-brand-black pt-4 grid grid-cols-1 sm:grid-cols-${d?"3":"4"} gap-2 sm:gap-3">
                        ...
                        <button id="btn-cerrar-cuenta" class="${x>0?"bg-emerald-600 hover:bg-emerald-700":"bg-green-700 hover:bg-green-800"} text-white font-heading font-black text-xs py-3.5 rounded border-2 border-brand-black shadow-brutal active:translate-y-0.5">
                            ${d?"COBRAR Y LIQUIDAR DEUDA (TASA LIBRE)":x>0?`LIQUIDAR (+$${R(x)})`:"LIQUIDAR Y COBRAR"}
                        </button>
                    </div>
                </div>`})():`
            ...
            </div>`,x()},p=()=>{const g=t.moneda==="USD",S=g&&d>0?d:i,h=g?n*S:n,A=i>0?h/i:0,f=a+A,E=f-o,$=E*i;let k="";if(n<=1e-4)k=`
                    <div class="rounded p-2.5 border-2 border-brand-black bg-gray-50 text-xs text-gray-700 font-bold">
                        <div class="flex justify-between items-center">
                            <span>Aporte de este abono:</span>
                            <span class="font-heading font-black text-sm text-gray-800">$0.00 USD (Bs. 0.00)</span>
        ...
        <div class="fixed inset-0 bg-black/40 flex items-center justify-center z-[100] p-4">
        ...
        </div>`;const r=this.modal.querySelector("#tab-edit-usd"),i=this.modal.querySelector("#tab-edit-bs"),c=this.modal.querySelector("#sec-edit-usd"),b=this.modal.querySelector("#sec-edit-bs"),t=this.modal.querySelector("#edit-abono-usd-val"),n=this.modal.querySelector("#edit-abono-bs-val"),d=this.modal.querySelector("#edit-usd-equiv"),s=this.modal.querySelector("#edit-bs-equiv"),u=this.modal.querySelector("#edit-preview-box");let p="usd";const x=()=>{let E=0;p==="usd"?(E=ae(t==null?void 0:t.value),d&&(d.textContent=`Bs. ${(E*a).toFixed(2)}`)):(E=ae(n==null?void 0:n.value)/a,s&&(s.textContent=`$${E.toFixed(2)} USD`)),Number.isFinite(E)||(E=0);const $=E-e;if(u)if($>.001)u.className="rounded p-2.5 border-2 border-emerald-600 bg-emerald-50 text-xs text-emerald-950 font-bold",u.innerHTML=`
                        <div class="flex justify-between items-center mb-1">
            ...
            <div class="bg-emerald-50 border-2 border-emerald-600 rounded p-2.5 text-center mb-3">
                <p class="text-xs font-black uppercase text-emerald-800">Total Exactamente Cubierto</p>
                <p class="text-xs text-emerald-700 font-bold">Importe liquidado sin diferencia pendiente.</p>
            </div>`},E=()=>{var H,_;const v=A(),C=b.filter(O=>O.moneda==="USD"),j=b.filter(O=>O.moneda==="BS"),z=u.map((O,I)=>{const B=C.map(N=>`<option value="${N.nombre}" ${N.nombre===O.metodo?"selected":""}>${N.nombre} ($ - Tasa Dinámica)</option>`).join(""),T=`
                        <optgroup label="MÉTODOS EN BOLÍVARES (Bs.)">
                    ...
                    <div data-pago-card="${O.id}" class="border-2 border-brand-black rounded-lg p-3 bg-gray-50 mb-2.5">
                        ...
                        <div data-aporte-info="${O.id}" class="flex flex-wrap items-center justify-between gap-1 mt-1.5 text-[11px]">
                            <span class="font-bold text-brand-purple">Aporte: Bs. ${R(J)}</span>
            ...
            </div>`,$()},$=()=>{var _,O,I,B,P,T,K,J,Y,N,q;const v=()=>{this.cuentaSeleccionada&&(this.pagosBorradorPorCuenta.set(this.cuentaSeleccionada.ventaId,u),this.resolucionVueltoPorCuenta.set(this.cuentaSeleccionada.ventaId,{estado:x,metodo:g,tasa:S}))},C=()=>{v(),this.modal.innerHTML=""};(_=this.modal.querySelector("#modal-liq-cerrar"))==null||_.addEventListener("click",C),(O=this.modal.querySelector("#modal-liq-cancel"))==null||O.addEventListener("click",C),(I=this.modal.querySelector("#modal-liq-agregar-mas"))==null||I.addEventListener("click",()=>{v(),this.modal.innerHTML=""}),(B=this.modal.querySelector("#btn-vuelto-pagado-cuenta"))==null||B.addEventListener("click",()=>{x="PAGADO",v(),E()}),(P=this.modal.querySelector("#btn-vuelto-retenido-cuenta"))==null||P.addEventListener("click",()=>{x="RETENIDO",v(),E()}),(T=this.modal.querySelector("#select-metodo-vuelto-cuenta"))==null||T.addEventListener("change",L=>{g=L.target.value,v(),E()});const j=this.modal.querySelector("#input-tasa-vuelto-cuenta");if(j&&(j.addEventListener("input",L=>{const M=ae(L.target.value);S=M>0?M:t,v()}),j.addEventListener("blur",()=>{E()})),(K=this.modal.querySelector("#btn-reset-tasa-vuelto-cuenta"))==null||K.addEventListener("click",()=>{S=t,v(),E()}),e){const L=this.modal.querySelector("#deuda-tasa-input"),M=this.modal.querySelector("#btn-tasa-bcv");L&&Ae(L,1e5,9,()=>{const F=ae(L.value);t=Number.isFinite(F)&&F>0?F:c,v(),E()}),M==null||M.addEventListener("click",()=>{t=c,v(),E()})}this.modal.querySelectorAll("select[data-pago-metodo]").forEach(L=>{L.addEventListener("change",M=>{const F=L.dataset.pagoMetodo,te=M.target.value,Z=u.find(oe=>oe.id===F);if(Z){const oe=b.find(le=>le.nombre===te),de=(oe==null?void 0:oe.moneda)||"BS";Z.moneda=de,Z.tasaCambio=t,u.length===1?Z.monto=Z.moneda==="USD"?r:Number((r*t).toFixed(2)):de!==Z.moneda&&(de==="USD"?Z.monto=Number((t>0?Z.monto/t:0).toFixed(2)):Z.monto=Number((Z.monto*t).toFixed(2))),Z.metodo=te,v(),E()}})});const z=()=>{var te,Z,oe,de;const L=A(),M=this.modal.querySelector("#contenedor-estado-balance-cuenta");if(M){M.innerHTML=f(L),(te=this.modal.querySelector("#btn-vuelto-pagado-cuenta"))==null||te.addEventListener("click",()=>{x="PAGADO",v(),E()}),(Z=this.modal.querySelector("#btn-vuelto-retenido-cuenta"))==null||Z.addEventListener("click",()=>{x="RETENIDO",v(),E()}),(oe=this.modal.querySelector("#select-metodo-vuelto-cuenta"))==null||oe.addEventListener("change",he=>{g=he.target.value,v(),E()});const le=this.modal.querySelector("#input-tasa-vuelto-cuenta");le&&(le.addEventListener("input",he=>{const xe=ae(he.target.value);S=xe>0?xe:t,v()}),le.addEventListener("blur",()=>E())),(de=this.modal.querySelector("#btn-reset-tasa-vuelto-cuenta"))==null||de.addEventListener("click",()=>{S=t,v(),E()})}const F=this.modal.querySelector("#modal-liq-ok");F&&(F.disabled=!L.puedeConfirmar)},V=L=>{const M=this.modal.querySelector(`div[data-aporte-info-cuenta="${L.id}"]`);if(!M)return;const F=L.tasaCambio>0?L.tasaCambio:t,te=L.moneda==="USD"?L.monto*F:L.monto,Z=t>0?te/t:0;L.moneda==="USD"?M.innerHTML=`
        ...
        <div id="inv-seccion-ajustes" class="${this.tabActiva==="ajustes"?"block":"hidden"} grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="border-2 border-brand-black rounded-lg p-3 bg-white shadow-sm flex flex-col justify-between h-[175px] min-h-[175px] max-h-[175px] box-border overflow-hidden">
                ...
                </div>`,e.insertAdjacentElement("afterend",s),(n=document.getElementById("repo-pag-ant"))==null||n.addEventListener("click",()=>{this.paginaRepo>1&&(this.paginaRepo--,this.renderRepoLista())}),(d=document.getElementById("repo-pag-sig"))==null||d.addEventListener("click",()=>{this.paginaRepo<i&&(this.paginaRepo++,this.renderRepoLista())})}e.querySelectorAll("input[data-repo-cant]").forEach(s=>{ge(s,9999,5)}),e.querySelectorAll("[data-repo-del]").forEach(s=>{s.addEventListener("click",async()=>{var S;const u=s.dataset.repoDel,p=this.productos.find(h=>h.sku===u);if(!u||!p)return;if(((S=this.modelo.getConfig())==null?void 0:S.privacidadInventario)&&!this.duenoAutenticado){be("Acción no permitida en modo operador. Solicite al Dueño(a).","error");return}if(!this.duenoAutenticado&&this.modelo.hasPinSet()){const h=await pe("Acción protegida. Ingrese el PIN de Administrador(a) / Dueño(a):","","AUTENTICACIÓN DUEÑO(A)");if(!h||!await this.modelo.verificarPin(h)){be("PIN incorrecto o no suministrado. Acción cancelada.","error");return}}await ce(`¿Seguro que deseas eliminar permanentemente el producto "${p.nombre}" del catálogo?`,"ELIMINAR PRODUCTO")&&D.eliminarProducto(u).then(()=>{D.productos().then(h=>{this.productos=h,this.renderRepoLista(),this.renderCatalogoLista();const A=document.getElementById("repo-contador-estado");A&&(A.textContent=`${this.productos.length} artículos en depósito`)})})})}),e.querySelectorAll("[data-repo-in]").forEach(s=>{s.addEventListener("click",async()=>{var g;const u=s.dataset.repoIn,p=e.querySelector(`input[data-repo-cant="${u}"]`),x=p==null?void 0:p.value;if(u&&x&&Number(x)>0&&Number(x)<=9999){if(((g=this.modelo.getConfig())==null?void 0:g.privacidadInventario)&&!this.duenoAutenticado){be("Acción no permitida en modo operador. Solicite al Dueño(a).","error");return}if(!this.duenoAutenticado&&this.modelo.hasPinSet()){const $=await pe("Acción de inventario protegida. Ingrese el PIN de Administrador(a) / Dueño(a):","","AUTENTICACIÓN DUEÑO(A)");if(!$||!await this.modelo.verificarPin($)){be("PIN incorrecto o no suministrado. Operación cancelada.","error");return}}const h=e.querySelector(`select[data-repo-tipo="${u}"]`),A=(h==null?void 0:h.value)||"unidad",f=this.productos.find($=>$.sku===u);let E=Number(x);A==="caja"&&(f!=null&&f.esCaja)&&(f!=null&&f.unidadesPorCaja)&&f.unidadesPorCaja>1&&(E=E*f.unidadesPorCaja),D.compraStock(u,String(E)).then(()=>{D.productos().then($=>{this.productos=$,this.renderRepoLista(),this.renderCatalogoLista()})})}})}),e.querySelectorAll("[data-repo-red]").forEach(s=>{s.addEventListener("click",async()=>{var g;const u=s.dataset.repoRed,p=e.querySelector(`input[data-repo-cant="${u}"]`),x=p==null?void 0:p.value;if(u&&x&&Number(x)>0&&Number(x)<=9999){if(((g=this.modelo.getConfig())==null?void 0:g.privacidadInventario)&&!this.duenoAutenticado){be("Acción no permitida en modo operador. Solicite al Dueño(a).","error");return}if(!this.duenoAutenticado&&this.modelo.hasPinSet()){const $=await pe("Acción de inventario protegida. Ingrese el PIN de Administrador(a) / Dueño(a):","","AUTENTICACIÓN DUEÑO(A)");if(!$||!await this.modelo.verificarPin($)){be("PIN incorrecto o no suministrado. Operación cancelada.","error");return}}const h=e.querySelector(`select[data-repo-tipo="${u}"]`),A=(h==null?void 0:h.value)||"unidad",f=this.productos.find($=>$.sku===u);let E=Number(x);A==="caja"&&(f!=null&&f.esCaja)&&(f!=null&&f.unidadesPorCaja)&&f.unidadesPorCaja>1&&(E=E*f.unidadesPorCaja),D.reducirStock(u,String(E)).then(()=>{D.productos().then($=>{this.productos=$,this.renderRepoLista(),this.renderCatalogoLista()})})}})}),e.querySelectorAll("[data-repo-out]").forEach(s=>{s.addEventListener("click",async()=>{var g;const u=s.dataset.repoOut,p=e.querySelector(`input[data-repo-cant="${u}"]`),x=p==null?void 0:p.value;if(u&&x&&Number(x)>0&&Number(x)<=9999){if(((g=this.modelo.getConfig())==null?void 0:g.privacidadInventario)&&!this.duenoAutenticado){be("Acción no permitida en modo operador. Solicite al Dueño(a).","error");return}if(!this.duenoAutenticado&&this.modelo.hasPinSet()){const $=await pe("Acción de inventario protegida. Ingrese el PIN de Administrador(a) / Dueño(a):","","AUTENTICACIÓN DUEÑO(A)");if(!$||!await this.modelo.verificarPin($)){be("PIN incorrecto o no suministrado. Operación cancelada.","error");return}}const h=e.querySelector(`select[data-repo-tipo="${u}"]`),A=(h==null?void 0:h.value)||"unidad",f=this.productos.find($=>$.sku===u);let E=Number(x);A==="caja"&&(f!=null&&f.esCaja)&&(f!=null&&f.unidadesPorCaja)&&f.unidadesPorCaja>1&&(E=E*f.unidadesPorCaja),D.merma(u,String(E),null).then(()=>{D.productos().then($=>{this.productos=$,this.renderRepoLista(),this.renderCatalogoLista()})})}})})}async guardarProducto(){var z;const e=document.getElementById("prod-nombre"),o=document.getElementById("prod-categoria"),a=document.getElementById("prod-impuesto"),r=document.getElementById("prod-precio-bruto"),i=document.getElementById("prod-margen"),c=document.getElementById("prod-precio-neto"),b=document.getElementById("prod-stock"),t=((z=document.getElementById("prod-unidad"))==null?void 0:z.value)||"un",n=document.getElementById("prod-sin-stock"),d=document.getElementById("alta-prod-msg"),s=V=>{if(!V)return 0;const H=V.trim().replace(",","."),_=parseFloat(H);return Number.isFinite(_)&&!isNaN(_)?_:0};if(d&&(d.classList.add("hidden"),d.textContent=""),!(e!=null&&e.value.trim())){d&&(d.textContent="El nombre del producto o servicio es obligatorio.",d.style.color="#C60C15",d.classList.remove("hidden"),e==null||e.focus());return}const u=s(c==null?void 0:c.value);if(u<=0||u>99999.99){d&&(d.textContent="El precio neto final debe ser un monto positivo entre $0.01 y $99,999.99 USD.",d.style.color="#C60C15",d.classList.remove("hidden"),c==null||c.focus());return}const p=s(b==null?void 0:b.value),x=(n==null?void 0:n.checked)||!1;if(!x&&(p<0||p>99999)){d&&(d.textContent="El stock inicial debe estar comprendido entre 0 y 99,999 unidades.",d.style.color="#C60C15",d.classList.remove("hidden"),b==null||b.focus());return}if(!x&&t==="un"&&(!Number.isInteger(p)||p%1!==0)){d&&(d.textContent='Los productos vendidos por Unidad (un.) deben tener un stock entero exacto (ej: 10). Para registrar fracciones o decimales, seleccione Tipo de Medida "Por Peso (kg)" o "Por Volumen (ml)".',d.style.color="#C60C15",d.classList.remove("hidden"),b==null||b.focus());return}const g=s((a==null?void 0:a.value)||"0"),S=s((i==null?void 0:i.value)||"30");let h=s(r==null?void 0:r.value);h<=0&&(h=u/(1+g/100)/(1+S/100));const A=document.getElementById("prod-es-caja"),f=document.getElementById("prod-unidades-caja"),E=(A==null?void 0:A.checked)||!1,$=E&&f?Math.max(2,parseInt(f.value,10)||12):void 0,k=document.getElementById("prod-tiene-paquete"),w=document.getElementById("prod-nombre-paquete"),y=document.getElementById("prod-precio-paquete"),v=(k==null?void 0:k.checked)||!1,C=v&&w?w.value.trim():void 0,j=v&&y?s(y.value).toFixed(2):void 0;try{await D.crearProducto({nombre:e.value.trim(),precioBrutoUsd:h.toFixed(2),margenPct:String(S),precioUsd:u.toFixed(2),impuestoPct:String(g),stockInicial:n!=null&&n.checked?"0":String(p),categoriaId:(o==null?void 0:o.value)||"",sinStock:(n==null?void 0:n.checked)||!1,pesable:t==="kg"||t==="ml",unidad:t,alcoholica:!1,esCaja:E,unidadesPorCaja:$,precioPaqueteUsd:j,nombrePaquete:C}),d&&(d.textContent="Producto o servicio registrado exitosamente.",d.style.color="#00823B",d.classList.remove("hidden")),setTimeout(()=>void this.render("alta"),800)}catch(V){d&&(d.textContent=V instanceof Error?V.message:String(V),d.style.color="#C60C15",d.classList.remove("hidden"))}}}const ke=l=>{if(typeof l=="number")return Number.isFinite(l)?l:0;if(l==null)return 0;let e=String(l).trim();if(!e)return 0;e.includes(",")&&!e.includes(".")?e=e.replace(",","."):e.includes(".")&&e.includes(",")&&(e=e.replace(/\./g,"").replace(",","."));const o=parseFloat(e);return Number.isFinite(o)&&!isNaN(o)?o:0},W=l=>ke(l).toLocaleString("es-VE",{minimumFractionDigits:2,maximumFractionDigits:2}),qe=l=>{const e=l.finUnix||Math.floor(Date.now()/1e3),o=Math.max(0,e-l.inicioUnix),a=Math.floor(o/3600),r=Math.floor(o%3600/60);return`${a}h ${r}m`},bt=(l,e)=>{const o=l.operadoresRelevo.length>0?l.operadoresRelevo.join(" / "):l.operadorActual,a=e.filter(s=>{const u=s.fechaUnix&&s.fechaUnix>0?s.fechaUnix:(()=>{const p=(s.fechaHora||"").match(/(\d{4})-(\d{2})-(\d{2})[T\s](\d{2}):(\d{2}):(\d{2})/);if(p&&p[1]&&p[2]&&p[3]&&p[4]&&p[5]&&p[6]){const x=Date.UTC(parseInt(p[1],10),parseInt(p[2],10)-1,parseInt(p[3],10),parseInt(p[4],10),parseInt(p[5],10),parseInt(p[6],10));if(!isNaN(x))return Math.floor(x/1e3)}return 0})();return!(u<=0||u<l.inicioUnix||l.finUnix&&u>l.finUnix)}),r=[["INFORME DE JORNADA",l.id],["Estado",l.estado==="abierta"?"En curso":"Cerrada"],["Apertura",l.inicioStr],["Cierre",l.finStr||"-"],["Duracion",qe(l)],["Operadores",o],["Total USD",`$${W(l.ventasTotalUsd)}`],["Total Bs",`Bs. ${W(l.ventasTotalBs)}`],["Tickets emitidos",String(l.ticketsEmitidos)],["Vuelto pagado Bs",`Bs. ${W(l.vueltoPagadoBs)}`],["Vuelto retenido Bs",`Bs. ${W(l.vueltoRetenidoBs)}`],["Deudas liquidadas USD",`$${W(l.deudasLiquidadasUsd)}`],["Entradas stock",String(l.entradasStockReg)],["Mermas",String(l.mermasStockReg)],["Cambios de precio",String(l.cambiosPrecioReg)],["Tasa apertura",`Bs. ${l.tasaInicio}`],["Tasa cierre",l.tasaFin?`Bs. ${l.tasaFin}`:"-"],...l.checksumSha256?[["SHA-256",l.checksumSha256]]:[],[],["TICKETS DE LA JORNADA"],["ID Ticket","Fecha/Hora","Canal","Operador","Metodo(s) de Pago","Total USD","Total Bs"]],i=a.map(s=>{const u=s.pagos&&s.pagos.length>0?s.pagos.map(p=>`${p.metodo}:$${W(p.montoUsd)}`).join(" | "):"DIRECTO";return[s.ventaId,s.fechaHora||"",s.canal||"VENTA DIRECTA",s.operador||"Caja",u,`$${W(s.totalUsd)}`,`Bs. ${W(s.totalBs)}`]}),b=[...r,...i].map(s=>s.map(u=>`"${String(u).replace(/"/g,'""')}"`).join(",")).join(`\r
`),t=new Blob(["\uFEFF"+b],{type:"text/csv;charset=utf-8;"}),n=URL.createObjectURL(t),d=document.createElement("a");d.href=n,d.download=`jornada_${l.id}.csv`,document.body.appendChild(d),d.click(),document.body.removeChild(d),URL.revokeObjectURL(n)};class ut{constructor(e,o,a){U(this,"contenedor");U(this,"modalRoot");U(this,"modelo");U(this,"jornadas",[]);U(this,"jornadaActual",null);U(this,"todosLosTickets",[]);U(this,"busqueda","");U(this,"expandidos",new Set);U(this,"paginaActual",1);U(this,"POR_PAGINA",20);this.contenedor=e,this.modalRoot=o,this.modelo=a}parseFechaUnix(e){if(!e)return 0;const o=e.match(/(\d{4})-(\d{2})-(\d{2})[T\s](\d{2}):(\d{2}):(\d{2})/);if(o&&o[1]&&o[2]&&o[3]&&o[4]&&o[5]&&o[6]){const a=Date.UTC(parseInt(o[1],10),parseInt(o[2],10)-1,parseInt(o[3],10),parseInt(o[4],10),parseInt(o[5],10),parseInt(o[6],10));if(!isNaN(a))return Math.floor(a/1e3)}return 0}async render(){const[e,o,a]=await Promise.all([D.obtenerJornadaActual(),D.listarHistoricoJornadas(),D.ventas()]);this.jornadaActual=e,this.jornadas=o,this.todosLosTickets=a;const r=e?o.filter(n=>n.id!==e.id):o,i=[...e?[e]:[],...r],c=i.length,b=i.reduce((n,d)=>n+ke(d.ventasTotalUsd),0),t=i.reduce((n,d)=>n+d.ticketsEmitidos,0);this.contenedor.innerHTML=`
        ...
        <!-- Lista -->
        <div id="lista-jornadas" class="flex flex-col gap-2"></div>`,this.conectarEventosBuscador(),this.renderLista()}conectarEventosBuscador(){var e;(e=document.getElementById("jornadas-buscar"))==null||e.addEventListener("input",o=>{this.busqueda=o.target.value,this.paginaActual=1,this.renderLista()})}renderLista(){var s,u;const e=document.getElementById("lista-jornadas");if(!e)return;const o=this.jornadaActual?this.jornadas.filter(p=>{var x;return p.id!==((x=this.jornadaActual)==null?void 0:x.id)}):this.jornadas,a=[...this.jornadaActual?[this.jornadaActual]:[],...o],r=this.busqueda.trim().toLowerCase(),i=r?a.filter(p=>p.id.toLowerCase().includes(r)||p.operadorActual.toLowerCase().includes(r)||p.operadoresRelevo.some(x=>x.toLowerCase().includes(r))||p.inicioStr.toLowerCase().includes(r)||(p.finStr||"").toLowerCase().includes(r)):a,c=document.getElementById("paginacion-jornadas");if(c&&c.remove(),i.length===0){e.innerHTML=`
            ...
            </div>`;return}const b=Math.max(1,Math.ceil(i.length/this.POR_PAGINA));this.paginaActual>b&&(this.paginaActual=b),this.paginaActual<1&&(this.paginaActual=1);const t=this.paginaActual,n=i.slice((t-1)*this.POR_PAGINA,t*this.POR_PAGINA);e.innerHTML=n.map(p=>this.renderFila(p)).join(""),n.forEach(p=>{if(this.expandidos.has(p.id)){const x=document.getElementById(`detalle-${p.id}`);x==null||x.classList.remove("hidden");const g=document.getElementById(`icon-${p.id}`);g&&(g.textContent="▲")}}),e.querySelectorAll("[data-toggle-jornada]").forEach(p=>{p.addEventListener("click",()=>{const x=p.dataset.toggleJornada,g=document.getElementById(`detalle-${x}`),S=document.getElementById(`icon-${x}`);if(g){const h=g.classList.contains("hidden");g.classList.toggle("hidden",!h),S&&(S.textContent=h?"▲":"▼"),h?this.expandidos.add(x):this.expandidos.delete(x)}})}),e.querySelectorAll("[data-exportar-jornada]").forEach(p=>{p.addEventListener("click",x=>{x.stopPropagation();const g=p.dataset.exportarJornada,S=i.find(h=>h.id===g);S&&bt(S,this.todosLosTickets)})}),e.querySelectorAll("[data-ver-ticket]").forEach(p=>{p.addEventListener("click",()=>{const x=p.dataset.verTicket,g=this.todosLosTickets.find(S=>S.ventaId===x);g&&this.mostrarModalTicket(g)})});const d=document.createElement("div");d.id="paginacion-jornadas",d.innerHTML=`
