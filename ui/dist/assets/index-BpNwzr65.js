...
Esta tasa afectará todos los nuevos cobros y tickets a partir de este momento.`)){const p=await J("fijar_tasa_manual",{tasa:s.toFixed(2)});p&&this.pintar(p),r()}})}async actualizar(){const e=await this.obtenerTasa("obtener_tasa_bcv");e&&this.pintar(e)}async forzar(){const e=document.getElementById("bcv-refresh");e&&e.classList.add("animate-spin");const o=await this.obtenerTasa("forzar_actualizacion_tasa");e&&e.classList.remove("animate-spin"),o&&this.pintar(o)}async obtenerTasa(e){try{return await J(e)}catch{return null}}pintar(e){const o=document.getElementById("bcv-valor"),a=document.getElementById("bcv-fluctuacion");if(!o||!a)return;const r=Number(e.valor);o.textContent=Number.isFinite(r)&&r>0?r.toLocaleString("es-VE",{minimumFractionDigits:2,maximumFractionDigits:2}):"--",Number.isFinite(r)&&r>0&&window.dispatchEvent(new CustomEvent("tasa_actualizada",{detail:r}));const i=Number(e.fluctuacionPct);if(e.direccion&&Number.isFinite(i)){const c=e.direccion==="subio"?Xe:e.direccion==="bajo"?Ye:Qe,b=i>0?"+":"";a.textContent=`${b}${i.toFixed(2)}%`,a.style.color=c}else a.textContent=""}}class tt{constructor(){U(this,"config",null);U(this,"rol","caja");U(this,"tasa",0);U(this,"oyentes",[])}suscribir(e){this.oyentes.push(e)}notificar(){this.oyentes.forEach(e=>e())}async cargarConfig(){return this.config=await D.config(),this.config}getConfig(){return this.config}async inicializar(e,o,a,r,i){await D.inicializar(e,o,a,r,i),await this.cargarConfig(),this.notificar()}get rubroAbasto(){var e,o;return(((e=this.config)==null?void 0:e.rubros)??0&Ce)!==0||(((o=this.config)==null?void 0:o.rubros)??0)===Ce}tieneRubro(e){var o;return((((o=this.config)==null?void 0:o.rubros)??0)&e)!==0}tieneCapacidad(e){var o;return((((o=this.config)==null?void 0:o.capacidades)??0)&e)!==0}setRol(e){this.rol=e,this.notificar()}get rolActual(){return this.rol}setTasa(e){this.tasa=e,this.notificar()}get tasaActual(){return this.tasa}bs(e){return(Number(e)*this.tasa).toLocaleString("es-VE",{minimumFractionDigits:2,maximumFractionDigits:2})}hasPinSet(){var e;return!!((e=this.config)!=null&&e.tienePin)}async verificarPin(e){return D.validarPin(e)}}function at(l,e){const o=l.replace(/[^a-zA-Z0-9]/g,"");if(o.length!==16)return!1;const a=o.split("").map(b=>parseInt(b,10)).filter(b=>!isNaN(b)&&b<10);if(a.length!==16)return!1;let r=0;for(let b=0;b<12;b++)r+=a[b]*(b+1);const i=r%1e4,c=a[12]*1e3+a[13]*100+a[14]*10+a[15];return i===c}class ot{constructor(e,o,a){U(this,"contenedor");U(this,"modelo");U(this,"alTerminar");U(this,"modo","nuevo");U(this,"bloquearPanel",!0);this.contenedor=e,this.modelo=o,this.alTerminar=a}render(){this.contenedor.innerHTML=`
        ...
        `;const i=r.querySelector("#dialog-input-val"),c=t=>{r.remove(),a(t)};r.querySelectorAll("[data-dialog-cancel]").forEach(t=>t.addEventListener("click",()=>c(null))),(b=r.querySelector("[data-dialog-accept]"))==null||b.addEventListener("click",()=>{c((i==null?void 0:i.value)??"")}),i==null||i.addEventListener("keydown",t=>{t.key==="Enter"&&c(i.value),t.key==="Escape"&&c(null)}),document.body.appendChild(r),i==null||i.focus(),i==null||i.select()})}const Oe="America/Caracas";function he(l){if(!l||l<=0)return"-";const e=new Date(l*1e3);return new Intl.DateTimeFormat("es-VE",{timeZone:Oe,year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit",second:"2-digit",hour12:!1}).format(e)}function Ie(l,e=!1){return new Intl.DateTimeFormat("es-VE",{timeZone:Oe,year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit",...e?{second:"2-digit"}:{},hour12:!1}).format(l)}function nt(l){if(!l||l<=0)return"Hoy";const e=Math.floor(Date.now()/1e3),o=Math.max(0,e-l);if(o<3600)return"Hoy (reciente)";const a=Math.floor(o/3600);if(a<24)return`Hoy (hace ${a}h)`;const r=Math.floor(a/24);return r===1?"Hace 1 día":`Hace ${r} días`}const de=l=>X(l).toLocaleString("es-VE",{minimumFractionDigits:2,maximumFractionDigits:2});async function Me(l){var s,u,p;const[e,o]=await Promise.all([D.obtenerJornadaActual(),D.ventas()]);if(!e){l.innerHTML=`
        ...
        </div>`,(s=l.querySelector("#btn-cerrar-historial-turno"))==null||s.addEventListener("click",()=>{l.innerHTML=""});return}const a=e.inicioUnix,r=o.filter(m=>(m.fechaUnix||0)>=a);let i=0,c=0,b={},t={};r.forEach(m=>{const g=X(m.totalUsd),S=X(m.totalBs);i+=g,c+=S;const f=m.operador||e.operadorActual||"Cajero";t[f]||(t[f]={usd:0,bs:0,tickets:0}),t[f].usd+=g,t[f].bs+=S,t[f].tickets+=1,Array.isArray(m.pagos)&&m.pagos.length>0&&m.pagos.forEach(A=>{const h=A.metodo||"EFECTIVO";b[h]||(b[h]={usd:0,bs:0}),b[h].usd+=X(A.montoUsd),b[h].bs+=X(A.montoBs)})});const n=e.operadoresActivos&&e.operadoresActivos.length>0?e.operadoresActivos.join(", "):e.operadorActual||"Sin asignar";l.innerHTML=`
    <div class="fixed inset-0 bg-black/40 flex items-center justify-center z-[100] p-4">
        <div class="bg-white border-2 border-brand-black rounded-lg shadow-brutal p-5 sm:p-6 w-full max-w-3xl max-h-[90vh] overflow-y-auto flex flex-col">
            <div class="flex justify-between items-center border-b-2 border-brand-black pb-3 mb-3">
                <div>
                    ...
                    <p class="text-xs text-gray-500 font-bold">
                        Apertura: ${he(e.inicioUnix)} · Operador(es): <strong>${n}</strong>
                    </p>
                </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
                <div class="bg-gray-50 border-2 border-brand-black rounded p-3">
                    ...
                    <p class="font-heading font-black text-xl text-brand-black mt-0.5">$ ${de(i)} USD</p>
                    <p class="text-xs font-black text-brand-purple">Bs. ${de(c)}</p>
                </div>
                <div class="bg-gray-50 border-2 border-brand-black rounded p-3">
                    ...
                    <p class="text-xs font-bold text-gray-500">Tasa de inicio: Bs. ${de(e.tasaInicio)}</p>
                </div>
                <div class="bg-gray-50 border-2 border-brand-black rounded p-3">
                    <span class="text-[10px] font-black uppercase text-gray-500 block">Vueltos Otorgados / Retenidos</span>
                    <p class="font-heading font-black text-base text-gray-800 mt-0.5">Pagado: Bs. ${de(e.vueltoPagadoBs)}</p>
                    <p class="text-xs font-bold text-emerald-800">Retenido: Bs. ${de(e.vueltoRetenidoBs)}</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
                <div class="border-2 border-brand-black rounded p-3 bg-white">
                    ...
                    <div class="space-y-1.5 max-h-36 overflow-y-auto pr-1">
                        ${Object.keys(t).length===0?'<p class="text-xs text-gray-400 italic">Sin transacciones registradas por operadores.</p>':Object.entries(t).map(([m,g])=>`
                                <div class="flex justify-between items-center text-xs border-b border-gray-100 pb-1">
                                    ...
                                    <span class="font-mono font-black text-brand-black">$${de(g.usd)} · Bs. ${de(g.bs)}</span>
                                </div>
                            `).join("")}
                    </div>
                </div>
                <div class="border-2 border-brand-black rounded p-3 bg-white">
                    ...
                    <div class="space-y-1.5 max-h-36 overflow-y-auto pr-1">
                        ${Object.keys(b).length===0?'<p class="text-xs text-gray-400 italic">Sin pagos asentados.</p>':Object.entries(b).map(([m,g])=>`
                                <div class="flex justify-between items-center text-xs border-b border-gray-100 pb-1">
                                    <span class="font-bold text-gray-800">${m}</span>
                                    <span class="font-mono font-black text-brand-black">${g.usd>0?`$${de(g.usd)} `:""}Bs. ${de(g.bs)}</span>
                                </div>
                            `).join("")}
                    </div>
                </div>
                            ...
                            <div class="border border-brand-black bg-white rounded overflow-hidden shadow-sm">
                                <div class="flex items-center justify-between p-2.5 text-xs font-bold hover:bg-amber-50/50 cursor-pointer select-none" data-toggle-ticket-detalle="${m.ventaId}">
                                    ...
                                    <div class="text-right shrink-0 flex items-center gap-3">
                                        <div>
                                            <p class="font-heading font-black text-sm text-brand-black">$${de(m.totalUsd)}</p>
                                            <p class="text-[10px] font-bold text-brand-purple">Bs. ${de(m.totalBs)}</p>
                                        </div>
                                ...
                                <div id="detalle-ticket-${m.ventaId}" class="hidden border-t border-brand-black bg-gray-50 p-2.5 text-xs">
                                        ...
                                        ${m.lineas&&m.lineas.length>0?m.lineas.map(g=>`
                                                <div class="flex justify-between items-center text-[11px] border-b border-gray-200 pb-1">
                                                    <div class="min-w-0 flex-1 pr-2">
                                                        ...
                                                        <span class="text-[10px] text-gray-500 block">${g.cantidad} × $${de(g.precioUsd)} ${g.modoVenta==="paquete"?"(Empaque/Caja)":""}</span>
                                                    </div>
                                                    <div class="text-right shrink-0 font-mono font-bold">
                                                        <span class="text-brand-black">$${de(g.subtotalUsd)}</span>
                                                        <span class="text-[10px] text-gray-500 block">Bs. ${de(g.subtotalBs)}</span>
                                                    </div>
                                                </div>
                                    ...
                                    ${m.pagos&&m.pagos.length>0?`
                                        <div class="pt-1.5 border-t border-gray-200 flex flex-wrap justify-between items-center text-[10px] font-bold text-gray-600">
                                            <span>Pagos: ${m.pagos.map(g=>`${g.metodo} ($${de(g.montoUsd)}${g.moneda==="BS"?` / Bs.${de(g.montoBs)}`:""})`).join(", ")}</span>
                                            ${m.vueltoBs&&X(m.vueltoBs)>0?`<span class="text-emerald-800">Vuelto: Bs. ${de(m.vueltoBs)} (${m.estadoVuelto||"PAGADO"})</span>`:""}
                                        </div>
                                    `:""}
                                </div>
                            </div>
    ...
    </div>`,l.querySelectorAll("[data-toggle-ticket-detalle]").forEach(m=>{m.addEventListener("click",()=>{const g=m.dataset.toggleTicketDetalle,S=l.querySelector(`#detalle-ticket-${g}`),f=l.querySelector(`[data-arrow-detalle="${g}"]`);S&&(S.classList.contains("hidden")?(S.classList.remove("hidden"),f&&(f.textContent="▲")):(S.classList.add("hidden"),f&&(f.textContent="▼")))})});const d=()=>{l.innerHTML=""};(u=l.querySelector("#btn-cerrar-historial-turno"))==null||u.addEventListener("click",d),(p=l.querySelector("#btn-cerrar-historial-turno-footer"))==null||p.addEventListener("click",d)}const Q=l=>X(l).toLocaleString("es-VE",{minimumFractionDigits:2,maximumFractionDigits:2});class st{constructor(e,o,a,r){U(this,"contenedor");U(this,"vm");U(this,"modelo");U(this,"modal");U(this,"pagosBorrador",null);U(this,"resolucionVueltoBorrador",null);U(this,"semaforoStock",{rojoMax:5,amarilloMax:15});this.contenedor=e,this.modal=o,this.vm=a,this.modelo=r,D.obtenerSemaforoStock().then(i=>{this.semaforoStock=i,this.renderDinamico()}),this.vm.suscribir(()=>this.renderDinamico()),this.modelo.suscribir(()=>this.renderDinamico())}render(){var e,o,a,r,i,c;this.contenedor.innerHTML=`
        ...
        <div class="fixed inset-0 bg-black/40 flex items-center justify-center z-[100] p-4">
            ...
            `,"max-w-md"),(r=this.modal.querySelector("[data-cerrar]"))==null||r.addEventListener("click",()=>this.cerrarModal()),(i=this.modal.querySelector("#btn-cancelar-inicio-jornada"))==null||i.addEventListener("click",()=>this.cerrarModal()),(c=this.modal.querySelector("#form-nuevo-cajero-inicio"))==null||c.addEventListener("submit",async t=>{t.preventDefault();const n=this.modal.querySelector("#in-nuevo-cajero-inicio"),d=(n==null?void 0:n.value.trim())||"";d&&(await D.crearOperador(d),this.cerrarModal(),this.abrirModalIniciarJornada())}),(b=this.modal.querySelector("#btn-abrir-turno"))==null||b.addEventListener("click",async()=>{const t=this.modal.querySelectorAll("input[data-inicio-cajero-chk]:checked"),n=[];if(t.forEach(s=>{s.value&&n.push(s.value)}),n.length===0){this.mostrarToast("Selecciona al menos un operador para abrir el turno.","error");return}const d=n[0];await D.abrirJornada(d,n),this.cerrarModal(),this.renderDinamico()})})()}async abrirModalCobro(){var g,S,f,A;if(this.vm.lineasCarrito.length===0)return;let e=[];try{e=await D.listarMetodosPago()}catch{e=[{nombre:"PUNTOD.VENTA",moneda:"BS"},{nombre:"BIOPAGO",moneda:"BS"},{nombre:"PAGO MOVIL",moneda:"BS"},{nombre:"TRANSF.BS.",moneda:"BS"},{nombre:"BS.EFEC.",moneda:"BS"},{nombre:"DOL.CASH",moneda:"USD"},{nombre:"ZELLE",moneda:"USD"},{nombre:"BINAN.USDT",moneda:"USD"}]}const o=Number(this.vm.totalUsd.toFixed(2)),a=this.vm.tasaTicket,r=Number((o*a).toFixed(2)),i=e.find(h=>h.nombre==="BIOPAGO")||e.find(h=>h.moneda==="BS")||e[0]||{nombre:"BIOPAGO",moneda:"BS"};let c;if(this.pagosBorrador&&this.pagosBorrador.length>0){if(c=this.pagosBorrador.map(h=>({...h})),c.length===1&&c[0]){const h=c[0];h.monto=h.moneda==="USD"?o:r,h.tasaCambio=a}else if(c.length>1){let h=0;for(let k=0;k<c.length-1;k++){const w=c[k],y=w.tasaCambio>0?w.tasaCambio:a;h+=w.moneda==="USD"?w.monto*y:w.monto}const E=Math.max(0,Math.round((r-h)*100)/100),C=c[c.length-1];C&&(C.monto=C.moneda==="USD"?a>0?Number((E/a).toFixed(2)):0:E,C.tasaCambio=a)}}else c=[{id:"p-1",metodo:i.nombre,moneda:i.moneda,monto:i.moneda==="USD"?o:r,tasaCambio:a,referencia:""}];let b=((g=this.resolucionVueltoBorrador)==null?void 0:g.estado)||"PAGADO",t=((S=this.resolucionVueltoBorrador)==null?void 0:S.metodo)||"BS.EFEC.",n=((f=this.resolucionVueltoBorrador)==null?void 0:f.tasa)||a;e.some(h=>h.nombre===t)||(t=((A=e[0])==null?void 0:A.nombre)||"BS.EFEC.");let d=!1;const s=()=>{let h=0;for(const $ of c)if($.moneda==="USD"){const j=$.tasaCambio>0?$.tasaCambio:a;h+=$.monto*j}else h+=$.monto;h=Math.round(h*100)/100;const E=a>0?Math.round(h/a*100)/100:0,C=Math.max(0,Math.round((r-h)*100)/100),k=a>0?Math.max(0,Math.round(C/a*100)/100):0,w=Math.max(0,Math.round((h-r)*100)/100),y=a>0?Math.max(0,Math.round(w/a*100)/100):0,v=h>=r-.01&&c.every($=>$.monto>0);return{cubiertoUsd:E,cubiertoBs:h,faltanteUsd:k,faltanteBs:C,vueltoUsd:y,vueltoBs:w,puedeConfirmar:v}},u=h=>{const E=e.find(y=>y.nombre===t),C=(E==null?void 0:E.moneda)==="USD",k=n>0?n:a,w=C&&k>0?Number((h.vueltoBs/k).toFixed(2)):h.vueltoUsd;return h.faltanteBs>.009?`
                <div class="bg-amber-50 border-2 border-amber-500 rounded p-3 text-center mb-3">
                    ...
                    <p class="font-heading font-black text-xl text-amber-700">Faltan Bs. ${Q(h.faltanteBs)} · <span class="text-base text-amber-900">$ ${Q(h.faltanteUsd)} (Equiv. Oficial)</span></p>
                </div>`:h.vueltoBs>.009?`
                <div class="border-2 border-emerald-700 bg-emerald-50/70 rounded-lg p-3 mb-3">
                    <div class="flex flex-wrap items-center justify-between gap-1 mb-2">
                        <div>
                            ...
                            <span class="font-heading font-black text-xl text-emerald-800">Bs. ${Q(h.vueltoBs)}</span>
                        </div>
                        <span class="text-xs font-bold text-emerald-900 font-mono">$ ${Q(h.vueltoUsd)} equiv. oficial</span>
                    </div>
                    ...
                    <div class="bg-white border border-emerald-600 rounded p-2.5 space-y-2">
                        ...
                        <div class="flex items-center justify-between pt-1 border-t border-gray-100 text-xs">
                            ...
                            <span class="font-heading font-black text-base text-brand-black">Bs. ${Q(h.vueltoBs)}</span>
                        </div>
                        `}
                    </div>
                    `:`
                    <div class="bg-white border border-gray-300 rounded p-2 text-xs text-gray-700">
                        ...
                        <p class="text-[11px] text-gray-600 leading-tight">
                            Excedente de <b>Bs. ${Q(h.vueltoBs)} ($ ${Q(h.vueltoUsd)})</b> asentado a favor sin egreso de efectivo.
                        </p>
                    </div>
                    `}
                </div>`:`
            <div class="bg-emerald-50 border-2 border-emerald-600 rounded p-2.5 text-center mb-3">
                ...
                <p class="text-xs text-emerald-700 font-bold">Importe liquidado sin diferencia pendiente.</p>
            </div>`},p=()=>{const h=s(),E=e.filter(y=>y.moneda==="USD"),C=e.filter(y=>y.moneda==="BS"),k=c.map((y,v)=>{const $=E.map(O=>`<option value="${O.nombre}" ${O.nombre===y.metodo?"selected":""}>${O.nombre} ($ - Tasa Dinámica)</option>`).join(""),G=`
                        <optgroup label="MÉTODOS EN BOLÍVARES (Bs.)">
                            ${C.map(O=>`<option value="${O.nombre}" ${O.nombre===y.metodo?"selected":""}>${O.nombre} (Bs.)</option>`).join("")}
                        </optgroup>
                    ...
                    <div data-pago-card="${y.id}" class="border-2 border-brand-black rounded-lg p-3 bg-gray-50 mb-2.5">
                        ...
                        `}
                    </div>`}).join(""),w=`<div id="contenedor-estado-balance">${u(h)}</div>`;this.modalContenido(`
                <div>
                    <div class="flex justify-between items-center border-b-2 border-brand-black pb-3 mb-3">
                        <div>
                    ...
                    <div class="flex flex-wrap items-center justify-between gap-2 pt-2 border-t-2 border-brand-black">
                        ...
                        <button id="btn-confirmar-cobro" ${h.puedeConfirmar?"":"disabled"}
            ...
            <button data-cerrar class="w-full bg-brand-black text-white font-heading font-black text-lg py-3 rounded border-2 border-brand-black shadow-brutal-sm hover:bg-gray-800">
                LISTO
            </button>`,"max-w-md")}renderDinamico(){const e=this.contenedor.querySelector("#caja-grid");if(!e)return;const o=this.semaforoStock;e.innerHTML=this.vm.visibles.map(c=>{const b=(c.capacidades&fe)!==0,t=Number(c.stock),n=t<=0,d=c.sinStock?"text-purple-700":t<=o.rojoMax?"text-red-600":t<=o.amarilloMax?"text-amber-600":"text-emerald-700",s=c.unidad?c.unidad.toLowerCase()==="un"?"un.":c.unidad.toLowerCase():b?"kg":"un.",u=s==="kg"?"kg":s==="ml"?"ml":"un.";let p="";if(c.sinStock)p="LIBRE";else if(n)p="AGOTADO";else if(c.esCaja&&c.unidadesPorCaja&&c.unidadesPorCaja>1){const g=Math.floor(t/c.unidadesPorCaja),S=t%c.unidadesPorCaja;p=`${g} cajas y ${S} un. (${t} un.)`}else p=`${t} ${s}`;const m=c.precioPaqueteUsd&&c.nombrePaquete||c.esCaja&&c.unidadesPorCaja&&c.unidadesPorCaja>1;return`
            ...
            <tr class="hover:bg-gray-50 transition-colors">
                <td class="py-2.5 px-3 flex items-center gap-2">
                    ...
                    <span class="font-black text-brand-black text-xs">${C.nombre}</span>
                </td>
                <td class="py-2.5 px-3 text-center">
                    <span class="bg-gray-100 border border-brand-black px-2 py-0.5 rounded text-[11px] font-black">${C.cantidadProductos} art.</span>
                </td>
                <td class="py-2.5 px-3 text-right font-mono">${C.unidadesStock} un.</td>
                <td class="py-2.5 px-3 text-right font-black font-mono text-brand-black">$ ${ee(C.dineroBrutoUsd)}</td>
                <td class="py-2.5 px-3 text-right font-black font-mono text-brand-purple">Bs. ${ee(C.dineroBrutoBs)}</td>
                <td class="py-2.5 px-3 text-right font-mono text-gray-700">$ ${ee(C.dineroVentaUsd)}</td>
                <td class="py-2.5 px-3 text-right font-mono text-emerald-700">$ ${ee(C.margenBrutoProyectadoUsd)} (${C.margenBrutoPct}%)</td>
                <td class="py-2.5 px-3 text-right">
                    <div class="flex items-center justify-end gap-1.5">
                        ...
                        <span class="font-mono text-[11px] w-10 text-right">${C.porcentajeCapital}%</span>
                    </div>
                </td>
                ...
                <table class="w-full text-left text-xs">
                    ...
                    <tfoot class="border-t-2 border-brand-black bg-gray-100 font-black font-heading text-xs">
                        <tr>
                            ...
                            <td class="py-2.5 px-3 text-right text-brand-black">$ ${ee(m)}</td>
                            <td class="py-2.5 px-3 text-right text-brand-purple">Bs. ${ee(g)}</td>
                            <td class="py-2.5 px-3 text-right text-brand-black">$ ${ee(S)}</td>
                            <td class="py-2.5 px-3 text-right text-emerald-700">$ ${ee(f)} (${A}%)</td>
        ...
        <div class="bg-white border-2 border-brand-black rounded-lg shadow-brutal p-4 sm:p-6 mb-4 sm:mb-6">
            ...
            ${(()=>{const C=Math.ceil(this.transacciones.length/this.POR_PAGINA_TRX);return C<=1?"":`
                <div class="flex items-center justify-between border-t-2 border-brand-black pt-3 mt-3">
                    <span class="text-xs font-bold text-gray-600">Página ${this.paginaTrx} de ${C} · ${this.transacciones.length} transacciones</span>
                    <div class="flex gap-2">
                        ...
                        <button id="trx-pag-sig" class="border-2 border-brand-black px-3 py-1 rounded text-xs font-heading font-black shadow-brutal-sm ${this.paginaTrx===C?"opacity-30 pointer-events-none":"hover:bg-gray-100"}">SIGUIENTE</button>
                    </div>
                </div>`})()}
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
            ...
            <div class="bg-white border-2 border-brand-black rounded-lg shadow-brutal p-4 sm:p-6 flex flex-col justify-between">
                <div>
                    ...
                    <div id="lista-respaldos" class="space-y-2 max-h-48 overflow-y-auto pr-1">
                        ${this.respaldos.map(C=>`
                            <div class="border border-brand-black rounded p-2.5 bg-gray-50 flex justify-between items-center text-xs">
                                <div class="min-w-0 pr-2">
                                    <p class="font-mono font-black text-brand-black truncate text-[11px]">Respaldos/${C.archivoNombre||`${C.id}.backup`}</p>
                                    <p class="text-[10px] text-gray-500 font-mono truncate max-w-[260px]" title="${C.checksumSha256}">SHA: ${C.checksumSha256.slice(0,16)}... · ${C.fecha}</p>
                                </div>
                                <div class="text-right shrink-0">
                                    <span class="font-bold text-gray-600 text-[11px]">${C.registros} reg. (${C.tamanoKb} KB)</span>
            ...
            <div class="bg-white border-2 border-brand-black rounded-lg shadow-brutal p-4 sm:p-6 flex flex-col justify-between">
                ...
                <div class="mt-4 pt-3 border-t border-brand-black">
                    ...
                    `).join("")),this.mostrarToast("Copia de respaldo generada exitosamente.","success")}catch(w){this.mostrarToast("Error generando respaldo: "+(w instanceof Error?w.message:String(w)),"error")}finally{u.disabled=!1,u.textContent=k||"+ COPIA"}});const p=async()=>{var K,z,Y;const k=await D.obtenerJornadaActual(),w=await D.listarOperadores(),y=await D.obtenerSemaforoStock(),v=document.getElementById("jornada-estado-texto"),$=document.getElementById("jornada-fechas-texto"),j=document.getElementById("jornada-operador-activo"),G=document.getElementById("jornada-balance-texto"),V=document.getElementById("jornada-tickets-texto"),H=document.getElementById("box-acciones-jornada"),_=document.getElementById("lista-operadores-chips"),O=(k==null?void 0:k.estado)==="abierta";v&&(v.textContent=O?"Jornada en Curso":"Jornada Cerrada"),$&&($.textContent=O?`Inicio: ${(k==null?void 0:k.inicioStr)||"Reciente"}`:`Último cierre: ${(k==null?void 0:k.finStr)||"Sin jornadas"}`);const I=k!=null&&k.operadoresActivos&&k.operadoresActivos.length>0?k.operadoresActivos.join(", "):((K=k==null?void 0:k.operadorActual)==null?void 0:K.trim())||"Sin operador asignado";j&&(j.textContent=I),G&&(G.textContent=`$ ${ee((k==null?void 0:k.ventasTotalUsd)||0)} USD (Bs. ${ee((k==null?void 0:k.ventasTotalBs)||0)})`),V&&(V.textContent=`${(k==null?void 0:k.ticketsEmitidos)||0} tickets emitidos · ${(k==null?void 0:k.entradasStockReg)||0} reposiciones`),H&&(O?H.innerHTML=`
                ...
                </div>`,v.appendChild($);const j=()=>$.remove();(G=$.querySelector("#modal-asig-close"))==null||G.addEventListener("click",j),(V=$.querySelector("#modal-asig-cancelar"))==null||V.addEventListener("click",j),(H=$.querySelector("#modal-asig-guardar"))==null||H.addEventListener("click",async()=>{const _=$.querySelectorAll("input[data-asig-chk]:checked"),O=[];if(_.forEach(I=>{I.dataset.asigChk&&O.push(I.dataset.asigChk)}),O.length===0){this.mostrarToast("Selecciona al menos un operador para el turno.","error");return}await D.asignarOperadoresTurno(O),j(),p()})}),(f=document.getElementById("form-crear-metodo-panel"))==null||f.addEventListener("submit",async k=>{k.preventDefault();const w=document.getElementById("in-nuevo-metodo-nombre"),y=document.getElementById("select-nuevo-metodo-moneda"),v=(w==null?void 0:w.value.trim())||"",$=(y==null?void 0:y.value)==="USD"?"USD":"BS";v&&(await D.crearMetodoPago(v,$),w&&(w.value=""),p())}),(A=document.getElementById("form-crear-operador"))==null||A.addEventListener("submit",async k=>{k.preventDefault();const w=document.getElementById("in-nuevo-operador"),y=(w==null?void 0:w.value.trim())||"";y&&(await D.crearOperador(y),w&&(w.value=""),p())}),(h=document.getElementById("form-semaforo-stock"))==null||h.addEventListener("submit",async k=>{k.preventDefault();const w=document.getElementById("in-semaforo-rojo"),y=document.getElementById("in-semaforo-amarillo"),v=document.getElementById("msg-semaforo-stock"),$=parseInt((w==null?void 0:w.value)||"5",10),j=parseInt((y==null?void 0:y.value)||"15",10);if($>=j){this.mostrarToast("El umbral rojo debe ser estrictamente menor que el umbral amarillo.","error");return}await D.guardarSemaforoStock($,j),v&&(v.textContent="Umbrales del semáforo guardados y aplicados a todo el sistema.",v.classList.remove("hidden"),setTimeout(()=>v.classList.add("hidden"),2500))}),(E=document.getElementById("btn-reset-semaforo"))==null||E.addEventListener("click",async()=>{await D.guardarSemaforoStock(5,15),p()}),(C=document.getElementById("form-cambiar-clave"))==null||C.addEventListener("submit",async k=>{k.preventDefault();const w=document.getElementById("pin-actual"),y=document.getElementById("pin-nuevo"),v=document.getElementById("msg-cambio-clave");if(v)try{await D.cambiarPinDueno((w==null?void 0:w.value.trim())||"",(y==null?void 0:y.value.trim())||""),await this.modelo.cargarConfig(),v.textContent=y!=null&&y.value.trim()?"Clave actualizada exitosamente. El panel requerirá esta nueva clave para ingresar.":"Clave eliminada. El panel ha quedado en modo abierto sin clave.",v.style.color="#00823B",v.classList.remove("hidden"),setTimeout(()=>void this.render(),1200)}catch($){v.textContent=$ instanceof Error?$.message:String($),v.style.color="#C60C15",v.classList.remove("hidden")}})}aplicarFiltroFechas(e){const o=document.getElementById("filtro-fecha-desde"),a=document.getElementById("filtro-fecha-hasta"),r=document.getElementById("label-estado-fechas"),i=document.getElementById("tabla-transacciones-body"),c=document.getElementById("conteo-transacciones-badge"),b=(o==null?void 0:o.value.trim())||"",t=(a==null?void 0:a.value.trim())||"";if(!b&&!t){r&&(r.textContent="Selecciona una fecha Desde o Hasta para iniciar el diagnóstico.",r.classList.remove("hidden"));return}let n=0,d=Number.MAX_SAFE_INTEGER;if(b){const[P,T,K]=b.split("-").map(Number);n=Math.floor(new Date(P,T-1,K,0,0,0,0).getTime()/1e3)}if(t){const[P,T,K]=t.split("-").map(Number);d=Math.floor(new Date(P,T-1,K,23,59,59,999).getTime()/1e3)}if(n>d){r&&(r.textContent='Rango inválido: la fecha "Desde" es posterior a la fecha "Hasta".',r.classList.remove("hidden"));return}const s=this.transacciones.filter(P=>{const T=this.obtenerTimestampTicket(P);return T>=n&&T<=d}),u=s.reduce((P,T)=>P+re(T.totalUsd),0),p=s.reduce((P,T)=>P+re(T.totalBs),0),m=s.length;let g=0;s.forEach(P=>{(P.lineas||[]).forEach(T=>{const K=this.productos.find(z=>z.sku===T.sku);K&&(g+=re(K.precioBrutoUsd)*re(T.cantidad))})});const S=Math.max(0,u-g),f=S*.95,A=f*(this.modelo.tasaActual||807.39),h=m>0?(u/m).toFixed(2):"0.00",E=document.getElementById("kpi-ventas-usd"),C=document.getElementById("kpi-ventas-bs"),k=document.getElementById("kpi-tickets"),w=document.getElementById("kpi-gan-bruta"),y=document.getElementById("kpi-gan-neta"),v=document.getElementById("kpi-gan-neta-bs"),$=document.getElementById("kpi-ticket-prom");E&&(E.textContent="$ "+ee(u)),C&&(C.textContent="Bs. "+me(p),C.title="Bs. "+ee(p)),k&&(k.textContent=String(m)),w&&(w.textContent="$ "+ee(S)),y&&(y.textContent="$ "+ee(f)),v&&(v.textContent="Bs. "+me(A),v.title="Bs. "+ee(A)),$&&($.textContent="$ "+h),i&&(i.innerHTML=this.renderFilasTransacciones(s)),c&&(c.textContent=`${s.length} transacciones en rango`),this.contenedor.querySelectorAll("[data-rango]").forEach(P=>{P.className="w-12 sm:w-14 h-8 flex items-center justify-center font-heading font-bold text-xs rounded text-brand-black hover:bg-gray-100 transition-colors shrink-0"});const G={};s.forEach(P=>{(P.lineas||[]).forEach(T=>{G[T.nombre]||(G[T.nombre]={nombre:T.nombre,cantidad:0,totalUsd:0}),G[T.nombre].cantidad+=Number(T.cantidad)||0,G[T.nombre].totalUsd+=re(T.subtotalUsd)||Number(T.cantidad)*re(T.precioUsd)})});const V=Object.values(G).reduce((P,T)=>P+T.totalUsd,0)||1,H=Object.values(G).sort((P,T)=>T.totalUsd-P.totalUsd).map(P=>({nombre:P.nombre,cantidad:String(P.cantidad),totalUsd:P.totalUsd.toFixed(2),porcentaje:(P.totalUsd/V*100).toFixed(1)})),_={...e,ventas24hUsd:u.toFixed(2),ventas24hBs:p.toFixed(2),gananciaBrutaUsd:S.toFixed(2),gananciaNetaUsd:f.toFixed(2),tickets24h:m,topProductos:H.length>0?H:s.length===0?[]:e.topProductos};this.vm.setDatos(_),this.pintarGrafico(_);const O=P=>P.toLocaleString("es-VE",{year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit"}),I=b?new Date(n*1e3):new Date(2020,0,1),B=t?new Date(d*1e3):new Date;if(this.rangoDetalleFechas=`Del ${O(I)} al ${O(B)}`,r){const P=b||"Inicio",T=t||"Actualidad";r.textContent=`Diagnóstico activo (${P} al ${T}): ${s.length} transacciones registradas`,r.classList.remove("hidden")}}aplicarRango(e,o){const a=new Date,r=w=>w.toLocaleString("es-VE",{year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit"});let i=864e5;e==="7d"?i=7*864e5:e==="30d"?i=30*864e5:e==="1a"?i=365*864e5:e==="todo"&&(i=5*365*864e5);const c=new Date(a.getTime()-i);this.rangoDetalleFechas=`Del ${r(c)} al ${r(a)}`;const b=Math.floor(c.getTime()/1e3),t=this.transacciones.filter(w=>this.obtenerTimestampTicket(w)>=b),n=this.modelo.tasaActual||807.39;let d=0,s=0;t.forEach(w=>{d+=re(w.totalUsd),s+=re(w.totalBs)});const u=t.length,p=document.getElementById("kpi-ventas-usd"),m=document.getElementById("kpi-ventas-bs"),g=document.getElementById("kpi-tickets"),S=document.getElementById("kpi-gan-bruta"),f=document.getElementById("kpi-gan-neta"),A=document.getElementById("kpi-gan-neta-bs"),h=document.getElementById("kpi-ticket-prom");if(p&&(p.textContent="$ "+ee(d)),m&&(m.textContent="Bs. "+me(s),m.title="Bs. "+ee(s)),g&&(g.textContent=String(u)),S&&(S.textContent="$ "+ee(d*this.margenBrutoPct/100)),f&&(f.textContent="$ "+ee(d*this.margenNetoPct/100)),A){const w=d*this.margenNetoPct/100*n;A.textContent="Bs. "+me(w),A.title="Bs. "+ee(w)}h&&(h.textContent="$ "+(u>0?(d/u).toFixed(2):"0.00"));const E=new Map;t.forEach(w=>{var y;(y=w.lineas)==null||y.forEach(v=>{const $=E.get(v.sku);$?($.cantidad+=re(v.cantidad),$.totalUsd+=re(v.subtotalUsd)):E.set(v.sku,{nombre:v.nombre,cantidad:re(v.cantidad),totalUsd:re(v.subtotalUsd)})})});const C=Array.from(E.values()).sort((w,y)=>y.totalUsd-w.totalUsd).slice(0,10).map(w=>({nombre:w.nombre,cantidad:String(Math.round(w.cantidad)),totalUsd:w.totalUsd>0?String(w.totalUsd.toFixed(2)):void 0,porcentaje:d>0?(w.totalUsd/d*100).toFixed(1):"0.0"})),k={...o,topProductos:C};this.vm.setDatos(k),this.pintarGrafico(k)}kpi(e,o,a,r,i=""){return`
        <div class="border-2 border-brand-black rounded-lg shadow-brutal-sm p-4 ${r} flex flex-col justify-between min-w-0 overflow-hidden" title="${i}">
            ...
            `}).join(""),m=r?`${Math.round(c)} un.`:`$ ${ee(c)}`;o.innerHTML=`
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6 items-start">
            ...
            <div class="space-y-3">
                ...
                <div id="cuentas-lista" class="space-y-2 max-h-[65vh] overflow-y-auto pr-1">
                    ${r.length===0?`<div class="bg-white border-2 border-brand-black rounded-lg p-6 text-center text-brand-text font-bold">
                        ...
                        </div>`:(()=>{const d=Math.ceil(r.length/this.POR_PAGINA_CUENTAS);this.paginaCuentas>d&&(this.paginaCuentas=d),this.paginaCuentas<1&&(this.paginaCuentas=1);const s=(this.paginaCuentas-1)*this.POR_PAGINA_CUENTAS;return r.slice(s,s+this.POR_PAGINA_CUENTAS).map(p=>{var k;const m=p.tipo==="deuda",g=ae(p.totalParcialUsd),S=ae(p.abonosUsd),f=Math.max(0,g-S),A=Math.max(0,S-g),h=((k=this.cuentaSeleccionada)==null?void 0:k.ventaId)===p.ventaId,E=Te(p.fechaCreacionUnix);let C="cursor-pointer border-2 border-brand-black rounded-lg p-3 sm:p-4 transition-all ";return h?C+=m?"bg-amber-50 text-brand-black border-l-[6px] border-l-amber-600 shadow-brutal ring-1 ring-amber-500/20":"bg-blue-50 text-brand-black border-l-[6px] border-l-blue-600 shadow-brutal ring-1 ring-blue-500/20":C+=m?"bg-stone-50 border-l-4 border-l-amber-300 text-brand-black shadow-brutal-sm hover:bg-amber-50/60 hover:-translate-y-0.5":"bg-stone-50 border-l-4 border-l-blue-200 text-brand-black shadow-brutal-sm hover:bg-blue-50/50 hover:-translate-y-0.5",`
                        <div data-cuenta-id="${p.ventaId}" class="${C}">
                            <div class="flex justify-between items-start gap-2">
                                ...
                                <div class="text-right shrink-0">
                                    ...
                                    ${A>0?`<p class="text-[11px] font-black text-emerald-700">A favor: +$${R(A)}</p>`:f>0?`<p class="text-xs font-bold ${m?"text-amber-900":"text-blue-900"}">Pend: $${R(f)}</p>`:'<p class="text-[11px] font-bold text-gray-400">Cubierta ($0,00)</p>'}
                                </div>
                            </div>
            ...
            <div class="lg:col-span-2">
                ${this.cuentaSeleccionada?(()=>{var S;const d=this.cuentaSeleccionada.tipo==="deuda",s=ae(this.cuentaSeleccionada.totalParcialUsd),u=ae(this.cuentaSeleccionada.abonosUsd),p=Math.max(0,s-u),m=Math.max(0,u-s),g=Te(this.cuentaSeleccionada.fechaCreacionUnix);return`
                <div class="bg-white border-2 border-brand-black rounded-lg shadow-brutal p-4 sm:p-6 flex flex-col space-y-4">
                    ...
                    <div class="border-2 border-brand-black rounded-lg p-3 sm:p-4 bg-gray-50 shadow-sm flex flex-col">
                        ...
                        <div id="cta-consumos-lista" class="space-y-2 overflow-y-auto max-h-[300px] pr-1">
                            ${!this.cuentaSeleccionada.consumos||this.cuentaSeleccionada.consumos.length===0?`<div class="py-6 text-center text-xs text-gray-400 font-bold">
                                ...
                                </div>`:this.cuentaSeleccionada.consumos.map(f=>`
                                <div class="flex items-center justify-between border-2 border-brand-black rounded-lg bg-white px-3 py-2 text-xs font-bold shadow-brutal-sm hover:bg-amber-50/50 transition-colors gap-2">
                                    <div class="flex-1 min-w-0 pr-2">
                                        <p class="truncate text-brand-black font-heading font-black text-sm" title="${f.nombre}">
                                            ${f.nombre}
                                            ${f.modoVenta==="paquete"?'<span class="inline-block ml-1 text-[9px] font-black bg-purple-100 text-purple-800 border border-brand-purple rounded px-1">paquete</span>':""}
                                        </p>
                                        <p class="text-[11px] text-gray-500 font-semibold">${f.cantidad} un. &times; $${R(f.precioUsd)} <span class="text-gray-400 font-normal">· Bs. ${this.modelo.bs(ae(f.precioUsd))} c/u</span></p>
                                    </div>
                                    <div class="flex items-center gap-2 shrink-0">
                                        <div class="text-right mr-1">
                                            <p class="font-heading font-black text-sm sm:text-base text-brand-black leading-tight">$${R(f.subtotalUsd)}</p>
                                            <p class="text-[10px] font-bold text-gray-500 leading-tight">Bs. ${this.modelo.bs(ae(f.subtotalUsd))}</p>
                                        </div>
                                        <div class="flex items-center gap-1 border-2 border-brand-black rounded bg-gray-100 p-0.5">
                                            <button data-dec-consumo="${f.id}" data-sku="${f.sku}" data-cant="${f.cantidad}" data-modo="${f.modoVenta||"unidad"}" title="Disminuir 1 unidad" class="w-6 h-6 flex items-center justify-center rounded border border-brand-black bg-white hover:bg-amber-200 text-brand-black font-black text-xs active:scale-95">-</button>
                                            <span class="w-6 text-center font-black text-xs font-mono">${f.cantidad}</span>
                                            <button data-inc-consumo="${f.id}" data-sku="${f.sku}" data-modo="${f.modoVenta||"unidad"}" title="Aumentar 1 unidad" class="w-6 h-6 flex items-center justify-center rounded border border-brand-black bg-white hover:bg-emerald-200 text-brand-black font-black text-xs active:scale-95">+</button>
                                        </div>
                                        <button data-del-consumo="${f.id}" title="Eliminar todo el consumo y reintegrar stock al inventario" class="w-7 h-7 flex items-center justify-center rounded border-2 border-brand-black bg-red-100 hover:bg-red-500 hover:text-white text-red-800 text-sm font-black transition-colors active:scale-95">&times;</button>
                                    </div>
                                </div>
                            `).join("")}
                        </div>
                    ...
                    <div>
                        ...
                        <div id="cta-grid-prod" class="grid grid-cols-2 sm:grid-cols-3 gap-2 max-h-48 overflow-y-auto p-1">
                            ${this.productos.map(f=>{const A=!f.sinStock&&Number(f.stock)<=0,h=f.precioPaqueteUsd&&f.nombrePaquete||f.esCaja&&f.unidadesPorCaja&&f.unidadesPorCaja>1;return`
                                <div class="relative" data-producto-row="${f.sku}">
                                    <button data-add-sku="${f.sku}" data-modo="unidad" data-agotado="${A?"1":"0"}" class="text-left border-2 border-brand-black rounded p-2 transition-all text-xs font-bold ${A?"bg-gray-100 opacity-60 cursor-not-allowed":"bg-gray-50 hover:bg-white shadow-sm hover:shadow-brutal-sm"} w-full">
                                        <div class="flex justify-between items-start gap-1">
                                            <p class="truncate flex-1" title="${f.nombre}">${f.nombre}</p>
                                            ${A?'<span class="bg-red-200 text-red-900 border border-red-400 text-[9px] font-black px-1 rounded">AGOTADO</span>':""}
                                        </div>
                                        <div class="flex justify-between items-center mt-1">
                                            <p class="text-brand-black font-black">$${R(f.precioUsd)}</p>
                                            <span class="text-[10px] ${A?"text-red-700 font-black":"text-gray-500"}">${Pe(f)}</span>
                                        </div>
                                    </button>
                                    ${h?`
                                    <button data-add-sku="${f.sku}" data-modo="paquete" data-agotado="${A?"1":"0"}" class="text-left border-2 border-brand-purple rounded p-1 transition-all text-[10px] font-bold mt-1 w-full ${A?"bg-gray-100 opacity-60 cursor-not-allowed":"bg-purple-50 hover:bg-purple-100 shadow-sm hover:shadow-brutal-sm"}">
                                        <div class="flex justify-between items-center">
                                            <span class="text-purple-800">${f.nombrePaquete} ($${Number(f.precioPaqueteUsd).toFixed(2)})</span>
                                            <span class="text-purple-600">${f.unidadesPorCaja||1} un.</span>
                                        </div>
                                    </button>`:""}
                                </div>`}).join("")}
                        </div>
                    ...
                    <div class="border-t-2 border-brand-black pt-4 grid grid-cols-1 sm:grid-cols-${d?"3":"4"} gap-2 sm:gap-3">
                        ...
                        <button id="btn-cerrar-cuenta" class="${m>0?"bg-emerald-600 hover:bg-emerald-700":"bg-green-700 hover:bg-green-800"} text-white font-heading font-black text-xs py-3.5 rounded border-2 border-brand-black shadow-brutal active:translate-y-0.5">
                            ${d?"COBRAR Y LIQUIDAR DEUDA (TASA LIBRE)":m>0?`LIQUIDAR (+$${R(m)})`:"LIQUIDAR Y COBRAR"}
                        </button>
                    </div>
                </div>`})():`
        ...
        </div>`,this.vincularEventos()}vincularEventos(){var a,r,i,c,b,t,n,d,s,u,p,m;(a=document.getElementById("btn-historial-turno-cuentas"))==null||a.addEventListener("click",()=>{Me(this.modal)}),(r=document.getElementById("btn-nueva-cuenta"))==null||r.addEventListener("click",()=>this.modalNuevaCuenta("activa")),(i=document.getElementById("btn-nueva-deuda"))==null||i.addEventListener("click",()=>this.modalNuevaCuenta("deuda")),(c=document.getElementById("tab-todas"))==null||c.addEventListener("click",()=>{this.filtroTab="todas",this.paginaCuentas=1,this.render()}),(b=document.getElementById("tab-activas"))==null||b.addEventListener("click",()=>{this.filtroTab="activa",this.paginaCuentas=1,this.render()}),(t=document.getElementById("tab-deudas"))==null||t.addEventListener("click",()=>{this.filtroTab="deuda",this.paginaCuentas=1,this.render()}),(n=document.getElementById("cta-pag-ant"))==null||n.addEventListener("click",()=>{this.paginaCuentas>1&&(this.paginaCuentas--,this.render())}),(d=document.getElementById("cta-pag-sig"))==null||d.addEventListener("click",()=>{this.paginaCuentas++,this.render()}),this.contenedor.querySelectorAll("[data-cuenta-id]").forEach(g=>{g.addEventListener("click",()=>{const S=g.dataset.cuentaId;this.cuentaSeleccionada=this.cuentas.find(f=>f.ventaId===S)||null,this.render()})}),this.contenedor.querySelectorAll("[data-inc-consumo]").forEach(g=>{g.addEventListener("click",async()=>{if(!this.cuentaSeleccionada)return;const S=g,f=S.dataset.sku||"",A=S.dataset.modo||"unidad";try{const h=await D.agregarConsumo(this.cuentaSeleccionada.ventaId,f,"1",!0,A);this.cuentaSeleccionada=h,this.render()}catch(h){this.mostrarToast(h instanceof Error?h.message:String(h),"error")}})}),this.contenedor.querySelectorAll("[data-dec-consumo]").forEach(g=>{g.addEventListener("click",async()=>{if(!this.cuentaSeleccionada)return;const S=g,f=S.dataset.decConsumo||"";if(!(Number(S.dataset.cant||"1")<=1&&!await ce("¿Deseas quitar este producto de la cuenta y devolverlo al inventario?","ELIMINAR CONSUMO")))try{const h=await D.eliminarConsumo(this.cuentaSeleccionada.ventaId,f,1);this.cuentaSeleccionada=h,this.render()}catch(h){this.mostrarToast(h instanceof Error?h.message:String(h),"error")}})}),this.contenedor.querySelectorAll("[data-del-consumo]").forEach(g=>{g.addEventListener("click",async()=>{if(!this.cuentaSeleccionada)return;const S=g.dataset.delConsumo||"";if(await ce("¿Deseas eliminar todo este consumo y reintegrar las unidades al inventario?","ELIMINAR CONSUMO"))try{const A=await D.eliminarConsumo(this.cuentaSeleccionada.ventaId,S);this.cuentaSeleccionada=A,this.render()}catch(A){this.mostrarToast(A instanceof Error?A.message:String(A),"error")}})}),(s=document.getElementById("btn-convertir-deuda"))==null||s.addEventListener("click",async()=>{if(!this.cuentaSeleccionada)return;const g=this.cuentaSeleccionada,S=g.cliente||g.etiqueta||"",f=await pe("Nombre o titular responsable de la deuda comercial:",S,"CONVERTIR A DEUDA COMERCIAL");if(!f||!f.trim())return;const A=g.nota||"",h=await pe("Nota o plazo de compromiso de pago (opcional):",A,"PLAZO / COMPROMISO");try{const E=await D.convertirCuentaADeuda(g.ventaId,f.trim(),h?h.trim():void 0);this.cuentaSeleccionada=E,this.filtroTab="deuda",this.mostrarToast("Cuenta convertida a deuda comercial correctamente.","success"),this.render()}catch(E){this.mostrarToast(E instanceof Error?E.message:String(E),"error")}});const e=this.contenedor.querySelector("#cta-buscar-prod");e&&$e(e,40),e==null||e.addEventListener("input",()=>{const g=e.value.trim().toLowerCase(),S=this.contenedor.querySelector("#cta-grid-prod");if(S){const f=this.productos.filter(A=>!g||A.nombre.toLowerCase().includes(g));f.length===0?S.innerHTML='<p class="col-span-full text-center text-xs text-gray-400 py-4 font-bold">No se encontraron productos coincidentes.</p>':(S.innerHTML=f.map(A=>{const h=!A.sinStock&&Number(A.stock)<=0,E=A.precioPaqueteUsd&&A.nombrePaquete||A.esCaja&&A.unidadesPorCaja&&A.unidadesPorCaja>1;return`
                        <div class="relative" data-producto-row="${A.sku}">
                            <button data-add-sku="${A.sku}" data-modo="unidad" data-agotado="${h?"1":"0"}" class="text-left border-2 border-brand-black rounded p-2 transition-all text-xs font-bold ${h?"bg-gray-100 opacity-60 cursor-not-allowed":"bg-gray-50 hover:bg-white shadow-sm hover:shadow-brutal-sm"} w-full">
                                <div class="flex justify-between items-start gap-1">
                                    <p class="truncate flex-1" title="${A.nombre}">${A.nombre}</p>
                                    ${h?'<span class="bg-red-200 text-red-900 border border-red-400 text-[9px] font-black px-1 rounded">AGOTADO</span>':""}
                                </div>
                                <div class="flex justify-between items-center mt-1">
                                    <p class="text-brand-black font-black">$${R(A.precioUsd)}</p>
                                    <span class="text-[10px] ${h?"text-red-700 font-black":"text-gray-500"}">${Pe(A)}</span>
                                </div>
                            </button>
                            ${E?`
                            <button data-add-sku="${A.sku}" data-modo="paquete" data-agotado="${h?"1":"0"}" class="text-left border-2 border-brand-purple rounded p-1 transition-all text-[10px] font-bold mt-1 w-full ${h?"bg-gray-100 opacity-60 cursor-not-allowed":"bg-purple-50 hover:bg-purple-100 shadow-sm hover:shadow-brutal-sm"}">
                                <div class="flex justify-between items-center">
            ...
            </div>`;const i=this.modal.querySelector("#modal-tab-activa"),c=this.modal.querySelector("#modal-tab-deuda");i==null||i.addEventListener("click",()=>{o="activa",a()}),c==null||c.addEventListener("click",()=>{o="deuda",a()});const b=this.modal.querySelector("#modal-cta-nombre"),t=this.modal.querySelector("#modal-cta-nota");b&&$e(b,40),t&&$e(t,60),setTimeout(()=>b==null?void 0:b.focus(),50);const n=()=>{this.modal.innerHTML=""};(d=this.modal.querySelector("#modal-cta-cancel"))==null||d.addEventListener("click",n),(s=this.modal.querySelector("#modal-cta-ok"))==null||s.addEventListener("click",async()=>{const u=b==null?void 0:b.value.trim().slice(0,40),p=t==null?void 0:t.value.trim().slice(0,60);if(u){const m=await D.abrirCuenta(u,o,p,u);this.cuentaSeleccionada=m,o==="deuda"&&(this.filtroTab="deuda"),n(),this.render()}else this.mostrarToast(r?"Debe ingresar el nombre del cliente o empresa deudora.":"Debe ingresar un identificador válido para la cuenta.","error"),b==null||b.focus()})};a()}async modalAbono(){if(!this.cuentaSeleccionada)return;let e=[];try{e=await D.listarMetodosPago()}catch{e=[{nombre:"PUNTOD.VENTA",moneda:"BS"},{nombre:"BIOPAGO",moneda:"BS"},{nombre:"PAGO MOVIL",moneda:"BS"},{nombre:"TRANSF.BS.",moneda:"BS"},{nombre:"BS.EFEC.",moneda:"BS"},{nombre:"DOL.CASH",moneda:"USD"},{nombre:"ZELLE",moneda:"USD"},{nombre:"BINAN.USDT",moneda:"USD"}]}const o=ae(this.cuentaSeleccionada.totalParcialUsd),a=ae(this.cuentaSeleccionada.abonosUsd),r=Math.max(0,o-a),i=this.modelo.tasaActual||807.39,c=(r*i).toFixed(2),b=this.cuentaSeleccionada.tipo==="deuda";let t=e.find(g=>g.nombre==="BIOPAGO")||e[0]||{nombre:"BIOPAGO",moneda:"BS"},n=t.moneda==="USD"?r:Number(c),d=i,s="";const u=()=>{var G;const g=e.filter(V=>V.moneda==="USD"),S=e.filter(V=>V.moneda==="BS"),f=g.map(V=>`<option value="${V.nombre}" ${V.nombre===t.nombre?"selected":""}>${V.nombre} ($ USD - Tasa Dinámica)</option>`).join(""),h=`
                <optgroup label="MÉTODOS EN BOLÍVARES (Bs.)">
            ...
            `,E=t.moneda==="USD",C=E&&d>0?d:i,k=E?n*C:n,w=i>0?k/i:0,y=a+w,v=y-o,$=v*i;let j="";if(n<=1e-4)j=`
                ...
                `;else if(v>.001)j=`
                    <div class="rounded p-2.5 border-2 border-emerald-600 bg-emerald-50 text-xs text-emerald-950 font-bold">
                        ...
                        <div class="flex justify-between items-center text-emerald-900 border-t border-emerald-300 pt-1">
                            ...
                            <span class="font-heading font-black text-base text-emerald-600">+$${R(v)} USD (Bs. ${R($)})</span>
                        </div>
            ...
            <div class="fixed inset-0 bg-black/40 flex items-center justify-center z-[100] p-4">
                <div class="bg-white border-2 border-brand-black rounded-lg shadow-brutal p-6 w-full max-w-md">
                    ...
                    <div class="space-y-3 mb-4">
                        ...
                        <div>
                            ...
                            <input id="input-tasa-abono" type="number" step="0.01" min="1" max="100000" value="${C}"
            ...
            </div>`,m()},p=()=>{const g=t.moneda==="USD",S=g&&d>0?d:i,f=g?n*S:n,A=i>0?f/i:0,h=a+A,E=h-o,C=E*i;let k="";if(n<=1e-4)k=`
                    <div class="rounded p-2.5 border-2 border-brand-black bg-gray-50 text-xs text-gray-700 font-bold">
                        <div class="flex justify-between items-center">
                            <span>Aporte de este abono:</span>
                            <span class="font-heading font-black text-sm text-gray-800">$0.00 USD (Bs. 0.00)</span>
                ...
                `;else if(E>.001)k=`
                    <div class="rounded p-2.5 border-2 border-emerald-600 bg-emerald-50 text-xs text-emerald-950 font-bold">
                        <div class="flex justify-between items-center mb-1">
                            <span>Nuevo Total Abonado:</span>
                            <span class="font-heading font-black text-sm text-emerald-700">$${R(h)}</span>
                        </div>
                        <div class="flex justify-between items-center text-emerald-900 border-t border-emerald-300 pt-1">
                            <span>Saldo a favor resultante:</span>
                            <span class="font-heading font-black text-base text-emerald-600">+$${R(E)} USD (Bs. ${R(C)})</span>
                        </div>
                        <p class="text-[10px] text-emerald-800 mt-1">✓ Abono superior al consumo: este excedente se convertirá en ganancia o crédito al liquidar.</p>
                    </div>
                `;else if(E<-.001){const v=Math.abs(E),$=v*i;k=`
                    <div class="rounded p-2.5 border-2 border-brand-black bg-gray-50 text-xs text-gray-800 font-bold">
                        <div class="flex justify-between items-center mb-1">
                            <span>Nuevo Total Abonado:</span>
                            <span class="font-heading font-black text-sm text-brand-black">$${R(h)}</span>
                        </div>
                        <div class="flex justify-between items-center text-gray-900 border-t border-gray-300 pt-1">
                            <span>Saldo pendiente restante:</span>
                            <span class="font-heading font-black text-base text-brand-black">$${R(v)} USD (Bs. ${R($)})</span>
                        </div>
                    </div>
                `}else k=`
                    <div class="rounded p-2.5 border-2 border-green-600 bg-green-50 text-xs text-green-950 font-bold">
                        <div class="flex justify-between items-center">
                            <span>Registro saldado exactamente:</span>
                            <span class="font-heading font-black text-base text-green-700">$${R(h)} USD</span>
                        </div>
                    </div>
                `;const w=this.modal.querySelector("#abono-preview-container");w&&(w.innerHTML=k);const y=this.modal.querySelector("#modal-abono-ok");if(y&&(y.disabled=n<=0),g){const v=this.modal.querySelector("[data-aporte-bs]");v&&(v.textContent=`Bs. ${R(f)}`);const $=this.modal.querySelector("[data-abono-computable]");$&&($.textContent=`$ ${R(A)} USD`)}},m=()=>{var S,f,A,h,E,C,k;const g=()=>{this.modal.innerHTML=""};(S=this.modal.querySelector("#modal-abono-cancel"))==null||S.addEventListener("click",g),(f=this.modal.querySelector("#select-metodo-abono"))==null||f.addEventListener("change",w=>{const y=w.target.value,v=e.find($=>$.nombre===y);if(v){const $=t.moneda==="USD";t=v,v.moneda==="USD"?(d=i,$||(n=r)):$&&(n=Number(c)),u()}}),(A=this.modal.querySelector("#input-monto-abono"))==null||A.addEventListener("input",w=>{n=ae(w.target.value),p()}),(h=this.modal.querySelector("#input-tasa-abono"))==null||h.addEventListener("input",w=>{d=ae(w.target.value),p()}),(E=this.modal.querySelector("#btn-reset-tasa-abono"))==null||E.addEventListener("click",()=>{d=i,u()}),(C=this.modal.querySelector("#input-ref-abono"))==null||C.addEventListener("input",w=>{s=w.target.value}),(k=this.modal.querySelector("#modal-abono-ok"))==null||k.addEventListener("click",async()=>{if(!this.cuentaSeleccionada)return;const w=t.moneda==="USD",y=w&&d>0?d:i,v=w?n*y:n,$=i>0?v/i:0;if($<=0||!Number.isFinite($)){this.mostrarToast("Por favor ingrese un monto válido.","error");return}const j=await D.abonarCuenta(this.cuentaSeleccionada.ventaId,$,v);j&&(this.cuentaSeleccionada=j),g(),this.render()})};u()}modalEditarAbono(){var S,f,A,h;if(!this.cuentaSeleccionada)return;const e=ae(this.cuentaSeleccionada.totalParcialUsd),o=ae(this.cuentaSeleccionada.abonosUsd),a=this.modelo.tasaActual||807.39;this.modal.innerHTML=`
        <div class="fixed inset-0 bg-black/40 flex items-center justify-center z-[100] p-4">
            <div class="bg-white border-2 border-brand-black rounded-lg shadow-brutal p-6 w-full max-w-md">
                ...
                <div class="grid grid-cols-2 gap-3">
                    ...
                    <button id="modal-edit-ok" class="bg-amber-400 hover:bg-amber-500 text-brand-black font-heading font-black py-3 rounded border-2 border-brand-black shadow-brutal-sm">GUARDAR CORRECCIÓN</button>
        ...
        </div>`;const r=this.modal.querySelector("#tab-edit-usd"),i=this.modal.querySelector("#tab-edit-bs"),c=this.modal.querySelector("#sec-edit-usd"),b=this.modal.querySelector("#sec-edit-bs"),t=this.modal.querySelector("#edit-abono-usd-val"),n=this.modal.querySelector("#edit-abono-bs-val"),d=this.modal.querySelector("#edit-usd-equiv"),s=this.modal.querySelector("#edit-bs-equiv"),u=this.modal.querySelector("#edit-preview-box");let p="usd";const m=()=>{let E=0;p==="usd"?(E=ae(t==null?void 0:t.value),d&&(d.textContent=`Bs. ${(E*a).toFixed(2)}`)):(E=ae(n==null?void 0:n.value)/a,s&&(s.textContent=`$${E.toFixed(2)} USD`)),Number.isFinite(E)||(E=0);const C=E-e;if(u)if(C>.001)u.className="rounded p-2.5 border-2 border-emerald-600 bg-emerald-50 text-xs text-emerald-950 font-bold",u.innerHTML=`
                        <div class="flex justify-between items-center mb-1">
                        ...
                        <div class="flex justify-between items-center text-emerald-900 border-t border-emerald-300 pt-1">
                            <span>Saldo a favor resultante:</span>
                            <span class="font-heading font-black text-base text-emerald-600">+$${R(C)} USD</span>
                        </div>
                        <p class="text-[10px] text-emerald-800 mt-1">✓ Excedente a favor del cliente.</p>
                    `;else if(C<-.001){const k=Math.abs(C);u.className="rounded p-2.5 border-2 border-brand-black bg-gray-50 text-xs text-gray-800 font-bold",u.innerHTML=`
                        <div class="flex justify-between items-center mb-1">
                            <span>Nuevo Saldo Abonado:</span>
                    ...
                    `};t&&Se(t,5e4,8,m),n&&Se(n,5e7,11,m),m(),(S=this.modal.querySelector("#btn-preset-cero"))==null||S.addEventListener("click",async()=>{const E=ae(p==="usd"?t==null?void 0:t.value:n==null?void 0:n.value);(o>0||E>0)&&!await ce("¿Desea restablecer el saldo abonado a $0.00? Esta acción dejará los pagos registrados en cero.","RESTABLECER ABONO")||(t&&(t.value="0.00"),n&&(n.value="0.00"),m())}),(f=this.modal.querySelector("#btn-preset-exacto"))==null||f.addEventListener("click",()=>{t&&(t.value=e.toFixed(2)),n&&(n.value=(e*a).toFixed(2)),m()}),r==null||r.addEventListener("click",()=>{p="usd",r.className="flex-1 py-1.5 font-heading font-black text-xs rounded bg-brand-black text-white",i.className="flex-1 py-1.5 font-heading font-bold text-xs rounded text-brand-black hover:bg-white",c==null||c.classList.remove("hidden"),b==null||b.classList.add("hidden"),m()}),i==null||i.addEventListener("click",()=>{p="bs",i.className="flex-1 py-1.5 font-heading font-black text-xs rounded bg-brand-black text-white",r.className="flex-1 py-1.5 font-heading font-bold text-xs rounded text-brand-black hover:bg-white",b==null||b.classList.remove("hidden"),c==null||c.classList.add("hidden"),m()});const g=()=>{this.modal.innerHTML=""};(A=this.modal.querySelector("#modal-edit-cancel"))==null||A.addEventListener("click",g),(h=this.modal.querySelector("#modal-edit-ok"))==null||h.addEventListener("click",async()=>{if(!this.cuentaSeleccionada)return;let E=0;if(p==="usd"?E=ae(t==null?void 0:t.value):E=ae(n==null?void 0:n.value)/a,!Number.isFinite(E)||E<0||E>5e4){this.mostrarToast("Monto inválido. Ingrese un valor entre $0.00 y $50,000.00 USD.","error");return}if(Math.abs(E-o)>.009){const w=E===0?`¿Confirmas reiniciar el saldo abonado a $0.00? (Monto previo registrado: $${R(o)})`:`¿Confirmas modificar el saldo abonado de esta cuenta a $${R(E)} USD (Bs. ${R(E*a)})? (Monto previo registrado: $${R(o)})`;if(!await ce(w,"MODIFICAR ABONO"))return}const k=await D.editarAbonoCuenta(this.cuentaSeleccionada.ventaId,E);k&&(this.cuentaSeleccionada=k),g(),this.render()})}async modalCierre(){var k,w,y;if(!this.cuentaSeleccionada)return;const e=this.cuentaSeleccionada.tipo==="deuda",o=ae(this.cuentaSeleccionada.totalParcialUsd),a=ae(this.cuentaSeleccionada.abonosUsd),r=Math.max(0,Math.round((o-a)*100)/100),i=Math.max(0,Math.round((a-o)*100)/100),c=this.modelo.tasaActual||807.39;if(r<=.009){this.modal.innerHTML=`
            <div class="fixed inset-0 bg-black/40 flex items-center justify-center z-[100] p-4">
                <div class="bg-white border-2 border-brand-black rounded-lg shadow-brutal p-6 w-full max-w-md">
            ...
            </div>`;const v=()=>{this.modal.innerHTML=""};(k=this.modal.querySelector("#modal-liq-cancel"))==null||k.addEventListener("click",v);let $=!1;(w=this.modal.querySelector("#modal-liq-ok"))==null||w.addEventListener("click",async()=>{if(!$&&this.cuentaSeleccionada){$=!0;try{await D.cerrarCuenta(this.cuentaSeleccionada.ventaId,"0.00",c.toFixed(4)),this.cuentaSeleccionada=null,v(),this.render()}catch{$=!1}}});return}let b=[];try{b=await D.listarMetodosPago()}catch{b=[{nombre:"PUNTOD.VENTA",moneda:"BS"},{nombre:"BIOPAGO",moneda:"BS"},{nombre:"PAGO MOVIL",moneda:"BS"},{nombre:"TRANSF.BS.",moneda:"BS"},{nombre:"BS.EFEC.",moneda:"BS"},{nombre:"DOL.CASH",moneda:"USD"},{nombre:"ZELLE",moneda:"USD"},{nombre:"BINAN.USDT",moneda:"USD"}]}let t=c;const n=e?b.find(v=>v.nombre==="TRANSF.BS.")||b[0]||{nombre:"PUNTOD.VENTA",moneda:"BS"}:b.find(v=>v.nombre==="PUNTOD.VENTA")||b[0]||{nombre:"PUNTOD.VENTA",moneda:"BS"},d=this.cuentaSeleccionada.ventaId,s=this.pagosBorradorPorCuenta.get(d);let u;if(s&&s.length>0){if(u=s.map(v=>({...v})),u.length===1&&u[0]){const v=u[0];v.monto=v.moneda==="USD"?r:Number((r*t).toFixed(2)),v.tasaCambio=t}else if(u.length>1){const v=Number((r*t).toFixed(2));let $=0;for(let V=0;V<u.length-1;V++){const H=u[V],_=H.tasaCambio>0?H.tasaCambio:t;$+=H.moneda==="USD"?H.monto*_:H.monto}const j=Math.max(0,Math.round((v-$)*100)/100),G=u[u.length-1];G&&(G.monto=G.moneda==="USD"?t>0?Number((j/t).toFixed(2)):0:j,G.tasaCambio=t)}}else u=[{id:"p-1",metodo:n.nombre,moneda:n.moneda,monto:n.moneda==="USD"?r:Number((r*t).toFixed(2)),tasaCambio:t,referencia:""}];const p=this.resolucionVueltoPorCuenta.get(d);let m=(p==null?void 0:p.estado)||"PAGADO",g=(p==null?void 0:p.metodo)||(e?"TRANSF.BS.":"BS.EFEC."),S=(p==null?void 0:p.tasa)||t;b.some(v=>v.nombre===g)||(g=((y=b[0])==null?void 0:y.nombre)||(e?"TRANSF.BS.":"BS.EFEC."));let f=!1;const A=()=>{let v=0;for(const I of u)if(I.moneda==="USD"){const B=I.tasaCambio>0?I.tasaCambio:t;v+=I.monto*B}else v+=I.monto;v=Math.round(v*100)/100;const $=Math.round(r*t*100)/100,j=t>0?Math.round(v/t*100)/100:0,G=Math.max(0,Math.round(($-v)*100)/100),V=t>0?Math.max(0,Math.round(G/t*100)/100):0,H=Math.max(0,Math.round((v-$)*100)/100),_=t>0?Math.max(0,Math.round(H/t*100)/100):0,O=v>=$-.01&&u.every(I=>I.monto>0);return{cubiertoUsd:j,cubiertoBs:v,totalEsperadoBs:$,faltanteUsd:V,faltanteBs:G,vueltoUsd:_,vueltoBs:H,puedeConfirmar:O}},h=v=>{const $=b.find(H=>H.nombre===g),j=($==null?void 0:$.moneda)==="USD",G=S>0?S:t,V=j&&G>0?Number((v.vueltoBs/G).toFixed(2)):v.vueltoUsd;return v.faltanteBs>.009?`
            ...
            <div class="bg-emerald-50 border-2 border-emerald-600 rounded p-2.5 text-center mb-3">
                <p class="text-xs font-black uppercase text-emerald-800">Total Exactamente Cubierto</p>
                <p class="text-xs text-emerald-700 font-bold">Importe liquidado sin diferencia pendiente.</p>
            </div>`},E=()=>{var H,_;const v=A(),$=b.filter(O=>O.moneda==="USD"),j=b.filter(O=>O.moneda==="BS"),G=u.map((O,I)=>{const B=$.map(N=>`<option value="${N.nombre}" ${N.nombre===O.metodo?"selected":""}>${N.nombre} ($ - Tasa Dinámica)</option>`).join(""),T=`
                        <optgroup label="MÉTODOS EN BOLÍVARES (Bs.)">
                    ...
                    <div data-pago-card="${O.id}" class="border-2 border-brand-black rounded-lg p-3 bg-gray-50 mb-2.5">
                        ...
                        `}
                    </div>`}).join(""),V=`<div id="contenedor-estado-balance-cuenta">${h(v)}</div>`;this.modal.innerHTML=`
            <div class="fixed inset-0 bg-black/40 flex items-center justify-center z-[100] p-4">
            ...
            </div>`,C()},C=()=>{var _,O,I,B,P,T,K,z,Y,N,q;const v=()=>{this.cuentaSeleccionada&&(this.pagosBorradorPorCuenta.set(this.cuentaSeleccionada.ventaId,u),this.resolucionVueltoPorCuenta.set(this.cuentaSeleccionada.ventaId,{estado:m,metodo:g,tasa:S}))},$=()=>{v(),this.modal.innerHTML=""};(_=this.modal.querySelector("#modal-liq-cerrar"))==null||_.addEventListener("click",$),(O=this.modal.querySelector("#modal-liq-cancel"))==null||O.addEventListener("click",$),(I=this.modal.querySelector("#modal-liq-agregar-mas"))==null||I.addEventListener("click",()=>{v(),this.modal.innerHTML=""}),(B=this.modal.querySelector("#btn-vuelto-pagado-cuenta"))==null||B.addEventListener("click",()=>{m="PAGADO",v(),E()}),(P=this.modal.querySelector("#btn-vuelto-retenido-cuenta"))==null||P.addEventListener("click",()=>{m="RETENIDO",v(),E()}),(T=this.modal.querySelector("#select-metodo-vuelto-cuenta"))==null||T.addEventListener("change",L=>{g=L.target.value,v(),E()});const j=this.modal.querySelector("#input-tasa-vuelto-cuenta");if(j&&(j.addEventListener("input",L=>{const M=ae(L.target.value);S=M>0?M:t,v()}),j.addEventListener("blur",()=>{E()})),(K=this.modal.querySelector("#btn-reset-tasa-vuelto-cuenta"))==null||K.addEventListener("click",()=>{S=t,v(),E()}),e){const L=this.modal.querySelector("#deuda-tasa-input"),M=this.modal.querySelector("#btn-tasa-bcv");L&&Se(L,1e5,9,()=>{const F=ae(L.value);t=Number.isFinite(F)&&F>0?F:c,v(),E()}),M==null||M.addEventListener("click",()=>{t=c,v(),E()})}this.modal.querySelectorAll("select[data-pago-metodo]").forEach(L=>{L.addEventListener("change",M=>{const F=L.dataset.pagoMetodo,te=M.target.value,Z=u.find(oe=>oe.id===F);if(Z){const oe=b.find(le=>le.nombre===te),ie=(oe==null?void 0:oe.moneda)||"BS";Z.moneda=ie,Z.tasaCambio=t,u.length===1?Z.monto=Z.moneda==="USD"?r:Number((r*t).toFixed(2)):ie!==Z.moneda&&(ie==="USD"?Z.monto=Number((t>0?Z.monto/t:0).toFixed(2)):Z.monto=Number((Z.monto*t).toFixed(2))),Z.metodo=te,v(),E()}})});const G=()=>{var te,Z,oe,ie;const L=A(),M=this.modal.querySelector("#contenedor-estado-balance-cuenta");if(M){M.innerHTML=h(L),(te=this.modal.querySelector("#btn-vuelto-pagado-cuenta"))==null||te.addEventListener("click",()=>{m="PAGADO",v(),E()}),(Z=this.modal.querySelector("#btn-vuelto-retenido-cuenta"))==null||Z.addEventListener("click",()=>{m="RETENIDO",v(),E()}),(oe=this.modal.querySelector("#select-metodo-vuelto-cuenta"))==null||oe.addEventListener("change",ve=>{g=ve.target.value,v(),E()});const le=this.modal.querySelector("#input-tasa-vuelto-cuenta");le&&(le.addEventListener("input",ve=>{const xe=ae(ve.target.value);S=xe>0?xe:t,v()}),le.addEventListener("blur",()=>E())),(ie=this.modal.querySelector("#btn-reset-tasa-vuelto-cuenta"))==null||ie.addEventListener("click",()=>{S=t,v(),E()})}const F=this.modal.querySelector("#modal-liq-ok");F&&(F.disabled=!L.puedeConfirmar)},V=L=>{const M=this.modal.querySelector(`div[data-aporte-info-cuenta="${L.id}"]`);if(!M)return;const F=L.tasaCambio>0?L.tasaCambio:t,te=L.moneda==="USD"?L.monto*F:L.monto,Z=t>0?te/t:0;L.moneda==="USD"?M.innerHTML=`
        ...
        <div id="inv-seccion-ajustes" class="${this.tabActiva==="ajustes"?"block":"hidden"} grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="border border-brand-black rounded p-2 bg-gray-50 flex justify-between items-center text-xs">
                <div class="min-w-0 flex-1 pr-2">
                    ...
                    <p class="text-[10px] text-gray-500 font-bold">
                        <span class="text-brand-purple font-black">${m}</span> · ${S?"•••":`$${Number(d.precioUsd).toFixed(2)}`} · IVA ${d.impuestoPct}%
                    </p>
                </div>
                <div class="text-right shrink-0">
            ...
            <div class="border-2 border-brand-black rounded-lg p-3 bg-white shadow-sm flex flex-col justify-between h-[175px] min-h-[175px] max-h-[175px] box-border overflow-hidden">
                ...
                </div>`,e.insertAdjacentElement("afterend",s),(n=document.getElementById("repo-pag-ant"))==null||n.addEventListener("click",()=>{this.paginaRepo>1&&(this.paginaRepo--,this.renderRepoLista())}),(d=document.getElementById("repo-pag-sig"))==null||d.addEventListener("click",()=>{this.paginaRepo<i&&(this.paginaRepo++,this.renderRepoLista())})}e.querySelectorAll("input[data-repo-cant]").forEach(s=>{ge(s,9999,5)}),e.querySelectorAll("[data-repo-del]").forEach(s=>{s.addEventListener("click",async()=>{var S;const u=s.dataset.repoDel,p=this.productos.find(f=>f.sku===u);if(!u||!p)return;if(((S=this.modelo.getConfig())==null?void 0:S.privacidadInventario)&&!this.duenoAutenticado){be("Acción no permitida en modo operador. Solicite al Dueño(a).","error");return}if(!this.duenoAutenticado&&this.modelo.hasPinSet()){const f=await pe("Acción protegida. Ingrese el PIN de Administrador(a) / Dueño(a):","","AUTENTICACIÓN DUEÑO(A)");if(!f||!await this.modelo.verificarPin(f)){be("PIN incorrecto o no suministrado. Acción cancelada.","error");return}}await ce(`¿Seguro que deseas eliminar permanentemente el producto "${p.nombre}" del catálogo?`,"ELIMINAR PRODUCTO")&&D.eliminarProducto(u).then(()=>{D.productos().then(f=>{this.productos=f,this.renderRepoLista(),this.renderCatalogoLista();const A=document.getElementById("repo-contador-estado");A&&(A.textContent=`${this.productos.length} artículos en depósito`)})})})}),e.querySelectorAll("[data-repo-in]").forEach(s=>{s.addEventListener("click",async()=>{var g;const u=s.dataset.repoIn,p=e.querySelector(`input[data-repo-cant="${u}"]`),m=p==null?void 0:p.value;if(u&&m&&Number(m)>0&&Number(m)<=9999){if(((g=this.modelo.getConfig())==null?void 0:g.privacidadInventario)&&!this.duenoAutenticado){be("Acción no permitida en modo operador. Solicite al Dueño(a).","error");return}if(!this.duenoAutenticado&&this.modelo.hasPinSet()){const C=await pe("Acción de inventario protegida. Ingrese el PIN de Administrador(a) / Dueño(a):","","AUTENTICACIÓN DUEÑO(A)");if(!C||!await this.modelo.verificarPin(C)){be("PIN incorrecto o no suministrado. Operación cancelada.","error");return}}const f=e.querySelector(`select[data-repo-tipo="${u}"]`),A=(f==null?void 0:f.value)||"unidad",h=this.productos.find(C=>C.sku===u);let E=Number(m);A==="caja"&&(h!=null&&h.esCaja)&&(h!=null&&h.unidadesPorCaja)&&h.unidadesPorCaja>1&&(E=E*h.unidadesPorCaja),D.compraStock(u,String(E)).then(()=>{D.productos().then(C=>{this.productos=C,this.renderRepoLista(),this.renderCatalogoLista()})})}})}),e.querySelectorAll("[data-repo-red]").forEach(s=>{s.addEventListener("click",async()=>{var g;const u=s.dataset.repoRed,p=e.querySelector(`input[data-repo-cant="${u}"]`),m=p==null?void 0:p.value;if(u&&m&&Number(m)>0&&Number(m)<=9999){if(((g=this.modelo.getConfig())==null?void 0:g.privacidadInventario)&&!this.duenoAutenticado){be("Acción no permitida en modo operador. Solicite al Dueño(a).","error");return}if(!this.duenoAutenticado&&this.modelo.hasPinSet()){const C=await pe("Acción de inventario protegida. Ingrese el PIN de Administrador(a) / Dueño(a):","","AUTENTICACIÓN DUEÑO(A)");if(!C||!await this.modelo.verificarPin(C)){be("PIN incorrecto o no suministrado. Operación cancelada.","error");return}}const f=e.querySelector(`select[data-repo-tipo="${u}"]`),A=(f==null?void 0:f.value)||"unidad",h=this.productos.find(C=>C.sku===u);let E=Number(m);A==="caja"&&(h!=null&&h.esCaja)&&(h!=null&&h.unidadesPorCaja)&&h.unidadesPorCaja>1&&(E=E*h.unidadesPorCaja),D.reducirStock(u,String(E)).then(()=>{D.productos().then(C=>{this.productos=C,this.renderRepoLista(),this.renderCatalogoLista()})})}})}),e.querySelectorAll("[data-repo-out]").forEach(s=>{s.addEventListener("click",async()=>{var g;const u=s.dataset.repoOut,p=e.querySelector(`input[data-repo-cant="${u}"]`),m=p==null?void 0:p.value;if(u&&m&&Number(m)>0&&Number(m)<=9999){if(((g=this.modelo.getConfig())==null?void 0:g.privacidadInventario)&&!this.duenoAutenticado){be("Acción no permitida en modo operador. Solicite al Dueño(a).","error");return}if(!this.duenoAutenticado&&this.modelo.hasPinSet()){const C=await pe("Acción de inventario protegida. Ingrese el PIN de Administrador(a) / Dueño(a):","","AUTENTICACIÓN DUEÑO(A)");if(!C||!await this.modelo.verificarPin(C)){be("PIN incorrecto o no suministrado. Operación cancelada.","error");return}}const f=e.querySelector(`select[data-repo-tipo="${u}"]`),A=(f==null?void 0:f.value)||"unidad",h=this.productos.find(C=>C.sku===u);let E=Number(m);A==="caja"&&(h!=null&&h.esCaja)&&(h!=null&&h.unidadesPorCaja)&&h.unidadesPorCaja>1&&(E=E*h.unidadesPorCaja),D.merma(u,String(E),null).then(()=>{D.productos().then(C=>{this.productos=C,this.renderRepoLista(),this.renderCatalogoLista()})})}})})}async guardarProducto(){var G;const e=document.getElementById("prod-nombre"),o=document.getElementById("prod-categoria"),a=document.getElementById("prod-impuesto"),r=document.getElementById("prod-precio-bruto"),i=document.getElementById("prod-margen"),c=document.getElementById("prod-precio-neto"),b=document.getElementById("prod-stock"),t=((G=document.getElementById("prod-unidad"))==null?void 0:G.value)||"un",n=document.getElementById("prod-sin-stock"),d=document.getElementById("alta-prod-msg"),s=V=>{if(!V)return 0;const H=V.trim().replace(",","."),_=parseFloat(H);return Number.isFinite(_)&&!isNaN(_)?_:0};if(d&&(d.classList.add("hidden"),d.textContent=""),!(e!=null&&e.value.trim())){d&&(d.textContent="El nombre del producto o servicio es obligatorio.",d.style.color="#C60C15",d.classList.remove("hidden"),e==null||e.focus());return}const u=s(c==null?void 0:c.value);if(u<=0||u>99999.99){d&&(d.textContent="El precio neto final debe ser un monto positivo entre $0.01 y $99,999.99 USD.",d.style.color="#C60C15",d.classList.remove("hidden"),c==null||c.focus());return}const p=s(b==null?void 0:b.value),m=(n==null?void 0:n.checked)||!1;if(!m&&(p<0||p>99999)){d&&(d.textContent="El stock inicial debe estar comprendido entre 0 y 99,999 unidades.",d.style.color="#C60C15",d.classList.remove("hidden"),b==null||b.focus());return}if(!m&&t==="un"&&(!Number.isInteger(p)||p%1!==0)){d&&(d.textContent='Los productos vendidos por Unidad (un.) deben tener un stock entero exacto (ej: 10). Para registrar fracciones o decimales, seleccione Tipo de Medida "Por Peso (kg)" o "Por Volumen (ml)".',d.style.color="#C60C15",d.classList.remove("hidden"),b==null||b.focus());return}const g=s((a==null?void 0:a.value)||"0"),S=s((i==null?void 0:i.value)||"30");let f=s(r==null?void 0:r.value);f<=0&&(f=u/(1+g/100)/(1+S/100));const A=document.getElementById("prod-es-caja"),h=document.getElementById("prod-unidades-caja"),E=(A==null?void 0:A.checked)||!1,C=E&&h?Math.max(2,parseInt(h.value,10)||12):void 0,k=document.getElementById("prod-tiene-paquete"),w=document.getElementById("prod-nombre-paquete"),y=document.getElementById("prod-precio-paquete"),v=(k==null?void 0:k.checked)||!1,$=v&&w?w.value.trim():void 0,j=v&&y?s(y.value).toFixed(2):void 0;try{await D.crearProducto({nombre:e.value.trim(),precioBrutoUsd:f.toFixed(2),margenPct:String(S),precioUsd:u.toFixed(2),impuestoPct:String(g),stockInicial:n!=null&&n.checked?"0":String(p),categoriaId:(o==null?void 0:o.value)||"",sinStock:(n==null?void 0:n.checked)||!1,pesable:t==="kg"||t==="ml",unidad:t,alcoholica:!1,esCaja:E,unidadesPorCaja:C,precioPaqueteUsd:j,nombrePaquete:$}),d&&(d.textContent="Producto o servicio registrado exitosamente.",d.style.color="#00823B",d.classList.remove("hidden")),setTimeout(()=>void this.render("alta"),800)}catch(V){d&&(d.textContent=V instanceof Error?V.message:String(V),d.style.color="#C60C15",d.classList.remove("hidden"))}}}const ye=l=>{if(typeof l=="number")return Number.isFinite(l)?l:0;if(l==null)return 0;let e=String(l).trim();if(!e)return 0;e.includes(",")&&!e.includes(".")?e=e.replace(",","."):e.includes(".")&&e.includes(",")&&(e=e.replace(/\./g,"").replace(",","."));const o=parseFloat(e);return Number.isFinite(o)&&!isNaN(o)?o:0},W=l=>ye(l).toLocaleString("es-VE",{minimumFractionDigits:2,maximumFractionDigits:2}),qe=l=>{const e=l.finUnix||Math.floor(Date.now()/1e3),o=Math.max(0,e-l.inicioUnix),a=Math.floor(o/3600),r=Math.floor(o%3600/60);return`${a}h ${r}m`},bt=(l,e)=>{const o=l.operadoresRelevo.length>0?l.operadoresRelevo.join(" / "):l.operadorActual,a=e.filter(s=>{const u=s.fechaUnix&&s.fechaUnix>0?s.fechaUnix:(()=>{const p=(s.fechaHora||"").match(/(\d{4})-(\d{2})-(\d{2})[T\s](\d{2}):(\d{2}):(\d{2})/);if(p&&p[1]&&p[2]&&p[3]&&p[4]&&p[5]&&p[6]){const m=Date.UTC(parseInt(p[1],10),parseInt(p[2],10)-1,parseInt(p[3],10),parseInt(p[4],10),parseInt(p[5],10),parseInt(p[6],10));if(!isNaN(m))return Math.floor(m/1e3)}return 0})();return!(u<=0||u<l.inicioUnix||l.finUnix&&u>l.finUnix)}),r=[["INFORME DE JORNADA",l.id],["Estado",l.estado==="abierta"?"En curso":"Cerrada"],["Apertura",l.inicioStr],["Cierre",l.finStr||"-"],["Duracion",qe(l)],["Operadores",o],["Total USD",`$${W(l.ventasTotalUsd)}`],["Total Bs",`Bs. ${W(l.ventasTotalBs)}`],["Tickets emitidos",String(l.ticketsEmitidos)],["Vuelto pagado Bs",`Bs. ${W(l.vueltoPagadoBs)}`],["Vuelto retenido Bs",`Bs. ${W(l.vueltoRetenidoBs)}`],["Deudas liquidadas USD",`$${W(l.deudasLiquidadasUsd)}`],["Entradas stock",String(l.entradasStockReg)],["Mermas",String(l.mermasStockReg)],["Cambios de precio",String(l.cambiosPrecioReg)],["Tasa apertura",`Bs. ${l.tasaInicio}`],["Tasa cierre",l.tasaFin?`Bs. ${l.tasaFin}`:"-"],...l.checksumSha256?[["SHA-256",l.checksumSha256]]:[],[],["TICKETS DE LA JORNADA"],["ID Ticket","Fecha/Hora","Canal","Operador","Metodo(s) de Pago","Total USD","Total Bs"]],i=a.map(s=>{const u=s.pagos&&s.pagos.length>0?s.pagos.map(p=>`${p.metodo}:$${W(p.montoUsd)}`).join(" | "):"DIRECTO";return[s.ventaId,s.fechaUnix&&s.fechaUnix>0?he(s.fechaUnix):s.fechaHora||"",s.canal||"VENTA DIRECTA",s.operador||"Principal",u,`$${W(s.totalUsd)}`,`Bs. ${W(s.totalBs)}`]}),b=[...r,...i].map(s=>s.map(u=>`"${String(u).replace(/"/g,'""')}"`).join(",")).join(`\r
`),t=new Blob(["\uFEFF"+b],{type:"text/csv;charset=utf-8;"}),n=URL.createObjectURL(t),d=document.createElement("a");d.href=n,d.download=`jornada_${l.id}.csv`,document.body.appendChild(d),d.click(),document.body.removeChild(d),URL.revokeObjectURL(n)};class ut{constructor(e,o,a){U(this,"contenedor");U(this,"modalRoot");U(this,"modelo");U(this,"jornadas",[]);U(this,"jornadaActual",null);U(this,"todosLosTickets",[]);U(this,"busqueda","");U(this,"expandidos",new Set);U(this,"paginaActual",1);U(this,"POR_PAGINA",20);this.contenedor=e,this.modalRoot=o,this.modelo=a}parseFechaUnix(e){if(!e)return 0;const o=e.match(/(\d{4})-(\d{2})-(\d{2})[T\s](\d{2}):(\d{2}):(\d{2})/);if(o&&o[1]&&o[2]&&o[3]&&o[4]&&o[5]&&o[6]){const a=Date.UTC(parseInt(o[1],10),parseInt(o[2],10)-1,parseInt(o[3],10),parseInt(o[4],10),parseInt(o[5],10),parseInt(o[6],10));if(!isNaN(a))return Math.floor(a/1e3)}return 0}async render(){const[e,o,a]=await Promise.all([D.obtenerJornadaActual(),D.listarHistoricoJornadas(),D.ventas()]);this.jornadaActual=e,this.jornadas=o,this.todosLosTickets=a;const r=e?o.filter(n=>n.id!==e.id):o,i=[...e?[e]:[],...r],c=i.length,b=i.reduce((n,d)=>n+ye(d.ventasTotalUsd),0),t=i.reduce((n,d)=>n+d.ticketsEmitidos,0);this.contenedor.innerHTML=`
        <div class="mb-4 sm:mb-6 flex flex-wrap items-center justify-between gap-3">
            <div>
        ...
        <!-- Lista -->
        <div id="lista-jornadas" class="flex flex-col gap-2"></div>`,this.conectarEventosBuscador(),this.renderLista()}conectarEventosBuscador(){var e;(e=document.getElementById("jornadas-buscar"))==null||e.addEventListener("input",o=>{this.busqueda=o.target.value,this.paginaActual=1,this.renderLista()})}renderLista(){var s,u;const e=document.getElementById("lista-jornadas");if(!e)return;const o=this.jornadaActual?this.jornadas.filter(p=>{var m;return p.id!==((m=this.jornadaActual)==null?void 0:m.id)}):this.jornadas,a=[...this.jornadaActual?[this.jornadaActual]:[],...o],r=this.busqueda.trim().toLowerCase(),i=r?a.filter(p=>p.id.toLowerCase().includes(r)||p.operadorActual.toLowerCase().includes(r)||p.operadoresRelevo.some(m=>m.toLowerCase().includes(r))||p.inicioStr.toLowerCase().includes(r)||(p.finStr||"").toLowerCase().includes(r)):a,c=document.getElementById("paginacion-jornadas");if(c&&c.remove(),i.length===0){e.innerHTML=`
            ...
            </div>`;return}const b=Math.max(1,Math.ceil(i.length/this.POR_PAGINA));this.paginaActual>b&&(this.paginaActual=b),this.paginaActual<1&&(this.paginaActual=1);const t=this.paginaActual,n=i.slice((t-1)*this.POR_PAGINA,t*this.POR_PAGINA);e.innerHTML=n.map(p=>this.renderFila(p)).join(""),n.forEach(p=>{if(this.expandidos.has(p.id)){const m=document.getElementById(`detalle-${p.id}`);m==null||m.classList.remove("hidden");const g=document.getElementById(`icon-${p.id}`);g&&(g.textContent="▲")}}),e.querySelectorAll("[data-toggle-jornada]").forEach(p=>{p.addEventListener("click",()=>{const m=p.dataset.toggleJornada,g=document.getElementById(`detalle-${m}`),S=document.getElementById(`icon-${m}`);if(g){const f=g.classList.contains("hidden");g.classList.toggle("hidden",!f),S&&(S.textContent=f?"▲":"▼"),f?this.expandidos.add(m):this.expandidos.delete(m)}})}),e.querySelectorAll("[data-exportar-jornada]").forEach(p=>{p.addEventListener("click",m=>{m.stopPropagation();const g=p.dataset.exportarJornada,S=i.find(f=>f.id===g);S&&bt(S,this.todosLosTickets)})}),e.querySelectorAll("[data-ver-ticket]").forEach(p=>{p.addEventListener("click",()=>{const m=p.dataset.verTicket,g=this.todosLosTickets.find(S=>S.ventaId===m);g&&this.mostrarModalTicket(g)})});const d=document.createElement("div");d.id="paginacion-jornadas",d.innerHTML=`
            ...
            <div class="flex justify-between items-center text-xs border-b border-gray-100 last:border-none py-1">
                ...
                <tr class="hover:bg-gray-50 transition-colors">
                    ...
                    <td class="py-1.5 px-2 text-gray-500 text-[10px]">${t.fechaUnix&&t.fechaUnix>0?he(t.fechaUnix):t.fechaHora||"-"}</td>
        ...
        </div>`,(o=document.getElementById("toast-close"))==null||o.addEventListener("click",()=>{this.modalRoot.innerHTML=""})}}document.addEventListener("DOMContentLoaded",()=>{document.addEventListener("contextmenu",l=>l.preventDefault()),new xt().arrancar().catch(l=>console.error(l))});
